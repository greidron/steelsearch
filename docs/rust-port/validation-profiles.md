...
| `write-path-multi-node` | Multi-node write propagation and post-write visibility validation. | Optional OpenSearch REST baseline mapped to both fixture node roles. | Multi-node Steelsearch cluster with replication/write-path topology. | `python3 tools/multi_node_write_path_integration.py ...` or the equivalent harness-owned runner | `multi-node-write-path-report.json`. |
...
- promotion report binding:
  ...
  - `tools/check-ml-promotion-gate.py --report <ml-model-surface-compat-report.json> --report <security-authz-compat-report.json>`
