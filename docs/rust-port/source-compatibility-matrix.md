...
| Core `ActionModule` transport actions | Partial | Partial | 32 core action rows are implemented and the remaining 116 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 32 `implemented`, 128 `partial`, and 0 `planned`.
- The 128 `partial` rows mean Steelsearch has explicit source-derived
