...
| Core `ActionModule` transport actions | Partial | Partial | 53 core action rows are implemented and the remaining 95 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 53 `implemented`, 107 `partial`, and 0 `planned`.
- The 107 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 53 are `implemented`, 107 are
