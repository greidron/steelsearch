...
| Core `ActionModule` transport actions | Partial | Partial | 31 core action rows are implemented and the remaining 117 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 31 `implemented`, 129 `partial`, and 0 `planned`.
- The 129 `partial` rows mean Steelsearch has explicit source-derived
