...
| Core `ActionModule` transport actions | Partial | Partial | 20 core action rows are implemented and the remaining 128 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 20 `implemented`, 140 `partial`, and 0 `planned`.
- The 140 `partial` rows mean Steelsearch has explicit source-derived
