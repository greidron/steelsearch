...
| Core `ActionModule` transport actions | Partial | Partial | 19 core action rows are implemented and the remaining 129 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 19 `implemented`, 141 `partial`, and 0 `planned`.
- The 141 `partial` rows mean Steelsearch has explicit source-derived
