...
| Core `ActionModule` transport actions | Partial | Partial | 101 core action rows are implemented and the remaining 47 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 101 `implemented`, 59 `partial`, and 0 `planned`.
- The 59 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 101 are `implemented`, 59 are
