...
| Core `ActionModule` transport actions | Partial | Partial | 26 core action rows are implemented and the remaining 122 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 26 `implemented`, 134 `partial`, and 0 `planned`.
- The 134 `partial` rows mean Steelsearch has explicit source-derived
