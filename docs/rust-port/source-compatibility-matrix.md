...
| Core `ActionModule` transport actions | Partial | Partial | 100 core action rows are implemented and the remaining 48 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 100 `implemented`, 60 `partial`, and 0 `planned`.
- The 60 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 100 are `implemented`, 60 are
