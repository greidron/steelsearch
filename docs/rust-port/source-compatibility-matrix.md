...
| Core `ActionModule` transport actions | Partial | Partial | 103 core action rows are implemented and the remaining 45 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 103 `implemented`, 57 `partial`, and 0 `planned`.
- The 57 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 103 are `implemented`, 57 are
