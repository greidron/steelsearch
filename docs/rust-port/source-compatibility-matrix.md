...
| Core `ActionModule` transport actions | Partial | Partial | 69 core action rows are implemented and the remaining 79 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 69 `implemented`, 91 `partial`, and 0 `planned`.
- The 91 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 69 are `implemented`, 91 are
