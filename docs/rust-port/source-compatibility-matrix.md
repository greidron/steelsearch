...
| Core `ActionModule` transport actions | Partial | Partial | 87 core action rows are implemented and the remaining 61 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 87 `implemented`, 73 `partial`, and 0 `planned`.
- The 73 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 87 are `implemented`, 73 are
