...
| Core `ActionModule` transport actions | Partial | Partial | 35 core action rows are implemented and the remaining 113 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 35 `implemented`, 125 `partial`, and 0 `planned`.
- The 125 `partial` rows mean Steelsearch has explicit source-derived
