...
| Core `ActionModule` transport actions | Partial | Partial | 93 core action rows are implemented and the remaining 55 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 93 `implemented`, 67 `partial`, and 0 `planned`.
- The 67 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 93 are `implemented`, 67 are
