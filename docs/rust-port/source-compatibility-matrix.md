...
| Core `ActionModule` transport actions | Partial | Partial | 99 core action rows are implemented and the remaining 49 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 99 `implemented`, 61 `partial`, and 0 `planned`.
- The 61 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 99 are `implemented`, 61 are
