...
| Core `ActionModule` transport actions | Partial | Partial | 127 core action rows are implemented and the remaining 21 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 127 `implemented`, 33 `partial`, and 0 `planned`.
- The 33 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 127 are `implemented`, 33 are
