...
| Core `ActionModule` transport actions | Partial | Partial | 110 core action rows are implemented and the remaining 38 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 110 `implemented`, 50 `partial`, and 0 `planned`.
- The 50 `partial` rows mean Steelsearch has explicit source-derived
