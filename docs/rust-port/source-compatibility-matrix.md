...
| Core `ActionModule` transport actions | Partial | Partial | 112 core action rows are implemented and the remaining 36 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 112 `implemented`, 48 `partial`, and 0 `planned`.
- The 48 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 112 are `implemented`, 48 are
