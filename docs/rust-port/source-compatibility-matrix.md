...
| Core `ActionModule` transport actions | Partial | Partial | 16 core action rows are implemented and the remaining 132 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 16 `implemented`, 144 `partial`, and 0 `planned`.
- The 144 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 16 are `implemented`, 144 are
