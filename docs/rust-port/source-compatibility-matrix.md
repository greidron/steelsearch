...
| Core `ActionModule` transport actions | Partial | Partial | 111 core action rows are implemented and the remaining 37 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 111 `implemented`, 49 `partial`, and 0 `planned`.
- The 49 `partial` rows mean Steelsearch has explicit source-derived
