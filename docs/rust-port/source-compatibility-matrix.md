...
| Core `ActionModule` transport actions | Partial | Partial | 13 core action rows are implemented and the remaining 135 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 13 `implemented`, 147 `partial`, and 0 `planned`.
- The 147 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 13 are `implemented`, 147 are
