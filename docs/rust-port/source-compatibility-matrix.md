...
| Core `ActionModule` transport actions | Partial | Partial | 107 core action rows are implemented and the remaining 41 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 107 `implemented`, 53 `partial`, and 0 `planned`.
- The 53 `partial` rows mean Steelsearch has explicit source-derived
