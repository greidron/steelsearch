...
| Core `ActionModule` transport actions | Partial | Partial | 22 core action rows are implemented and the remaining 126 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 22 `implemented`, 138 `partial`, and 0 `planned`.
- The 138 `partial` rows mean Steelsearch has explicit source-derived
