...
| Core `ActionModule` transport actions | Partial | Partial | 129 core action rows are implemented and the remaining 19 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 129 `implemented`, 31 `partial`, and 0 `planned`.
- The 31 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 129 are `implemented`, 31 are
