...
| Core `ActionModule` transport actions | Partial | Partial | 104 core action rows are implemented and the remaining 44 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 104 `implemented`, 56 `partial`, and 0 `planned`.
- The 56 `partial` rows mean Steelsearch has explicit source-derived
