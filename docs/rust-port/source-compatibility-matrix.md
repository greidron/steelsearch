...
| Core `ActionModule` transport actions | Partial | Partial | 17 core action rows are implemented and the remaining 131 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 17 `implemented`, 143 `partial`, and 0 `planned`.
- The 143 `partial` rows mean Steelsearch has explicit source-derived
