...
| Core `ActionModule` transport actions | Partial | Partial | 140 core action rows are implemented and the remaining 8 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 140 `implemented`, 20 `partial`, and 0 `planned`.
- The 20 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 140 are `implemented`, 20 are
