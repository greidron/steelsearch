...
| Core `ActionModule` transport actions | Partial | Partial | 61 core action rows are implemented and the remaining 87 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 61 are `implemented`, 99 are
