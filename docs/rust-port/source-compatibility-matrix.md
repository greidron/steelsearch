...
| Core `ActionModule` transport actions | Partial | Partial | 66 core action rows are implemented and the remaining 82 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 66 `implemented`, 94 `partial`, and 0 `planned`.
- The 94 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 66 are `implemented`, 94 are
