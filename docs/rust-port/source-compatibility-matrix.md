...
| Core `ActionModule` transport actions | Partial | Partial | 95 core action rows are implemented and the remaining 53 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 95 `implemented`, 65 `partial`, and 0 `planned`.
- The 65 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 95 are `implemented`, 65 are
