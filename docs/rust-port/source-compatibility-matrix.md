...
| Core `ActionModule` transport actions | Partial | Partial | 27 core action rows are implemented and the remaining 121 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 27 `implemented`, 133 `partial`, and 0 `planned`.
- The 133 `partial` rows mean Steelsearch has explicit source-derived
