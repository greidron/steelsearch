...
| Core `ActionModule` transport actions | Partial | Partial | 65 core action rows are implemented and the remaining 83 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 65 `implemented`, 95 `partial`, and 0 `planned`.
- The 95 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 65 are `implemented`, 95 are
