...
| Core `ActionModule` transport actions | Partial | Partial | 133 core action rows are implemented and the remaining 15 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 133 `implemented`, 27 `partial`, and 0 `planned`.
- The 27 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 133 are `implemented`, 27 are
