...
| Core `ActionModule` transport actions | Partial | Partial | 141 core action rows are implemented and the remaining 7 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 141 `implemented`, 19 `partial`, and 0 `planned`.
- The 19 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 141 are `implemented`, 19 are
