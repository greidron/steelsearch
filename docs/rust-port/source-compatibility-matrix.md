...
| Core `ActionModule` transport actions | Partial | Partial | 122 core action rows are implemented and the remaining 26 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 122 `implemented`, 38 `partial`, and 0 `planned`.
- The 38 `partial` rows mean Steelsearch has explicit source-derived
