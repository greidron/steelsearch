...
| Core `ActionModule` transport actions | Partial | Partial | 108 core action rows are implemented and the remaining 40 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 108 `implemented`, 52 `partial`, and 0 `planned`.
- The 52 `partial` rows mean Steelsearch has explicit source-derived
