...
| Core `ActionModule` transport actions | Partial | Partial | 67 core action rows are implemented and the remaining 81 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 67 `implemented`, 93 `partial`, and 0 `planned`.
- The 93 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 67 are `implemented`, 93 are
