...
| Core `ActionModule` transport actions | Partial | Partial | 28 core action rows are implemented and the remaining 120 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 28 `implemented`, 132 `partial`, and 0 `planned`.
- The 132 `partial` rows mean Steelsearch has explicit source-derived
