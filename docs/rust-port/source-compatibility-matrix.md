...
| Core `ActionModule` transport actions | Partial | Partial | 30 core action rows are implemented and the remaining 118 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 30 `implemented`, 130 `partial`, and 0 `planned`.
- The 130 `partial` rows mean Steelsearch has explicit source-derived
