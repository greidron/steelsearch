...
| Core `ActionModule` transport actions | Partial | Partial | 42 core action rows are implemented and the remaining 106 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 42 `implemented`, 118 `partial`, and 0 `planned`.
- The 118 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  registration rows, 112 are now classified as `implemented` from Rust DSL and
  engine evidence, while 15 remain `planned`.
