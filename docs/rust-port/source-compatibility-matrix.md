...
| Core `ActionModule` transport actions | Partial | Partial | 83 core action rows are implemented and the remaining 65 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 83 `implemented`, 77 `partial`, and 0 `planned`.
- The 77 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 83 are `implemented`, 77 are
