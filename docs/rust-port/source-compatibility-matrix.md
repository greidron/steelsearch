...
| Core `ActionModule` transport actions | Partial | Partial | 97 core action rows are implemented and the remaining 51 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 97 `implemented`, 63 `partial`, and 0 `planned`.
- The 63 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 97 are `implemented`, 63 are
