...
| Core `ActionModule` transport actions | Partial | Partial | 105 core action rows are implemented and the remaining 43 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 105 `implemented`, 55 `partial`, and 0 `planned`.
- The 55 `partial` rows mean Steelsearch has explicit source-derived
