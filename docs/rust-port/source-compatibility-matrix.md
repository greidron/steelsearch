...
| Core `ActionModule` transport actions | Partial | Partial | 33 core action rows are implemented and the remaining 115 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 33 `implemented`, 127 `partial`, and 0 `planned`.
- The 127 `partial` rows mean Steelsearch has explicit source-derived
