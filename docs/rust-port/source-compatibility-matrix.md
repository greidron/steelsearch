...
| Core `ActionModule` transport actions | Partial | Partial | 90 core action rows are implemented and the remaining 58 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 90 `implemented`, 70 `partial`, and 0 `planned`.
- The 70 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 90 are `implemented`, 70 are
