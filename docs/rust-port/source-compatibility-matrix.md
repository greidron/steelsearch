...
| Core `ActionModule` transport actions | Partial | Partial | 15 core action rows are implemented and the remaining 133 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 15 `implemented`, 145 `partial`, and 0 `planned`.
- The 145 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 15 are `implemented`, 145 are
