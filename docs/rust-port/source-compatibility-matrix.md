...
| Core `ActionModule` transport actions | Partial | Partial | 91 core action rows are implemented and the remaining 57 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 91 `implemented`, 69 `partial`, and 0 `planned`.
- The 69 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 91 are `implemented`, 69 are
