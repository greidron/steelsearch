...
| Core `ActionModule` transport actions | Partial | Partial | 21 core action rows are implemented and the remaining 127 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 21 `implemented`, 139 `partial`, and 0 `planned`.
- The 139 `partial` rows mean Steelsearch has explicit source-derived
