...
| Core `ActionModule` transport actions | Partial | Partial | 130 core action rows are implemented and the remaining 18 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 130 `implemented`, 30 `partial`, and 0 `planned`.
- The 30 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 130 are `implemented`, 30 are
