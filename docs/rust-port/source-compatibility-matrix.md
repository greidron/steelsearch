...
| Core `ActionModule` transport actions | Partial | Partial | 94 core action rows are implemented and the remaining 54 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 94 `implemented`, 66 `partial`, and 0 `planned`.
- The 66 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 94 are `implemented`, 66 are
