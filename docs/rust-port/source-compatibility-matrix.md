...
| Core `ActionModule` transport actions | Partial | Partial | 23 core action rows are implemented and the remaining 125 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 23 `implemented`, 137 `partial`, and 0 `planned`.
- The 137 `partial` rows mean Steelsearch has explicit source-derived
