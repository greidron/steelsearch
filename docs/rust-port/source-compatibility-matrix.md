...
| Core `ActionModule` transport actions | Partial | Partial | 34 core action rows are implemented and the remaining 114 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 34 `implemented`, 126 `partial`, and 0 `planned`.
- The 126 `partial` rows mean Steelsearch has explicit source-derived
