...
| Core `ActionModule` transport actions | Partial | Partial | 126 core action rows are implemented and the remaining 22 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 126 `implemented`, 34 `partial`, and 0 `planned`.
- The 34 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 126 are `implemented`, 34 are
