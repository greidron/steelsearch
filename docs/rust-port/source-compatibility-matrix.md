...
| Core `ActionModule` transport actions | Partial | Partial | 40 core action rows are implemented and the remaining 108 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
