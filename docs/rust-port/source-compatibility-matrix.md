...
| Core `ActionModule` transport actions | Partial | Partial | 125 core action rows are implemented and the remaining 23 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 125 `implemented`, 35 `partial`, and 0 `planned`.
- The 35 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 125 are `implemented`, 35 are
