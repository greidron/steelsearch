...
| Core `ActionModule` transport actions | Partial | Partial | 82 core action rows are implemented and the remaining 66 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 82 `implemented`, 78 `partial`, and 0 `planned`.
- The 78 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 82 are `implemented`, 78 are
