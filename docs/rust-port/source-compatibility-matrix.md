...
| Core `ActionModule` transport actions | Partial | Partial | 121 core action rows are implemented and the remaining 27 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 121 `implemented`, 39 `partial`, and 0 `planned`.
- The 39 `partial` rows mean Steelsearch has explicit source-derived
