...
- `tools/fixtures/interop-accepted-transport-action-evidence.json` records 174
  implemented transport evidence rows: 166 `bounded_local_subset`, 0
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  the node runtime rows, 71 are classified as `partial` because
  Steelsearch has corresponding bounded runtime owners, while 7 remain
