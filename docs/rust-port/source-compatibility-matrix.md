...
| Core `ActionModule` transport actions | Partial | Partial | 92 core action rows are implemented and the remaining 56 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 92 `implemented`, 68 `partial`, and 0 `planned`.
- The 68 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 92 are `implemented`, 68 are
