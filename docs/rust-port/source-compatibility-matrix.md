...
| Core `ActionModule` transport actions | Partial | Partial | 62 core action rows are implemented and the remaining 86 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 62 are `implemented`, 98 are
