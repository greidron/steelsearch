...
| Core `ActionModule` transport actions | Partial | Partial | 109 core action rows are implemented and the remaining 39 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 109 `implemented`, 51 `partial`, and 0 `planned`.
- The 51 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 104 are `implemented`, 56 are
