...
| Core `ActionModule` transport actions | Partial | Partial | 29 core action rows are implemented and the remaining 119 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 29 `implemented`, 131 `partial`, and 0 `planned`.
- The 131 `partial` rows mean Steelsearch has explicit source-derived
