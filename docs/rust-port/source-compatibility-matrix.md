...
| Core `ActionModule` transport actions | Partial | Partial | 96 core action rows are implemented and the remaining 52 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 96 `implemented`, 64 `partial`, and 0 `planned`.
- The 64 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 96 are `implemented`, 64 are
