...
| Core `ActionModule` transport actions | Partial | Partial | 38 core action rows are implemented and the remaining 110 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
