...
| Core `ActionModule` transport actions | Partial | Partial | 132 core action rows are implemented and the remaining 16 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 132 `implemented`, 28 `partial`, and 0 `planned`.
- The 28 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 132 are `implemented`, 28 are
