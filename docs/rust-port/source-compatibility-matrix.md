...
| Core `ActionModule` transport actions | Partial | Partial | 41 core action rows are implemented and the remaining 107 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 36 `implemented`, 124 `partial`, and 0 `planned`.
- The 124 `partial` rows mean Steelsearch has explicit source-derived
