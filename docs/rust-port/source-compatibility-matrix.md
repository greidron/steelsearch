...
| Core `ActionModule` transport actions | Partial | Partial | 139 core action rows are implemented and the remaining 9 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 139 `implemented`, 21 `partial`, and 0 `planned`.
- The 21 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 139 are `implemented`, 21 are
