...
| Core `ActionModule` transport actions | Partial | Partial | 102 core action rows are implemented and the remaining 46 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 102 `implemented`, 58 `partial`, and 0 `planned`.
- The 58 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 102 are `implemented`, 58 are
