...
| Core `ActionModule` transport actions | Partial | Partial | 138 core action rows are implemented and the remaining 10 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 138 `implemented`, 22 `partial`, and 0 `planned`.
- The 22 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 138 are `implemented`, 22 are
