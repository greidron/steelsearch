...
| Core `ActionModule` transport actions | Partial | Partial | 135 core action rows are implemented and the remaining 13 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 135 `implemented`, 25 `partial`, and 0 `planned`.
- The 25 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 135 are `implemented`, 25 are
