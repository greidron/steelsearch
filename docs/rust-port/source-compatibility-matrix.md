...
| Core `ActionModule` transport actions | Partial | Partial | 84 core action rows are implemented and the remaining 64 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 84 `implemented`, 76 `partial`, and 0 `planned`.
- The 76 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 84 are `implemented`, 76 are
