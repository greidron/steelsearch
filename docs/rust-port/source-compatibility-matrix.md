...
| Core `ActionModule` transport actions | Partial | Partial | 18 core action rows are implemented and the remaining 130 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 18 `implemented`, 142 `partial`, and 0 `planned`.
- The 142 `partial` rows mean Steelsearch has explicit source-derived
