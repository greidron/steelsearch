...
| Core `ActionModule` transport actions | Partial | Partial | 59 core action rows are implemented and the remaining 89 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 59 `implemented`, 101 `partial`, and 0 `planned`.
- The 101 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 59 are `implemented`, 101 are
