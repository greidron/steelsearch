...
| Core `ActionModule` transport actions | Partial | Partial | 106 core action rows are implemented and the remaining 42 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 106 `implemented`, 54 `partial`, and 0 `planned`.
- The 54 `partial` rows mean Steelsearch has explicit source-derived
