...
| Core `ActionModule` transport actions | Partial | Partial | 98 core action rows are implemented and the remaining 50 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 98 `implemented`, 62 `partial`, and 0 `planned`.
- The 62 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 98 are `implemented`, 62 are
