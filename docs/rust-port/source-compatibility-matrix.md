...
| Core `ActionModule` transport actions | Partial | Partial | 81 core action rows are implemented and the remaining 67 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 81 `implemented`, 79 `partial`, and 0 `planned`.
- The 79 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 81 are `implemented`, 79 are
