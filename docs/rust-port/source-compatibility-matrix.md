...
| Core `ActionModule` transport actions | Partial | Partial | 86 core action rows are implemented and the remaining 62 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 86 `implemented`, 74 `partial`, and 0 `planned`.
- The 74 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 86 are `implemented`, 74 are
