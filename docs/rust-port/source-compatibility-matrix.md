...
| Core `ActionModule` transport actions | Partial | Partial | 134 core action rows are implemented and the remaining 14 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 134 `implemented`, 26 `partial`, and 0 `planned`.
- The 26 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 134 are `implemented`, 26 are
