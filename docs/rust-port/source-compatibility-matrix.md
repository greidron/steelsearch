...
| Core `ActionModule` transport actions | Partial | Partial | 60 core action rows are implemented and the remaining 88 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 60 `implemented`, 100 `partial`, and 0 `planned`.
- The 100 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 60 are `implemented`, 100 are
