...
| Core `ActionModule` transport actions | Partial | Partial | 131 core action rows are implemented and the remaining 17 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 131 `implemented`, 29 `partial`, and 0 `planned`.
- The 29 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 131 are `implemented`, 29 are
