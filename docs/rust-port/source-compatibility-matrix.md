...
| Core `ActionModule` transport actions | Partial | Partial | 123 core action rows are implemented and the remaining 25 core rows have explicit fail-closed transport boundaries; most server-side execution semantics remain partial. |
...
- `tools/report-transport-action-coverage.py` compares the source-derived
  ...
  actions: 123 `implemented`, 37 `partial`, and 0 `planned`.
- The 37 `partial` rows mean Steelsearch has explicit source-derived
...
- Current source-derived inventory is not an exhaustive OpenSearch API
  ...
  runtime entries. Of the transport source rows, 123 are `implemented`, 37 are
