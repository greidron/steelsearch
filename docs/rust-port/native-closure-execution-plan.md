...
- The same runner now has a `production-security` batch. It passed on
  2026-06-17 with 15/15 tests and `zero_tests=0`, covering runtime env user,
  ...
  management mutation routes, and task cancel/rethrottle surfaces, and explicit
...
- `tools/run-native-closure-validation.py --batch production-security` must
  ...
  and template management mutation routes, bounded security audit event
