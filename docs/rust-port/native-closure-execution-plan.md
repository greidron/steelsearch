...
- The same runner now has a `production-security` batch. It passed on
  2026-06-17 with 26/26 tests and `zero_tests=0`, covering runtime env user,
  ...
  audit events, bulk/search/session allow/deny checks, reindex,
  delete-by-query, and update-by-query writer-route checks, service-account
  writer authz,
