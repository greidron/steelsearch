...
- The same runner now has a `runtime-backpressure` batch. It passed on
  2026-06-17 with 6/6 tests and `zero_tests=0`, covering administrative
  ...
  admission through the same runtime-owned waiting and rejection model.
...
- `tools/run-native-closure-validation.py --batch runtime-backpressure` must
  ...
  plus snapshot route admission as runtime-control evidence.
