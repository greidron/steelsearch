...
- The same runner now has a `production-security` batch. It passed on
  2026-06-17 with 14/14 tests and `zero_tests=0`, covering runtime env user,
  ...
  voting config exclusions, snapshot mutation/control routes, and task
...
- `tools/run-native-closure-validation.py --batch production-security` must
  ...
  weighted routing, voting config exclusions, and snapshot mutation/control
  routes, bounded security audit event persistence for authn/authz/fail-closed
  and shared permission-evaluator read/write denials, ML connector secret
  redaction, and OpenSearch Security plugin API fail-closed responses plus persisted audit
