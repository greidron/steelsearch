...
- The same runner now has a `production-security` batch. It passed on
  2026-06-17 with 22/22 tests and `zero_tests=0`, covering runtime env user,
  ...
  index maintenance control routes, index structure mutation routes, and task
...
- `tools/run-native-closure-validation.py --batch production-security` must
  ...
  delete/block/resize controls, bounded security audit event persistence for
