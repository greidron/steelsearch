...
- The same runner now has a `source-backed-query` batch. It passed on
  2026-06-17 with 35/35 tests and `zero_tests=0`, covering native execution,
...
- `nested` exact scalar, exists, and string prefix leaves: child ordinal indexes
  now narrow term/terms/exists/prefix and supported bool nested leaves before
  parent document lookup. Unsupported nested shapes still use the explicit
