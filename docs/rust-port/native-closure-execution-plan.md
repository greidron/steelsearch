...
- The same runner now has a `runtime-backpressure` batch. It passed on
  2026-06-17 with 5/5 tests and `zero_tests=0`, covering administrative
  ...
  runtime-owned waiting and rejection model.
...
- `tools/run-native-closure-validation.py --batch runtime-backpressure` must
  ...
  admission as runtime-control evidence.
...
3. queue/backpressure smoke tests for search/write/admin routes. Runtime queue
   ...
   overload rejection are now guarded for search/write and maintenance pools;
