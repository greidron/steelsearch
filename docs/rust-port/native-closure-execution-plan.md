...
broader release-record inventory has a seventh required item,
`promotion_gate_suite`, generated by
`tools/check-all-promotion-gates.py --output`; this item is archived with the
release record but is not part of the production startup manifest accepted by
...
- `tools/report-release-evidence-inventory.py --require-complete` now also
  requires the release-record-only `promotion_gate_suite` artifact, so the full
  release record is 7/7 ready when promotion gates, startup items, and
  readiness attachments are all present.
