...
- The same runner now has a `source-backed-query` batch. It passed on
  2026-06-17 with 34/34 tests and `zero_tests=0`, covering native execution,
...
- `nested` exact scalar and exists leaves: child ordinal indexes now narrow
  term/terms/exists and supported bool nested leaves before parent document
  lookup. Unsupported nested shapes still use the explicit source-validation
  fallback.
