...
- The OpenSearch peer comparison harness
  `tools/compare_remote_transport_backpressure.py --mode both --profile
  mixed-java-rust-query-phase` passed on 2026-06-18 against the local OpenSearch
  3.7.0-SNAPSHOT distribution. It observed Steelsearch `remote_transport`
  `rejected=1` / `completed=1` readback and OpenSearch `search` thread-pool
...
- `tools/run-native-closure-validation.py --batch runtime-peer-backpressure`
  ...
  promoting peer backpressure parity evidence. The batch requires the
  `mixed-java-rust-query-phase` profile so the generated report names the Rust
  `indices:data/read/search[phase/query]` remote-transport surface, the Java
  search-thread-pool analogue, the required REST readbacks, and the current
  independent-probe limit explicitly.
...
broader release-record inventory has two additional required items:
`pit_e2e_coverage`, taken from the OpenSearch-compared PIT unified E2E report,
and `promotion_gate_suite`, generated by
`tools/check-all-promotion-gates.py --output`; these items are archived with the
release record but are not part of the production startup manifest accepted by
`STEELSEARCH_RELEASE_READINESS_FILE`.
The same current status artifact can be generated through
...
to generate the promotion gate suite artifact and refresh the current
...
- `tools/report-release-evidence-inventory.py --require-complete` now also
  requires release-record-only `pit_e2e_coverage` and `promotion_gate_suite`
  artifacts, so the full release record is 8/8 ready when PIT OpenSearch
  comparison, promotion gates, startup items, and readiness attachments are all
  present.
