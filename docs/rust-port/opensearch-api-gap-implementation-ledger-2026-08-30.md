...
|---|---|---|---|---|
| P1 | Snapshot/restore | Broader restore option combinations: feature states and partial shard failure materialization. | Cutover automation can depend on exact failure semantics. | Add rows to `tools/fixtures/snapshot-lifecycle-compat.json` and compare against live OpenSearch. |
