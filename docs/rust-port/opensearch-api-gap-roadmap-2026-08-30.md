...
| P1 | Snapshot/clone | Snapshot clone `indices` arrays dropped `null` selector values, so mixed arrays such as `[null, "logs-snapshot-clone-a"]` could silently clone the valid selector instead of failing like OpenSearch. | Malformed or generated client payloads could receive a false successful clone and later restore unintended snapshot state. | Fixed in follow-up pass; tracked as `API-SNAPSHOT-CLONE-NULL-SELECTOR-001`. |
...
- Snapshot clone now coerces numeric scalar values in `indices` arrays to
  selector strings before resolution, matching OpenSearch's
  `404 index_not_found_exception` response for missing numeric selectors.
...
- Follow-up snapshot feature-state restore validation:
  - Targeted runtime tests:
    `RUSTFLAGS='-Awarnings' cargo +nightly test -q -p os-node snapshot_restore_fails_closed_for_unsupported_remote_backed_options --features standalone-runtime -- --nocapture`
    and
    `RUSTFLAGS='-Awarnings' cargo +nightly test -q -p os-node snapshot_restore --features standalone-runtime -- --nocapture`
  ...
  - Full benchmark:
    `target/search-benchmark-matrix-api-feature-states-fail-closed-full-20260830/summary.json`
...
The snapshot clone numeric selector follow-up reported:

| Topology | SteelSearch Throughput | OpenSearch Throughput | Ratio | Refresh p99 |
|---|---:|---:|---:|---:|
| single-node | 722.067 ops/s | 222.347 ops/s | 3.25x | 34.909 ms |
| three-node | 878.050 ops/s | 75.866 ops/s | 11.57x | 22.329 ms |

The live snapshot lifecycle compare at
`target/api-gap-snapshot-clone-invalid-indices-fixed3-20260830/snapshot-lifecycle-compat-report.json`
reported `31 passed, 0 failed, 0 skipped`; the numeric selector case matched
OpenSearch's `404 index_not_found_exception` response for index `7`. The full
benchmark at
`target/search-benchmark-matrix-api-snapshot-clone-invalid-indices-full-20260830/summary.json`
reported no SteelSearch-slower-than-OpenSearch metrics.

The snapshot clone boolean selector follow-up reported:

| Topology | SteelSearch Throughput | OpenSearch Throughput | Ratio | Refresh p99 |
|---|---:|---:|---:|---:|
| single-node | 734.943 ops/s | 223.031 ops/s | 3.30x | 21.099 ms |
| three-node | 886.241 ops/s | 85.000 ops/s | 10.43x | 22.768 ms |

The live snapshot lifecycle compare at
`target/api-gap-snapshot-clone-boolean-indices-20260830/snapshot-lifecycle-compat-report.json`
reported `32 passed, 0 failed, 0 skipped`; the boolean selector case matched
...
The snapshot clone null selector follow-up reported:

| Topology | SteelSearch Throughput | OpenSearch Throughput | Ratio | Refresh p99 |
|---|---:|---:|---:|---:|
| single-node | 726.983 ops/s | 221.269 ops/s | 3.29x | 21.702 ms |
| three-node | 889.660 ops/s | 86.747 ops/s | 10.26x | 20.271 ms |

The live snapshot lifecycle compare at
`target/api-gap-snapshot-clone-null-indices-fixed3-20260830/snapshot-lifecycle-compat-report.json`
reported `35 passed, 0 failed, 0 skipped`; both null-only and mixed-null
selector cases matched OpenSearch's `400 action_request_validation_exception`
with reason `Validation Failed: 1: index is null;`. The full benchmark at
`target/search-benchmark-matrix-api-snapshot-clone-null-indices-full-20260830/summary.json`
reported no SteelSearch-slower-than-OpenSearch metrics. The single-node refresh
p99 moved from `17.560 ms` to `21.702 ms` versus the preceding object-selector
full matrix, but the changed code is limited to snapshot clone request
validation, so this is classified as benchmark tail movement rather than a
functional-change regression.
