...
- `tools/run-native-closure-validation.py --batch runtime-backpressure` passed
  ...
  bounded queued-execution drain and queue-full rejection for search/write
  pools;
...
- extend queue/backpressure smoke tests from bounded search/write queued-work
  drain and saturation rejection into real concurrent scheduling behavior;
