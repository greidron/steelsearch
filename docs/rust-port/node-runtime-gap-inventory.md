...
- `tools/run-native-closure-validation.py --batch runtime-tasks` passed on
  2026-06-17 with 17/17 tests and `zero_tests=0`, covering bounded task
  ...
  readback through restart until eviction, cancelled-terminal restart-sync and
  live-shutdown refusal with progress preservation, and shared-runtime restart
  readback for task queue state plus cancelled task ids, including accepted in-flight task
