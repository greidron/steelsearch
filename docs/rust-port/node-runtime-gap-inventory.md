...
- `tools/run-native-closure-validation.py --batch startup-preflight` passed on
  ...
  authentication-users-file rejection, runtime security enforcement env-var
  refusal when `STEELSEARCH_SECURITY_ENABLED=true` is not set, plus
  config-level and daemon-level data-path checks for missing, readonly,
  locked, file-backed, and daemon-refused paths, bind, duplicate node-id,
  invalid address/port, explicit OpenSearch `-E` config-setting rejection with
  the Steelsearch flag/env-var contract, role/bootstrap, and production-mode
  refusal cases;
...
- move from "production mode blocked by structured policy/checklist and
  TLS/authn bootstrap material checks" to concrete TLS/authn enforcement and
  runtime preflight checks.

Required tests:

- none for the current data-path, bind/config, explicit OpenSearch `-E`
  rejection, duplicate node-id, role/bootstrap, and production-gate startup
  refusal fixture scope;
...
- `tools/run-native-closure-validation.py --batch runtime-tasks` passed on
  2026-06-17 with 22/22 tests and `zero_tests=0`, covering bounded task
  ...
  live-shutdown, and node-role-transition refusal with progress preservation,
  acknowledged/failed terminal readback across node-role transition, active
  queued/in-flight node-role-transition cancellation/refusal and restart-smoke
  reload, and
  shared-runtime restart readback for task queue state plus cancelled task ids,
  including accepted in-flight task readback/refusal without queued replay,
  partial shared-state recovery error
...
- `tools/run-native-closure-validation.py --batch runtime-queue` passed on
  ...
  payloads include OpenSearch-shaped document, store, get, search, refresh,
  flush, cache, fielddata, merge, warmer, translog, runtime, connection, CPU,
  memory, GC, class, filesystem, and breaker fields, and nodes-info responses
  include the nodes-level `_nodes`
...
- `tools/run-native-closure-validation.py --batch runtime-throttle` passed on
  ...
  and shared-runtime restart readback for requested throttle
...
- `tools/run-native-closure-validation.py --batch runtime-task-children` passed
  on 2026-06-17 with 10/10 tests and `zero_tests=0`, covering same-node
