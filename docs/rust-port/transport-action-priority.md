...
failures, maintenance actions, secure settings reload, remote-store restore, and
extension proxy.
`tools/report-transport-action-coverage.py` therefore reports
`release_parity_evidence_complete=true`,
`release_parity_source_matched_action_count=160`, and
`release_parity_source_missing_action_count=0`.
