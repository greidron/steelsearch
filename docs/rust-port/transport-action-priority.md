...
| --- | ---: | --- |
| `bounded_local_subset` | 170 | Steelsearch has request/response evidence and serves the declared local subset. |
...
current release ledger covers 19 source-derived actions: refresh, get, mget,
bulk, index, update, delete, script catalog/storage actions, and ingest/search
pipeline manifest actions.  `tools/report-transport-action-coverage.py`
therefore reports `release_parity_evidence_complete=false`,
`release_parity_source_matched_action_count=19`, and
`release_parity_source_missing_action_count=141`.
...
- implemented classification for the bounded exact-name
  `cluster:admin/repository/verify` subset with manifest-backed repository
  metadata admission and OpenSearch-shaped local node-view response rendering;
- explicit rejection for custom cluster-manager timeout, custom acknowledgement
  timeout, blank names, wildcard names, multi-name selectors, and missing local
  repositories. Broader distributed repository verification remains outside
...
- OpenSearch `CreateSnapshotResponse` decode/build for the accepted response
  shape where optional `SnapshotInfo` is absent;
- implemented classification for `cluster:admin/snapshot/create` when the
  request uses default snapshot options, targets an exact local manifest
  repository, and creates a non-duplicate local snapshot record;
- manifest-backed local creation of object- and array-shaped snapshot records,
  followed by an accepted transport response;
- explicit rejection for custom cluster-manager timeout, blank snapshot names,
  blank repository names, repository/snapshot pattern selectors, index
  selectors, custom indices options, custom settings, disabled global state,
  wait-for-completion, partial snapshots, user metadata, missing local
  repositories, and duplicate local snapshot records.
...
- OpenSearch `AcknowledgedResponse` acknowledgement bit at the wire
  decode/build layer;
- implemented classification for the bounded local remove-metadata subset:
  default acknowledged-request timeouts, non-empty model id, remove flag set,
  no model metadata body, local runtime metadata removal when present, and
  acknowledgement rendering;
- explicit rejection for custom cluster-manager timeout, custom acknowledgement
  timeout, blank model ids, add requests without metadata, and model metadata
...
- implemented classification for the bounded local clear-cache subset:
  concrete manifest indices, k-NN-enabled index metadata, local primary shard
  counter calculation, shared-runtime cache-state mutation, and OpenSearch
  broadcast response rendering with successful shard counters;
- explicit rejection for missing indices, blank indices, custom index
  resolution options, non-k-NN indices, unresolved indices, non-empty shard
  failures, and response failure rendering.
...
- decoded OpenSearch `SearchContextId` PIT-segments routing now preserves raw
  `ShardSearchContextId` long ids, including negative values, matching
  OpenSearch `ShardSearchContextId` wire semantics and
  `TransportPitSegmentsAction.shards()` local-cluster filtering;
...
- OpenSearch `SearchFreeContextResponse` single-boolean `freed` rendering;
...
The search free-context boundary covers:

- OpenSearch `SearchTransportService.SearchFreeContextRequest` parent task,
  `ShardSearchContextId`, nullable `OriginalIndices.indices`, and
  `IndicesOptions` wire decode/build;
- OpenSearch `SearchFreeContextResponse` single-boolean `freed` rendering;
- local reader-context invalidation through the same `freeReaderContext(...)`
  lookup semantics used by OpenSearch, checking local scroll contexts first and
...
- the local can-match transport route executes the bounded subset by returning
  ...
  `match_none`, or when OpenSearch's bounded bool rewrite rules reduce a bool
  ...
  an empty local response; reader-context can-match requests whose keep-alive
  exceeds the local PIT keep-alive cap now render the same
  `IllegalArgumentException` keep-alive limit response used by PIT lifecycle
  transport routes before local can-match execution;
...
can_match_request_decode iterations=400000 elapsed_ms=425.297 ops_per_second=940518.59 nanos_per_op=1063.24
can_match_response_encode iterations=400000 elapsed_ms=49.763 ops_per_second=8038089.45 nanos_per_op=124.41
can_match_response_decode iterations=400000 elapsed_ms=51.106 ops_per_second=7826851.11 nanos_per_op=127.77
can_match_wire_bottleneck_ops_per_second=918057.70
```
...
- local create-reader-context admission now treats manifest-backed
  `ShardId` index UUID as part of shard identity, rejecting mismatched UUIDs
  before allocating a reader context while still accepting `_na_` and
  dev-created single-shard indices that do not carry manifest UUID metadata;
...
- local transport update-reader-context now requires a reader context id
  ...
  PIT state, and renders an OpenSearch `SearchContextMissingException`
  transport error for unknown or expired reader contexts, matching
  `SearchService.updatePitIdAndKeepAlive(...)` missing-context rejection for
  the local-node subset.
- local transport update-reader-context now accepts and preserves empty PIT id
  strings at the local execution boundary, matching OpenSearch
  `UpdatePitContextRequest`/`UpdatePitContextResponse` string-only wire
  semantics where the request class performs no empty-id validation before
  mutating reader/PIT lifecycle state.
- local transport update-reader-context now tracks per-reader PIT id and
  ...
  reader context now returns an OpenSearch-style `IllegalStateException`
  transport error with the `SetOnce` message and releases the local reader
  context instead of mutating PIT lifecycle state again.
- local transport update-reader-context now preserves raw `creationTime` on the
  active PIT reader context, including negative values, so get-all-PIT list
  rendering follows OpenSearch `PitReaderContext.getCreationTime()` rather than
  normalizing to wall-clock time.
...
- local transport update-reader-context now follows OpenSearch
  `SearchService.getReaderContext(...)` lookup semantics for shard-search
  session ids: an empty session string can update the active local PIT reader
  by numeric context id, while non-empty sessions require the exact local
  `(session,id)` reader key and otherwise render `SearchContextMissingException`.
...
- local transport free-PIT-context now follows OpenSearch
  `SearchService.getReaderContext(...)` lookup semantics for empty shard-search
  session ids: an empty session string can release the active local PIT reader
  by numeric context id, while non-empty sessions still require the exact local
  `(session,id)` reader key.
- local transport free-PIT-context now preserves a PIT context when the
  requested reader context is already missing but another local reader still
...
Current verify-repository wire microbenchmark:

```text
cargo run -q -p os-transport --release --bin verify-repository-wire-benchmark
verify_repository_request_encode iterations=400000 elapsed_ms=238.230 ops_per_second=1679052.34 nanos_per_op=595.57
verify_repository_request_decode iterations=400000 elapsed_ms=227.737 ops_per_second=1756408.28 nanos_per_op=569.34
verify_repository_request_validate iterations=400000 elapsed_ms=237.848 ops_per_second=1681748.76 nanos_per_op=594.62
verify_repository_response_encode iterations=400000 elapsed_ms=178.014 ops_per_second=2247019.59 nanos_per_op=445.03
verify_repository_response_decode iterations=400000 elapsed_ms=171.469 ops_per_second=2332778.87 nanos_per_op=428.67
verify_repository_wire_bottleneck_ops_per_second=1679052.34
```

The current verify-repository transport boundary bottleneck is request encode.
The payload matches the acknowledged cluster-manager request envelope plus
repository name, validates the bounded exact-name execution subset, and renders
the one-node response shape. At roughly 1.68M ops/s in the latest local release
run, this boundary is not a material transport bottleneck; the first
...
Current create-snapshot wire microbenchmark:

```text
cargo run -p os-transport --release --bin create-snapshot-wire-benchmark
create_snapshot_request_encode iterations=400000 elapsed_ms=313.587 ops_per_second=1275563.24 nanos_per_op=783.97
create_snapshot_request_decode iterations=400000 elapsed_ms=292.008 ops_per_second=1369823.45 nanos_per_op=730.02
create_snapshot_request_validate iterations=400000 elapsed_ms=312.858 ops_per_second=1278537.30 nanos_per_op=782.14
create_snapshot_response_encode iterations=400000 elapsed_ms=48.392 ops_per_second=8265831.97 nanos_per_op=120.98
create_snapshot_response_decode iterations=400000 elapsed_ms=54.549 ops_per_second=7332834.92 nanos_per_op=136.37
create_snapshot_wire_bottleneck_ops_per_second=1275563.24
```

The current create-snapshot transport subset bottleneck is request encode. The
payload includes the cluster-manager request envelope, snapshot name,
repository name, empty index selector array, indices options, settings,
global-state flags, partial/wait flags, generic user metadata marker, and
accepted response shape. At roughly 1.28M ops/s in the latest local release run,
...
decode once opaque scroll id coverage is included. Opaque scroll id decode
itself runs at roughly 1.57M ops/s, while the full explicit request decode path
lands near 1.00M ops/s. The small response encode/decode path runs above 7.0M
ops/s in the latest local release run, so response rendering is not the first
bottleneck. The next performance point to inspect is context invalidation under
larger multi-context scroll ids.
...
pit_context_clone_elapsed_ms=23.791
pit_context_clone_ops_per_second=21016362.20
pit_context_clone_snapshot_strong_count=1
pit_open_snapshot_iterations=250
pit_open_snapshot_elapsed_ms=362.367
pit_open_snapshot_ops_per_second=689.91
pit_open_snapshot_documents_per_second=6899076.33
