...
| Transport source inventory | `174` implemented transport evidence rows: `163 bounded_local_subset`, `4 bounded_seed_peer_fanout_subset`, `0 fail_closed_or_empty_subset`, `7 bounded_execution_boundary` | Implemented means the declared subset has evidence; it does not imply broad server-side transport parity without the scoped evidence row and live scenario coverage. |
