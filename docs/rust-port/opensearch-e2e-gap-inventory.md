...
| Transport source inventory | `174` implemented transport evidence rows: `113 bounded_local_subset`, `53 fail_closed_or_empty_subset`, `8 bounded_execution_boundary` | Implemented means the declared subset has evidence; it does not imply broad server-side transport parity without the scoped evidence row and live scenario coverage. |
