...
- `runtime-stateful-probe`: 519 passed, 0 failed, 0 skipped; 452 cases now
...
- Combined required classification:
  `canonical_equal=1925`, `strict_equal=937`, `semantic_equal=0`,
  `steelsearch_fail_closed=1`, `steelsearch_only=204`,
...
| --- | --- | --- |
| Live required OpenSearch E2E suites | `failed=0`, `missing=0`, raw `known_gap_or_skipped=22`, effective `known_gap_or_skipped=0` across `1925` canonical, `937` strict, `204` Steelsearch-only, and `1` Steelsearch fail-closed cases | Covered cases pass, and raw skipped cases are resolved by other required suites. |
