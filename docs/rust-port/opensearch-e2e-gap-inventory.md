...
| Transport source inventory | `174` implemented transport evidence rows: `115 bounded_local_subset`, `51 fail_closed_or_empty_subset`, `8 bounded_execution_boundary` | Implemented means the declared subset has evidence; it does not imply broad server-side transport parity without the scoped evidence row and live scenario coverage. |
