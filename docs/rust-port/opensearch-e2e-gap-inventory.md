...
- Remaining `steelsearch_only=4` breakdown:
  `ml-model-surface=4`. `multi-node-transport-admin` now has live OpenSearch
...
| --- | --- | --- |
| Live required OpenSearch E2E suites | Latest broad-current effective summary: `failed=0`, `missing=0`, `known_gap_or_skipped=0` across `1132` canonical, `842` strict, `3` semantic, and `4` Steelsearch-only cases. The broad report itself is `missing` because 31 of 33 required suites were reported in this age-gated refresh. | Covered current cases pass; remaining Steelsearch-only evidence is isolated to ML Commons/model-serving fixture cases. |
