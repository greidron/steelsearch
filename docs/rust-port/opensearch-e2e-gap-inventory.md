...
- Latest broad-current effective classification after the ML serving comparison
  refresh:
  `canonical_equal=2137`, `strict_equal=937`, `semantic_equal=3`,
...
- Effective required classification after skip resolution:
  `known_gap_or_skipped=0`; all 21 raw skipped cases are covered by other
  required suites.
...
| REST live-required source-route mapping | `379/379` in-scope source routes matched by live-required fixture routes, with `3558` live-required fixture routes and `0` required-suite Steelsearch-only cases | Live-required route mapping is closed for the current source inventory. |
