...
- Latest broad-current effective classification after the ML serving comparison
  refresh:
  `canonical_equal=2248`, `strict_equal=1010`, `semantic_equal=3`,
...
| REST live-required source-route mapping | `379/379` in-scope source routes matched by live-required fixture routes, with `3668` live-required fixture routes and `0` required-suite Steelsearch-only cases | Live-required route mapping is closed for the current source inventory. |
