...
| REST live-required source-route mapping | `379/379` in-scope source routes matched by live-required fixture routes, with `3588` live-required fixture routes and `0` required-suite Steelsearch-only cases | Live-required route mapping is closed for the current source inventory. |
