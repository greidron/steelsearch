...
| Transport source inventory | `160` transport actions: `127 implemented`, `33 partial`, `0 planned` | Implemented adapters have positive wire/route evidence; partial rows have explicit fail-closed boundaries and still need per-action execution semantics plus live comparison evidence. |
