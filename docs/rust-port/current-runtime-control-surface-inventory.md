...
| Search session control | `POST|GET|DELETE /_search/scroll`, `POST /{index}/_search/point_in_time`, `GET|DELETE /_search/point_in_time*`, `GET /_cat/pit_segments*` | `docs/api-spec/search.md`, generated route evidence, PIT snapshot/search-after tests | PIT has stateful local snapshot semantics; deeper runtime service guarantees for resource accounting and background cleanup remain open |
...
| Search session control | scroll/PIT lifecycle manager | PIT leasing, expiry, list/delete, snapshot search, and search-after behavior exist locally; remaining gap is production-grade resource accounting and background cleanup ownership |
