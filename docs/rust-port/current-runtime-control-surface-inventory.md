...
| Propagation model | `parent_task_id` child cancellation visibility includes same-node and cross-node multi-level descendant propagation, but current route evidence does not prove background-worker propagation |
...
- add fixture-backed distinction for:
  ...
  - background-worker parent task cancel versus child task visibility;
