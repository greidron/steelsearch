...
| Search session control | `POST|GET|DELETE /_search/scroll`, `POST /{index}/_search/point_in_time`, `GET|DELETE /_search/point_in_time*`, `GET /_cat/pit_segments*` | `docs/api-spec/search.md`, generated route evidence, PIT snapshot/search-after tests, `_nodes/stats` open-context accounting, PIT expiry reaper test | PIT has stateful local snapshot semantics, local open-context stats, and a daemon PIT expiry reaper; deeper distributed runtime service guarantees remain open |
...
| Search session control | scroll/PIT lifecycle manager | PIT leasing, expiry, list/delete, snapshot search, search-after behavior, `_nodes/stats` open-context accounting, and a local PIT expiry reaper exist; remaining gap is production-grade distributed ownership |
