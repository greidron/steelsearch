...
| Search session control | scroll/PIT lifecycle manager | PIT leasing, expiry, list/delete, snapshot search, search-after behavior, and `_nodes/stats` open-context accounting exist locally; remaining gap is production-grade background cleanup ownership |
