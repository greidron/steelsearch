...
- `tools/report-release-evidence-inventory.py --require-complete` for verifying
  the broader release record. It keeps the five-item production startup
  manifest and six readiness-attachment inputs distinct, then also requires the
  promotion gate suite artifact generated by
  `tools/check-all-promotion-gates.py --output`.
...
`ok` required suites in a unified E2E report. The current 0.2.5 broad-report
results are:
