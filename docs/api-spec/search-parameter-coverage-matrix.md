...
| `_source` filtering | consumed + validated | bounded body and REST query-param source projection in `standalone_runtime.rs` | Top-level include/exclude field selection is supported; broader wildcard/path semantics remain partial. |
| `fields` | consumed + validated | bounded fetch-field validation and hit `fields` projection in `standalone_runtime.rs` | Simple top-level field extraction is supported; broader field formatting semantics remain partial. |
...
| --- | --- | --- | --- | --- |
| `_source` filtering | bounded source projection | documented partial | Source retrieval now honors simple body/query include/exclude selection and `_source: false` while rejecting unsupported shapes. | Expand strict compare fixtures for wildcard and nested path selectors. |
| `fields` | bounded top-level field extraction | documented partial | Current support projects simple top-level values into hit `fields` and rejects unsupported shapes instead of silently ignoring them. | Expand strict compare fixtures for formatted, nested, and unmapped fields. |
