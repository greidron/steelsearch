...
| Query DSL | `intervals` | Partial | Live standalone subset now supports bounded `match`, `prefix`, `wildcard`, `all_of`, and match-only `any_of` interval forms with OpenSearch-shaped `ordered`/`mode`/`max_gaps` defaults and match/prefix/wildcard `use_field`; broader filter/analyzer and expansion-limit semantics remain narrower than OpenSearch. |
