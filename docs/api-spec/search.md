...
| Query DSL | `intervals` | Partial | Live standalone subset now supports bounded `match`, `prefix`, `wildcard`, `regexp`, `fuzzy`, `all_of`, `any_of`, and relation `filter` interval forms with OpenSearch-shaped `ordered`/`mode`/`max_gaps` defaults and match/prefix/wildcard/regexp/fuzzy `use_field`; analyzer, regexp flags, nested composite, script filter, and expansion-limit semantics remain narrower than OpenSearch. |
