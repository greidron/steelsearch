...
| Query DSL | `intervals` | Partial | Live standalone subset now supports bounded `match`, ordered `all_of`, and match-only `any_of` interval forms; broader filter/max_gaps semantics remain narrower than OpenSearch. |
