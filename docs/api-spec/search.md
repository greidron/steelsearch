...
| Query DSL | `regexp`, `fuzzy` | Partial | Live standalone subset now supports bounded field/value forms with simplified regex, `regexp.case_insensitive`, fuzzy `prefix_length`, and edit-distance semantics; broader rewrite, scoring, and analyzer parity remain narrower than OpenSearch. |
