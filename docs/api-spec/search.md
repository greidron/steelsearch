...
| Query DSL | `intervals` | Partial | Live standalone subset now supports bounded `match`, `prefix`, `all_of`, and match-only `any_of` interval forms with OpenSearch-shaped `ordered`/`mode`/`max_gaps` defaults and match/prefix `use_field`; broader filter/analyzer semantics remain narrower than OpenSearch. |
