...
| Query DSL | `intervals` | Partial | Live standalone subset now supports bounded `match`, `all_of`, and match-only `any_of` interval forms with OpenSearch-shaped `ordered`/`mode`/`max_gaps` defaults; broader filter/use-field/analyzer semantics remain narrower than OpenSearch. |
