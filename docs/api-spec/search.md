...
| Query DSL | `intervals` | Partial | Live standalone subset now supports bounded `match`, `prefix`, `wildcard`, `regexp`, `all_of`, and match-only `any_of` interval forms with OpenSearch-shaped `ordered`/`mode`/`max_gaps` defaults and match/prefix/wildcard/regexp `use_field`; broader filter/analyzer, regexp flags, and expansion-limit semantics remain narrower than OpenSearch. |
