...
| Query DSL | `intervals` | Partial | Live standalone subset now supports bounded `match`, `all_of`, and match-only `any_of` interval forms with OpenSearch-shaped `ordered`/`mode`/`max_gaps` defaults and match-level `use_field`; broader filter/analyzer semantics remain narrower than OpenSearch. |
