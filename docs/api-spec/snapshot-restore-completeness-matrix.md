...
| Snapshot create/delete | `PUT /_snapshot/{repo}/{snap}`, `DELETE /_snapshot/{repo}/{snap}` | partial | duplicate create now fail-closes as `400 invalid_snapshot_name_exception`; repeated delete fail-closes | [snapshot-migration-semantic-gap-matrix.md](/home/ubuntu/steelsearch/docs/api-spec/snapshot-migration-semantic-gap-matrix.md) | cutover-safe only within the bounded create/delete semantics covered by fixtures |
...
| Snapshot create/delete | exact duplicate-create and repeated-delete root-cause/body parity may diverge outside the bounded status/type compare |
