...
| repository verify / cleanup | `POST /_snapshot/{repo}/_verify`, `POST /_snapshot/{repo}/_cleanup` | partial | partial | partial | partial | [snapshot-lifecycle-compat.json](/home/ubuntu/steelsearch/tools/fixtures/snapshot-lifecycle-compat.json) now pins happy-path, repeated cleanup idempotency, missing-repository failures for both routes, local `fs` repository verification probes for inaccessible paths, and local runtime cleanup of nested temporary blobs plus SteelSearch snapshot directories that are no longer referenced by the snapshot manifest | verify/cleanup handlers in [standalone_runtime.rs](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs) | Repository cleanup now has bounded side effects for local orphan SteelSearch snapshot state and `fs` verify no longer silently succeeds without touching the repository path, but full OpenSearch repository compaction, repository-byte parity, remote repository verification, and long-running verify semantics are not claimed. |
| restore | `POST /_snapshot/{repo}/{snap}/_restore` | partial | partial | partial | partial | [snapshot-lifecycle-compat.json](/home/ubuntu/steelsearch/tools/fixtures/snapshot-lifecycle-compat.json) now pins happy-path, renamed-index document count readback, wildcard/exclusion restore selectors, `ignore_unavailable` versus `partial` missing-selector behavior, alias rename plus `alias_write_index_policy=strip_write_index` readback, stale/corrupt/incompatible metadata fail-closed, and missing-snapshot failure; local daemon coverage now also pins `_status` materialization for detected missing/checksum-mismatched shard manifests; [alias-template-persistence-compat.json](/home/ubuntu/steelsearch/tools/fixtures/alias-template-persistence-compat.json) now carries restored template/index/data-stream metadata summaries | restore/status handlers in [standalone_runtime.rs](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs) plus [alias_template_persistence_compat.py](/home/ubuntu/steelsearch/tools/alias_template_persistence_compat.py) | Broader restore-time parity still needs live OpenSearch comparison for partial shard restore, data stream attachment, remote-backed repositories, and option-combination materialization rows beyond the current bounded fixture. |
...
| repeated cleanup | `POST /_snapshot/{repo}/_cleanup` | bounded idempotent `200` response for an existing repository; cleanup now counts and removes nested `.tmp` blobs plus unreferenced SteelSearch snapshot directories, and a repeat cleanup returns zero deletions; missing repository still fail-closes as `404` | `cleanup_snapshot_repository_repeated_idempotent` in [snapshot-lifecycle-compat.json](/home/ubuntu/steelsearch/tools/fixtures/snapshot-lifecycle-compat.json); `snapshot_cleanup_removes_orphan_snapshot_dirs_and_nested_temp_blobs` | [handle_snapshot_cleanup_route](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs) | cleanup side effects are bounded to SteelSearch local repository layout and do not claim OpenSearch repository compaction parity |
