...
| refresh visibility | partial | existing stateful probes, `update_by_query_refresh_true_readback` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Update-by-query `refresh=true` readback is OpenSearch-compared; broader refresh=false/visibility generation behavior remains bounded. |
