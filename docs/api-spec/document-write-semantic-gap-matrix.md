...
| destination routing / script transforms | partial | `reindex_route_honors_destination_routing_and_require_alias` in [standalone_runtime.rs](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs), `reindex_script_transform_unsupported_error` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Destination routing supports unset/`keep`, `discard`, and `=<value>` with validation; script transforms are explicitly fail-closed until parity is implemented. |
...
| unsupported script semantics | explicit fail-closed | `reindex_script_transform_unsupported_error` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Script transform parity is still unclaimed; current behavior rejects the unsupported shape instead of silently copying unmodified documents. |
