...
| retry / task-mode semantics | partial | rethrottle probes and route coverage, `reindex_slices_summary` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Bounded `slices=2` synchronous summary behavior is OpenSearch-compared; task-mode, rethrottle, and retry lifecycle semantics are not yet summarized as replacement-ready. |
