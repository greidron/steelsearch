...
| refresh visibility | partial | `bulk_refresh_false_readback_not_found`, `bulk_refresh_true_readback`, and `bulk_refresh_wait_for_readback` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Bulk-specific read-after-write visibility is separated for default/false, `refresh=true`, and `refresh=wait_for`; broader refresh generation and retry interactions remain bounded. |
| retry / repeated call | partial | `bulk_repeated_create_replay_conflict` and `bulk_repeated_index_replay_overwrite_readback` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Repeated `create` conflict replay and repeated `index` overwrite/readback semantics are OpenSearch-compared; broader `update` retry taxonomy remains bounded. |
...
| --- | --- | --- | --- | --- |
| `index` | partial | partial | bulk runtime tests in [standalone_runtime.rs](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs), `bulk_repeated_index_replay_overwrite_readback` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Basic create/update item envelope, external versioning, and repeated `index` overwrite/readback semantics are OpenSearch-compared; broader retry taxonomy remains bounded. |
| `create` | partial | partial | duplicate-create conflict coverage in [standalone_runtime.rs](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs), `bulk_repeated_create_replay_conflict` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Duplicate id conflict and repeated `create` replay conflict are pinned; broader route-family retry/idempotency taxonomy remains bounded. |
...
| `refresh` | partial | single-doc put/post/create/update/delete, bulk route-level refresh | route-level request handling in single-doc helpers and bulk handler code in [standalone_runtime.rs](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs), plus single-doc and bulk readback fixtures | Single-doc and bulk default/false, `refresh=true`, and `refresh=wait_for` read-after-write behavior is OpenSearch-compared for the bounded fixture set; broader refresh generation, retry, and async lifecycle interactions remain incomplete. |
| `pipeline` | partial | single-doc put/post/create/update, bulk route-level and item metadata for index/create/update | bounded write-path execution in [standalone_runtime.rs](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs); `single_doc_put_ingest_set_pipeline_readback` and `bulk_ingest_set_pipeline_readback` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Empty ingest pipelines and `set` processors are applied before persistence and compared against OpenSearch for single-doc and bulk readback. Unsupported processors and missing pipelines fail closed; broad ingest processor parity remains bounded. |
