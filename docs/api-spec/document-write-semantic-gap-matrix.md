...
| `require_alias` | partial | single-doc index/create/update and bulk index/create/update | fail-closed concrete-target coverage in `single_doc_routes_surface_conflict_and_routing_negative_cases` and `bulk_route_supports_metadata_fields_and_rejects_unsupported_pipeline_metadata` within [standalone_runtime.rs](/home/ubuntu/steelsearch/crates/os-node/src/standalone_runtime.rs) | `require_alias=true` now rejects concrete write targets and allows alias write targets for bounded single-doc and bulk write paths; broader by-query/reindex coverage remains unclaimed. |
