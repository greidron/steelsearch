...
| Semantic axis | Current status | Evidence | Notes / missing work |
| --- | --- | --- | --- |
| matched / unmatched | partial | delete/update-by-query semantic tests and probes, `update_by_query_matched_summary` and `update_by_query_unmatched_summary` in [document-write-semantic-compat.json](/home/ubuntu/steelsearch/tools/fixtures/document-write-semantic-compat.json) | Matched/unmatched delete semantics and bounded update-by-query summary counters are pinned; broader query/script variants remain bounded. |
