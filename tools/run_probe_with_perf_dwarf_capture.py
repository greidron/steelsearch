...
def main() -> int:
    ...
    perf_data_path = work_dir / 'opensearch' / 'sudo-perf-record-dwarf.data'
    perf_script_path = work_dir / 'opensearch' / 'sudo-perf-record-dwarf.script.txt'
    ...
    summary = {
        ...
        'checker_result': (
            'perf_dwarf_capture_collected_on_failing_probe'
            if record_returncode == 0 and script_returncode == 0 and report is not None
            else 'perf_dwarf_capture_incomplete'
        ),
    }
