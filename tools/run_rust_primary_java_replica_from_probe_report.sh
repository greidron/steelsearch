...
PROFILE="rust-primary-java-replica"
REPORT_DIR="${ROOT_DIR}/target/java-mixed-cluster-binary"
PROBE_REPORT=""
INDEX_NAME="mixed-rust-primary-000001"
PRINT_ONLY=0
RECOVER_CMD=""
RESTART_CMD=""
...
usage() {
  cat <<'EOF'
Usage:
  tools/run_rust_primary_java_replica_from_probe_report.sh --probe-report <path> [options]
...
print("true" if membership_formed is True else "false")
print(cluster_url)
print(java_node)
print(rust_node)
PY
...
MEMBERSHIP_FORMED="${HANDOFF[0]:-false}"
CLUSTER_URL="${HANDOFF[1]:-}"
JAVA_NODE="${HANDOFF[2]:-}"
RUST_NODE="${HANDOFF[3]:-}"

if [[ "$MEMBERSHIP_FORMED" != "true" ]]; then
  echo "probe report is not membership_formed=true: $PROBE_REPORT" >&2
  exit 1
fi
