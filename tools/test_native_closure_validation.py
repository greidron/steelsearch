...
class NativeClosureValidationRunnerTests(unittest.TestCase):
    ...
    def test_e2e_required_parity_batch_checks_no_skips_for_required_suites(self):
        ...
        self.assertNotIn("tools/run-unified-opensearch-e2e.py", command_text)
        self.assertIn("target/unified-opensearch-e2e-audit/unified-opensearch-e2e-report.json", command_text)
    ...
    def test_e2e_search_compat_parity_batch_allows_known_skips_but_rejects_failures(self):
        ...
        command_text = " ".join(batch[0].command)
        self.assertNotIn("tools/run-unified-opensearch-e2e.py", command_text)
        self.assertIn("target/unified-opensearch-e2e-current/unified-opensearch-e2e-report.json", command_text)
        self.assertIn("--max-report-age-seconds", command_text)
        self.assertIn("604800", command_text)
        self.assertIn("tools/check-unified-opensearch-e2e-report.py", command_text)
        self.assertNotIn("--require-no-skips", command_text)
        pit_command_text = " ".join(batch[1].command)
        self.assertNotIn("tools/run-unified-opensearch-e2e.py", pit_command_text)
        self.assertIn("target/unified-opensearch-e2e-pit-current/unified-opensearch-e2e-report.json", pit_command_text)
    ...
    def test_rest_api_coverage_current_batch_reports_source_inventory_coverage(self):
        ...
        self.assertEqual(command_text.count("--max-report-age-seconds"), 1)
        ...
        self.assertIn("tempfile.NamedTemporaryFile", command_text)
        self.assertIn("shutil.copyfile", command_text)
        self.assertIn("if second.returncode == 0", command_text)
    ...
    def test_release_readiness_tooling_batch_runs_writer_and_checker_contract(self):
        ...
        self.assertIn("command_specs", command_text)
        self.assertIn("command_spec_digest", command_text)
    ...
    def test_release_evidence_inventory_current_batch_writes_artifact(self):
        ...
        self.assertIn("tempfile.NamedTemporaryFile", joined_command)
        self.assertIn("shutil.copyfile", joined_command)
        self.assertIn("if passed else None", joined_command)
        ...
        self.assertIn("604800", command_text)
        self.assertIn("--require-complete", command_text)
        self.assertIn("--output", command_text)
        self.assertIn("target/release-evidence-inventory-current.json", command_text)
        self.assertIn("tempfile.NamedTemporaryFile", command_text)
        self.assertIn("shutil.copyfile", command_text)
        self.assertIn("if result.returncode == 0", command_text)
