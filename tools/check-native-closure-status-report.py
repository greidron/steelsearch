...
REST_LIVE_REQUIRED_MATCHED_SOURCE_ROUTE_COUNT = 379
REST_FIXTURE_ROUTE_COUNT = 3777
REST_LIVE_REQUIRED_FIXTURE_ROUTE_COUNT = 3560
...
def rest_api_coverage_explanation_errors(current: dict[str, Any]) -> list[str]:
    ...
    owner_count_baselines = {
        "source_route_owner_counts": REST_SOURCE_ROUTE_OWNER_COUNTS,
...
def release_evidence_inventory_errors(current: dict[str, Any]) -> list[str]:
    ...
    if tuple(summary.get("readiness_item_names") or ()) != STARTUP_MANIFEST_ITEMS:
        errors.append(
    ...
    if tuple(summary.get("readiness_summary_item_names") or ()) != STARTUP_MANIFEST_ITEMS:
        errors.append(
            "gates.current_evidence.results release evidence inventory readiness summary item names mismatch"
        )
    if tuple(summary.get("readiness_summary_ready_item_names") or ()) != STARTUP_MANIFEST_ITEMS:
        errors.append(
    ...
    for batch, expected_test_count in RUNTIME_CONTROL_BATCH_COUNTS.items():
        errors.extend(
            runtime_control_batch_errors(
                ...
                batch,
                expected_test_count,
                RUNTIME_CONTROL_BATCH_NAME_DIGESTS[batch],
            )
        )
    return errors
...
def runtime_control_batch_errors(
    ...
    if batch_summary.get("test_name_count") != expected_test_count:
        errors.append(
            f"gates.current_evidence.results runtime controls {batch} test_name_count "
            f"is not {expected_test_count}"
        )
    if batch_summary.get("test_name_digest") != expected_test_name_digest:
        errors.append(
            f"gates.current_evidence.results runtime controls {batch} test_name_digest "
            "does not match current baseline"
        )
    if batch_summary.get("failed_count") != 0:
        errors.append(
