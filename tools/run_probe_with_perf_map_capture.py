...
def main() -> int:
    ...
    perf_data_path = work_dir / 'opensearch' / 'sudo-perf-record-perfmap.data'
    perf_script_path = work_dir / 'opensearch' / 'sudo-perf-record-perfmap.script.txt'
    perf_map_path = work_dir / 'opensearch' / 'perf.map'
    ...
    try:
        if wait_for_file(pid_path, 90):
            ...
            perf_map_copied = copy_if_exists(Path(f'/tmp/perf-{pid}.map'), perf_map_path)
            perf = subprocess.run([
                'sudo', 'perf', 'record', '-g',
            ...
            record_returncode = perf.returncode
            if not perf_map_copied:
                perf_map_copied = copy_if_exists(Path(f'/tmp/perf-{pid}.map'), perf_map_path)
        ...
        if 'pid' in locals() and not perf_map_copied:
            perf_map_copied = copy_if_exists(Path(f'/tmp/perf-{pid}.map'), perf_map_path)
    ...
    summary = {
        ...
        'checker_result': (
            'perf_map_capture_collected_on_failing_probe'
            if record_returncode == 0 and script_returncode == 0 and perf_map_copied and report is not None
            else 'perf_map_capture_incomplete'
        ),
    }
