#!/usr/bin/env python3
"""Run template HTTP compatibility checks against Steelsearch and OpenSearch."""
...
DEFAULT_FIXTURE = ROOT / "tools" / "fixtures" / "template-compat.json"
DEFAULT_OUTPUT = ROOT / "target" / "template-compat-report.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default=os.environ.get("TEMPLATE_COMPAT_REPORT", str(DEFAULT_OUTPUT)),
    )
...
def normalize_template_body(case: dict[str, Any], body: Any) -> Any:
    extract = case.get("extract")
    template_name = case.get("template_name")
    if not extract or not isinstance(body, dict):
        return body

    if extract == "component_template_single":
        if isinstance(body.get("component_templates"), list) and body["component_templates"]:
            first = body["component_templates"][0]
            return {
                "name": first.get("name"),
                **(first.get("component_template") or {}),
            }
        if template_name and template_name in body:
            named = body.get(template_name) or {}
            return {
                "name": template_name,
...
def normalize_response(case: dict[str, Any], response: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["body"] = normalize_template_body(case, response.get("body"))
    return normalized
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "name": fixture.get("name", "template-compat"),
