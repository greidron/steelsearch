...
def main() -> int:
    ...
    ap.add_argument('--event', default='io.netty.channel.socket.nio.NioSocketChannel.doReadBytes')
    ap.add_argument('--cstack', default='')
    ...
    collapsed_path = work_dir / 'opensearch' / 'late-async-profiler-method.collapsed'
    ...
    try:
        if wait_for_file(pid_path, 90):
            ...
            prof = subprocess.run([
                'sudo', str(ASPROF), '-d', str(args.record_seconds), '-e', args.event, '-t',
                *(['--cstack', args.cstack] if args.cstack else []),
            ...
            (work_dir / 'opensearch' / 'late-async-profiler-method.stderr').write_text((prof.stdout or '') + (prof.stderr or ''), encoding='utf-8')
    ...
    summary = {
        ...
        'checker_result': (
            'late_async_profiler_method_collected_on_failing_probe'
            if asprof_rc == 0 and report is not None and collapsed_path.exists()
            else 'late_async_profiler_method_collection_incomplete'
        ),
    }
