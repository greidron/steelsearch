...
def main() -> int:
    ...
    jhsdb_path = work_dir / 'opensearch' / 'jhsdb-jstack.txt'
    ...
    try:
        if wait_for_file(pid_path, 90):
            ...
            jhsdb = subprocess.run(['sudo', 'jhsdb', 'jstack', '--pid', pid], capture_output=True, text=True)
            jhsdb_returncode = jhsdb.returncode
            jhsdb_path.write_text(jhsdb.stdout + jhsdb.stderr, encoding='utf-8')
    ...
    summary = {
        ...
        'jhsdb_jstack_path': str(jhsdb_path),
        'strace_returncode': strace_returncode,
        'jhsdb_returncode': jhsdb_returncode,
        ...
        'checker_result': (
            'late_strace_and_jhsdb_collected_on_failing_probe'
            if strace_returncode == 0 and jhsdb_returncode == 0 and report is not None and strace_path.exists() and jhsdb_path.exists()
            else 'late_strace_and_jhsdb_collection_incomplete'
        ),
    }
