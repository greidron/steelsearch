...
E2E_SEARCH_COMPAT_PARITY_BATCH: tuple[ExternalValidation, ...] = (
    ...
    ExternalValidation(
        ...
        (
            "python3",
            "-c",
            "import subprocess, sys; report = 'target/unified-opensearch-e2e-pit-current/unified-opensearch-e2e-report.json'; pit_check = [sys.executable, 'tools/check-pit-e2e-coverage.py', report, '--require-all-pit-passed', '--max-report-age-seconds', '604800']; sys.exit(subprocess.run(pit_check).returncode)",
        ),
        timeout_seconds=360,
    ),
)
...
STARTUP_BOOTSTRAP_CURRENT_BATCH: tuple[ExternalValidation, ...] = (
    ExternalValidation(
        ...
        (
            ...
            "-c",
            "import hashlib, json, subprocess, sys\n"
            "batches = ['startup-preflight', 'startup-readiness']\n"
            "summaries = {}\n"
            "passed = True\n"
            ...
            "                test_names.append(f'{group}:{name}')\n"
            "    test_name_digest = hashlib.sha256(('\\n'.join(test_names) + '\\n').encode()).hexdigest()\n"
            "    summaries[batch] = {'test_count': summary.get('test_count'), 'failed_count': summary.get('failed_count'), 'zero_test_count': summary.get('zero_test_count'), 'group_counts': group_counts, 'group_count': len(group_counts), 'test_name_count': len(test_names), 'test_name_digest': test_name_digest}\n"
...
RELEASE_EVIDENCE_INVENTORY_GATE_BATCH: tuple[ExternalValidation, ...] = (
    ExternalValidation(
        ...
        (
            ...
            "result_names = sorted(name for name in by_name if isinstance(name, str))\n"
            ...
            "    'inventory_startup_ready_items': inventory.get('startup_ready_items'),\n"
            "    'inventory_readiness_attachment_ready_items': inventory.get('readiness_attachment_ready_items'),\n"
            "    'inventory_release_record_ready_items': inventory.get('release_record_ready_items'),\n"
            ...
            "    'readiness_item_names': readiness.get('item_names'),\n"
            "    'readiness_ready_items': readiness.get('ready_items'),\n"
            "    'readiness_ready_item_names': readiness.get('ready_item_names'),\n"
            "    'readiness_summary_item_names': readiness.get('summary_item_names'),\n"
            "    'readiness_summary_ready_item_names': readiness.get('summary_ready_item_names'),\n"
...
RELEASE_READINESS_CURRENT_COMMAND = (
    ...
    "required_item_names = ['benchmark_coverage', 'load_test_coverage', 'chaos_test_coverage', 'packaging_verified', 'rolling_upgrade_coverage']\n"
    "ready_item_names = [name for name in required_item_names if isinstance(items.get(name), dict) and items[name].get('passed') is True]\n"
    ...
    "    'item_names': payload.get('item_names') if isinstance(payload, dict) else None,\n"
    "    'ready_items': summary.get('ready_items'),\n"
    "    'ready_item_names': ready_item_names,\n"
    "    'summary_item_names': summary.get('item_names'),\n"
    "    'summary_ready_item_names': summary.get('ready_item_names'),\n"
