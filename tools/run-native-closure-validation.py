...
E2E_REQUIRED_PARITY_BATCH: tuple[ExternalValidation, ...] = (
    ExternalValidation(
        ...
        (
            "python3",
            "-c",
            "import subprocess, sys; output_dir = 'target/unified-opensearch-e2e-audit'; collect = [sys.executable, 'tools/run-unified-opensearch-e2e.py', '--output-dir', output_dir, '--max-report-age-seconds', '604800', '--suite', 'search-semantic', '--suite', 'vector-search']; check = [sys.executable, 'tools/check-unified-opensearch-e2e-report.py', f'{output_dir}/unified-opensearch-e2e-report.json', '--require-no-skips']; first = subprocess.run(collect, stdout=subprocess.DEVNULL); sys.exit(first.returncode) if first.returncode else sys.exit(subprocess.run(check).returncode)",
        ),
        timeout_seconds=120,
    ),
)
...
E2E_SEARCH_COMPAT_PARITY_BATCH: tuple[ExternalValidation, ...] = (
    ExternalValidation(
        ...
        (
            "python3",
            "-c",
            "import subprocess, sys; output_dir = 'target/unified-opensearch-e2e-current'; collect = [sys.executable, 'tools/run-unified-opensearch-e2e.py', '--output-dir', output_dir, '--max-report-age-seconds', '604800', '--suite', 'search-compat', '--suite', 'search-strict']; check = [sys.executable, 'tools/check-unified-opensearch-e2e-report.py', f'{output_dir}/unified-opensearch-e2e-report.json']; first = subprocess.run(collect, stdout=subprocess.DEVNULL); sys.exit(first.returncode) if first.returncode else sys.exit(subprocess.run(check).returncode)",
        ),
        timeout_seconds=120,
    ),
)
...
BROAD_E2E_PARITY_BATCH: tuple[ExternalValidation, ...] = (
    ExternalValidation(
        ...
        (
            "python3",
            "-c",
            "import subprocess, sys; output_dir = 'target/unified-opensearch-e2e-broad-current'; collect = [sys.executable, 'tools/run-unified-opensearch-e2e.py', '--output-dir', output_dir, '--max-report-age-seconds', '604800']; check = [sys.executable, 'tools/check-unified-opensearch-e2e-report.py', f'{output_dir}/unified-opensearch-e2e-report.json']; first = subprocess.run(collect, stdout=subprocess.DEVNULL); sys.exit(first.returncode) if first.returncode else sys.exit(subprocess.run(check).returncode)",
        ),
        timeout_seconds=180,
    ),
)
