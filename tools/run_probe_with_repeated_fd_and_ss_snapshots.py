...
def snapshot_ss(out_path: Path):
    ss = subprocess.run(['sudo', 'ss', '-tanp'], capture_output=True, text=True)
    out_path.write_text(ss.stdout + ss.stderr, encoding='utf-8')
...
def main() -> int:
    ...
    perf_data_path = work_dir / 'opensearch' / 'sudo-perf-record-fd-ss.data'
    perf_script_path = work_dir / 'opensearch' / 'sudo-perf-record-fd-ss.script.txt'
    fd_snapshot_dir = work_dir / 'opensearch' / 'fd-snapshots'
    ss_snapshot_dir = work_dir / 'opensearch' / 'ss-snapshots'
    ...
    try:
        if wait_for_file(pid_path, 90):
            ...
            ss_snapshot_dir.mkdir(parents=True, exist_ok=True)
            ...
            while perf.poll() is None:
                ...
                snapshot_ss(ss_snapshot_dir / f'{snapshot_count:04d}.ss')
                snapshot_count += 1
                time.sleep(interval)
            perf.communicate()
    ...
    summary = {
        ...
        'ss_snapshot_dir': str(ss_snapshot_dir),
        ...
        'checker_result': (
            'repeated_fd_and_ss_snapshots_collected_on_failing_probe'
            if record_returncode == 0 and script_returncode == 0 and report is not None and snapshot_count > 0
            else 'repeated_fd_and_ss_snapshots_collection_incomplete'
        ),
    }
