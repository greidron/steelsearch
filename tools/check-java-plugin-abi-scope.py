#!/usr/bin/env python3
"""Validate in-scope workstream definition for Java plugin ABI compatibility."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "fixture",
        nargs="?",
        default="tools/fixtures/java-plugin-abi-scope-matrix.json",
    )
...
def main() -> int:
    ...
    if not workstreams:
        raise SystemExit("java plugin ABI scope matrix is empty")

    required_areas = {
        "plugin_loading",
        "extension_point_binding",
        "lifecycle_sandbox",
    }
    seen_areas = set()
