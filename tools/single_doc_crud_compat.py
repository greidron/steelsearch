#!/usr/bin/env python3
"""Run single-document CRUD HTTP compatibility checks against Steelsearch and OpenSearch."""
...
DEFAULT_FIXTURE = ROOT / "tools" / "fixtures" / "single-doc-crud-compat.json"
DEFAULT_OUTPUT = ROOT / "target" / "single-doc-crud-compat-report.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default=os.environ.get("SINGLE_DOC_CRUD_COMPAT_REPORT", str(DEFAULT_OUTPUT)),
    )
...
def check_target(case: dict[str, Any], response: dict[str, Any]) -> list[str]:
    ...
    expected_body_text = compare.get("body_text_exact")
    if expected_body_text is not None and response.get("body_text") != expected_body_text:
        errors.append(
            f"expected body_text {expected_body_text!r} but got {response.get('body_text')!r}"
        )
...
def compare_targets(
    ...
    if compare.get("body_text_equal") and steelsearch.get("body_text") != opensearch.get("body_text"):
        errors.append(
            "body_text drift: "
            f"steelsearch={steelsearch.get('body_text')!r} "
            f"opensearch={opensearch.get('body_text')!r}"
        )
    return errors
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "name": fixture.get("name", "single-doc-crud-compat"),
