...
def main() -> int:
    ...
    perf_data_path = work_dir / 'opensearch' / 'sudo-perf-record-repeated-fd.data'
    perf_script_path = work_dir / 'opensearch' / 'sudo-perf-record-repeated-fd.script.txt'
    ...
    try:
        if wait_for_file(pid_path, 90):
            ...
            while perf.poll() is None:
                ...
                snapshot_count += 1
                time.sleep(interval)
            _, perf_stderr = perf.communicate()
    ...
    summary = {
        ...
        'checker_result': (
            'repeated_fd_snapshots_and_perf_record_collected_on_failing_probe'
            if record_returncode == 0 and script_returncode == 0 and report is not None and snapshot_count > 0
            else 'repeated_fd_snapshots_and_perf_record_collection_incomplete'
        ),
    }
