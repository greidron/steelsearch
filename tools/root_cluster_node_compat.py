#!/usr/bin/env python3
"""Run root/cluster/node HTTP compatibility checks against Steelsearch and OpenSearch."""
...
DEFAULT_FIXTURE = ROOT / "tools" / "fixtures" / "root-cluster-node-compat.json"
DEFAULT_OUTPUT = ROOT / "target" / "root-cluster-node-compat-report.json"
...
def check_target(case: dict[str, Any], response: dict[str, Any]) -> list[str]:
    ...
    expected_body_text = compare.get("expected_body_text")
    ...
    for path, expected in compare.get("body_exact", {}).items():
...
def compare_targets(case: dict[str, Any], steelsearch: dict[str, Any], opensearch: dict[str, Any]) -> list[str]:
    ...
    if compare.get("expected_body_text") is not None and steelsearch.get("body_text") != opensearch.get("body_text"):
        errors.append(
            "body_text drift: "
...
def main() -> int:
    ...
    report: dict[str, Any] = {
        "name": fixture.get("name", "root-cluster-node-compat"),
