...
def main() -> int:
        ...
        if wait_for_file(pid_path, 90):
            ...
            perf = subprocess.run(
                [
                    ...
                    '-p', pid, 'sleep', str(args.trace_seconds),
                ],
                capture_output=True,
                text=True,
                timeout=args.trace_seconds + 30,
            )
            ...
            _, _ = strace_proc.communicate(timeout=args.trace_seconds + 30)
            strace_returncode = 0 if strace_proc.returncode in (0, 124) else strace_proc.returncode
    ...
    summary = {
        ...
        'checker_result': (
            'late_strace_and_perf_uprobe_collected_on_failing_probe'
            if strace_returncode == 0 and perf_returncode == 0 and report is not None and strace_path.exists() and perf_stat_path.exists()
            else 'late_strace_and_perf_uprobe_collection_incomplete'
        ),
    }
