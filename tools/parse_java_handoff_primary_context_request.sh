...
SOURCE_FILE="${CACHE_DIR}/ParseHandoffPrimaryContextRequest.java"
CLASS_FILE="${CACHE_DIR}/ParseHandoffPrimaryContextRequest.class"
...
public class ParseHandoffPrimaryContextRequest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        RecoveryHandoffPrimaryContextRequest request = new RecoveryHandoffPrimaryContextRequest(in);
        if (in.available() != 0) {
            throw new IllegalArgumentException("trailing bytes after RecoveryHandoffPrimaryContextRequest: " + in.available());
        }
        System.out.println("{");
        ...
        System.out.println("  \"cluster_state_version\": " + request.primaryContext().clusterStateVersion() + ",");
        System.out.println("  \"checkpoint_count\": " + request.primaryContext().getCheckpointStates().size());
        System.out.println("}");
    }
}
JAVA
...
java -cp "${LIB_CP}:${TMPDIR:-/tmp}/steelsearch-java-response-cache" org.opensearch.indices.recovery.ParseHandoffPrimaryContextRequest "${payload_hex}" "${version_id}"
