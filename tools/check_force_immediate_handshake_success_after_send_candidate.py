...
def main():
    if len(sys.argv) != 3:
        print("usage: check_force_immediate_handshake_success_after_send_candidate.py BASELINE_REPORT CANDIDATE_REPORT")
        return 2
    baseline = summarize(load(sys.argv[1]))
    candidate = summarize(load(sys.argv[2]))
    result = "force_immediate_handshake_success_after_send_candidate_does_not_restore_formed_handoff"
    if candidate["membership_formed"]:
        result = "force_immediate_handshake_success_after_send_candidate_restored_formed_handoff"
    elif candidate["observed_node_count"] > baseline["observed_node_count"] or candidate["follow_up_count"] > baseline["follow_up_count"]:
        result = "force_immediate_handshake_success_after_send_candidate_improved_shape_but_did_not_restore_formed_handoff"
