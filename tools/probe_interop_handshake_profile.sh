...
FIXTURE_PATH="${ROOT_DIR}/tools/fixtures/interop-handshake-compat.json"
WORK_DIR="${PHASE_B_INTEROP_WORK_DIR:-${ROOT_DIR}/target/phase-b-interop-handshake}"
RAW_OUTPUT="${WORK_DIR}/probe.txt"
REPORT_PATH="${WORK_DIR}/interop-handshake-report.json"
mkdir -p "${WORK_DIR}"
...
for _ in $(seq 1 30); do
  if cargo run -q -p os-tcp-probe -- --addr "${TRANSPORT_ADDR}" > "${RAW_OUTPUT}" 2>"${WORK_DIR}/probe.stderr"; then
    probe_ok=1
    break
  fi
  sleep 1
...
expected = fixture["supported_version_range"]
report = {
    "fixture": fixture_path,
    "transport_addr": transport_addr,
    "observed": raw,
    "expected": {
        "transport_version_id": expected["transport_version_ids"][0],
        "minimum_compatible_transport_version_id": expected["minimum_compatible_transport_version_id"],
    },
    "checks": {
        "remote_version_matches_expected_transport": int(raw.get("remote_version_id", -1)) == expected["transport_version_ids"][0],
        "response_header_matches_expected_min_compat": int(raw.get("response_header_version_id", -1)) == expected["minimum_compatible_transport_version_id"],
        "payload_transport_matches_expected_transport": int(raw.get("transport_version_id", -1)) == expected["transport_version_ids"][0],
        "cluster_name_present": bool(raw.get("cluster_name")),
    },
}
report["summary"] = {
