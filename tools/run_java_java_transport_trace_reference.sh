...
WORK_DIR="${1:-$(mktemp -d -t java-java-trace.XXXXXX)}"
...
CLUSTER_NAME="java-java-trace-ref"
PRIMARY_NAME="java-primary-1"
FOLLOWER_NAME="java-follower-1"
INITIAL_MANAGERS="${PRIMARY_NAME},${FOLLOWER_NAME}"
TRACE_INCLUDE="internal:cluster/coordination/publish_state,internal:cluster/coordination/commit_state"
...
OPENSEARCH_TRANSPORT_TRACER_INCLUDE="${TRACE_INCLUDE}" \
OPENSEARCH_TRANSPORT_TRACER_LOG_LEVEL="TRACE" \
