#!/usr/bin/env python3
"""Validate source transport action rows still point at matching source lines."""
...
DEFAULT_SOURCE = Path("docs/rust-port/generated/source-transport-actions.tsv")
WINDOW_LINES = 8
...
def validate_row(row: dict[str, str], row_number: int) -> list[str]:
    ...
    if action not in window:
        errors.append(
            f"row {row_number}: action {action} not found near {source_path}:{line_number}"
        )
    if transport_handler not in window:
        errors.append(
            f"row {row_number}: handler {transport_handler} not found near {source_path}:{line_number}"
        )
    return errors
