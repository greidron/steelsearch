...
def main() -> int:
    ...
    perf_data_path = work_dir / 'opensearch' / 'sudo-perf-record-fdmap.data'
    perf_script_path = work_dir / 'opensearch' / 'sudo-perf-record-fdmap.script.txt'
    fd_snapshot_path = work_dir / 'opensearch' / 'fd-snapshot.json'
    ...
    try:
        if wait_for_file(pid_path, 90):
            ...
            fd_snapshot = snapshot_fds(pid, fd_snapshot_path)
    ...
    summary = {
        ...
        'fd_snapshot_path': str(fd_snapshot_path),
        ...
        'fd_snapshot_count': len(fd_snapshot),
        ...
        'checker_result': (
            'late_fd_snapshot_and_perf_record_collected_on_failing_probe'
            if record_returncode == 0 and script_returncode == 0 and report is not None and fd_snapshot
            else 'late_fd_snapshot_and_perf_record_collection_incomplete'
        ),
    }
