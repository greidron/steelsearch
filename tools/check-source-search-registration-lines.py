#!/usr/bin/env python3
"""Validate source search registration rows still match source windows."""
...
DEFAULT_SOURCE = Path("docs/rust-port/generated/source-search-registrations.tsv")
WINDOW_LINES = 24
...
def validate_row(row: dict[str, str], row_number: int) -> list[str]:
    ...
    if normalize(expression) not in normalize(window):
        return [
            f"row {row_number}: expression not found near {source_path}:{line_number}: {expression}"
        ]
    return []
