...
def main() -> int:
    if len(sys.argv) != 2:
        print(
            "usage: check_plain_runnable_runs_but_abstractrunnable_does_not.py <opensearch-stdout.log>",
            file=sys.stderr,
        )
        return 2
    ...
    if post_submit_sentinel_ran > 0 and abstract_control_task_ran == 0 and task_body_entry == 0:
        print("checker_result=plain_runnable_runs_but_abstractrunnable_control_and_connectionattempt_do_not")
        return 0
