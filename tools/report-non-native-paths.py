...
FAMILIES: tuple[Family, ...] = (
    ...
    Family(
        ...
        category="security",
        status="structured fail-closed boundary/checklist gate with PEM-marker TLS, certificate/private-key role mismatch rejection, invalid bootstrap file-content redaction, authn bootstrap material preflight, production startup refusal unless STEELSEARCH_SECURITY_ENABLED=true enables runtime security enforcement, shared users/service-account subject parser with service-account-only authentication-users-file acceptance and malformed authentication-users-file rejection, runtime env credential loading through that subject model, shared admin/reader/writer permission evaluator, guarded root/ML/bulk/search/session authn/authz checks, service-account writer authz, single-document read/write role checks, multi-document/document-analysis read role checks, reindex/delete-by-query/update-by-query writer-route checks, secure-settings reload admin-role enforcement, cluster-admin control route enforcement for settings/reroute/decommission, weighted routing, voting config exclusions, snapshot mutation/control routes, template management mutation routes, search/ingest pipeline management routes, stored script management routes, data stream management/rollover routes, alias metadata mutation routes with bulk/named mutation lock-reentry removed, index settings/mapping mutation routes with empty mapping-property update panic removed, index maintenance open/close/cache controls with closed-index reopen resolution, index structure delete/block/resize controls, k-NN settings/model/cache mutation routes, dangling-index/remote-store recovery mutation routes, ingestion pause/resume control routes, and task cancel/rethrottle surfaces, bounded security audit event persistence including permission-evaluator read/write denials and multi-route OpenSearch Security plugin API fail-closed decisions without request-secret persistence, ML connector secret redaction from REST responses and shared runtime persistence, and explicit OpenSearch Security plugin API fail-closed responses",
        next_action="wire HTTP/transport TLS enforcement and promote satisfied security boundaries into the structured production policy before enabling production startup",
        evidence_path=NATIVE_CLOSURE_VALIDATION,
