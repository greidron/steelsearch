...
FAMILIES: tuple[Family, ...] = (
    ...
    Family(
        name="production runtime controls",
        category="runtime",
        status="startup-preflight, startup-readiness, task-cancellation, task-cancel-idempotency, task-terminal-readback, task-restart-readback, task-queue, route-backpressure, task-throttle, task-parent-metadata, task-header, and same-node task-child grouping/cancellation/rethrottle batches are zero-test guarded; search/write success, request-error accounting, active-slot queue waiting/drain, bounded queue-full rejection, maintenance route admission, snapshot route admission, cluster-reroute admission, task-submission admission, repeated cancel idempotency, parent-task-id child cancellation visibility, queued-versus-in-flight cancellation visibility, repeated rethrottle last-write-wins readback, throttle-rate restart readback, cancelled/terminal rethrottle refusal, same-node parent/child independent rethrottle rate readback, terminal task readback/pending-depth separation, acknowledged/failed terminal retention/eviction restart readback, stale cancellation-marker pruning for evicted terminal tasks, and task queue/cancelled-id restart readback are runtime-derived, while multi-level/cross-node child propagation, cancelled-terminal lifecycle, shutdown-window recovery, and multi-node scheduling semantics remain partial",
        next_action="extend task child probes into multi-level/cross-node propagation and route backpressure probes beyond bounded single-node admission into cancelled-terminal lifecycle, shutdown-window recovery, and multi-node scheduling behavior",
        evidence_path=NATIVE_CLOSURE_VALIDATION,
