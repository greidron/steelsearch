...
FAMILIES: tuple[Family, ...] = (
    ...
    Family(
        name="production runtime controls",
        category="runtime",
        status="startup-preflight, startup-readiness, task-cancellation, task-cancel-idempotency, task-terminal-readback, task-restart-readback, task-queue, route-backpressure, task-throttle, task-parent-metadata, task-header, and task-child grouping/cancellation/rethrottle batches are zero-test guarded; search/write success, request-error accounting, active-slot queue waiting/drain, independent mixed search/maintenance backlog drain, bounded queue-full rejection, maintenance route admission, snapshot route admission, cluster-reroute admission, task-submission admission, accepted queued task-submission no-replay across shared-runtime restart and partial shared-state recovery errors, partial-recovery task-submission refusal, live-shutdown task-submission refusal, runtime thread-pool queue/counter restart reset, repeated cancel idempotency, parent-task-id child cancellation visibility, same-node, cross-node, and background-worker descendant cancellation propagation, queued-versus-in-flight cancellation visibility, multi-node queued/in-flight task visibility with remote node metadata, local overload counter isolation from remote task metadata, completion-race terminal refusal without cancelled-marker pollution, repeated rethrottle last-write-wins readback, throttle-rate restart readback, cancel/rethrottle per-request sync-on-restart mutation, accepted in-flight task restart readback/refusal without queued replay, partial shared-state recovery error task-listing/cancel continuity, cancelled-terminal restart-sync and live-shutdown refusal with progress preservation, cancelled/terminal rethrottle refusal, same-node parent/child and multi-level independent rethrottle rate readback, terminal task readback/pending-depth separation, acknowledged/failed terminal retention/eviction restart readback, stale cancellation-marker pruning for evicted terminal tasks, cancelled-task completion and partial-progress status readback through restart until eviction, and task queue/cancelled-id restart readback are runtime-derived, while broader multi-node fairness semantics remain partial",
        next_action="extend route backpressure probes beyond mixed-class drain evidence into broader multi-node fairness behavior",
        evidence_path=NATIVE_CLOSURE_VALIDATION,
        evidence_pattern=r"runtime_thread_pool_classes_drain_independently_under_mixed_backlog",
    ),
    Family(
