...
FAMILIES: tuple[Family, ...] = (
    ...
    Family(
        name="production runtime controls",
        category="runtime",
        status="startup-preflight, startup-readiness, task-cancellation, task-cancel-idempotency, task-terminal-readback, task-restart-readback, task-queue, route-backpressure, task-throttle, task-parent-metadata, task-header, and same-node task-child grouping/cancellation batches are zero-test guarded; search/write success, request-error accounting, active-slot queue waiting/drain, bounded queue-full rejection, maintenance route admission, snapshot route admission, cluster-reroute admission, task-submission admission, repeated cancel idempotency, parent-task-id child cancellation visibility, queued-versus-in-flight cancellation visibility, terminal task readback/pending-depth separation, and task queue/cancelled-id restart readback are runtime-derived, while multi-level/cross-node child propagation, terminal retention/eviction, shutdown-window recovery, and multi-node scheduling semantics remain partial",
