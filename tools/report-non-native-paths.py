...
FAMILIES: tuple[Family, ...] = (
    ...
    Family(
        name="production runtime controls",
        category="runtime",
        status="startup-preflight, startup-readiness, task-cancellation, task-queue, route-backpressure, task-throttle, task-parent-metadata, task-header, and same-node task-child grouping batches are zero-test guarded; search/write success, request-error accounting, active-slot queue waiting/drain, bounded queue-full rejection, maintenance route admission, snapshot route admission, and cluster-reroute admission are runtime-derived, while broader child propagation plus task-submission scheduling semantics remain partial",
        next_action="extend task child probes into multi-level/cross-node propagation and route backpressure probes beyond search/write/maintenance/snapshot/cluster-reroute into task-submission scheduling behavior",
        evidence_path=NATIVE_CLOSURE_VALIDATION,
        evidence_pattern=r"cluster_reroute_waits_drains_and_rejects_when_runtime_pool_is_saturated",
    ),
    Family(
