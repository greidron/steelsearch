...
FAMILIES: tuple[Family, ...] = (
    ...
    Family(
        name="production runtime controls",
        category="runtime",
        status="startup-preflight, startup-readiness, task-cancellation, task-cancel-idempotency, task-terminal-readback, task-restart-readback, task-queue, route-backpressure, task-throttle, task-parent-metadata, task-header, and task-child grouping/cancellation/rethrottle batches are zero-test guarded; search/write success, request-error accounting, active-slot queue waiting/drain, bounded queue-full rejection, maintenance route admission, snapshot route admission, cluster-reroute admission, task-submission admission, runtime thread-pool queue/counter restart reset, repeated cancel idempotency, parent-task-id child cancellation visibility, same-node and cross-node multi-level descendant cancellation propagation, queued-versus-in-flight cancellation visibility, completion-race terminal refusal without cancelled-marker pollution, repeated rethrottle last-write-wins readback, throttle-rate restart readback, cancel/rethrottle per-request sync-on-restart mutation, cancelled/terminal rethrottle refusal, same-node parent/child and multi-level independent rethrottle rate readback, terminal task readback/pending-depth separation, acknowledged/failed terminal retention/eviction restart readback, stale cancellation-marker pruning for evicted terminal tasks, cancelled-task completion readback through restart until eviction, and task queue/cancelled-id restart readback are runtime-derived, while background-worker child propagation, cancelled-terminal partial-progress/shutdown windows, accepted-work replay/refusal during recovery, and multi-node scheduling semantics remain partial",
        next_action="extend task child probes into background-worker propagation and route backpressure probes beyond bounded single-node admission into cancelled-terminal partial-progress/shutdown windows, accepted-work replay/refusal during recovery, and multi-node scheduling behavior",
        evidence_path=NATIVE_CLOSURE_VALIDATION,
