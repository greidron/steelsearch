#!/usr/bin/env python3
"""Validate source node runtime component rows still match source windows."""
...
DEFAULT_SOURCE = Path("docs/rust-port/generated/source-node-runtime-components.tsv")
WINDOW_LINES = 12
...
def validate_row(row: dict[str, str], row_number: int) -> list[str]:
    ...
    if normalize(component) not in normalize(window):
        return [
            f"row {row_number}: component {component} not found near {source_path}:{line_number}"
        ]
    return []
