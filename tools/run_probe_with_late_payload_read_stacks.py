...
def main() -> int:
    ...
    perf_data_path = work_dir / 'opensearch' / 'sudo-perf-record-read-stack.data'
    perf_script_path = work_dir / 'opensearch' / 'sudo-perf-record-read-stack.script.txt'
    ...
    summary = {
        ...
        'checker_result': (
            'late_payload_read_stack_record_collected_on_failing_probe'
            if record_returncode == 0 and script_returncode == 0 and report is not None
            else 'late_payload_read_stack_record_collection_incomplete'
        ),
    }
