...
impl<TScorer: Scorer, TScoreCombiner: ScoreCombiner> Union<TScorer, TScoreCombiner> {
    ...
    fn refill(&mut self) -> bool {
        while let Some(min_doc) = self.docsets.iter().map(DocSet::doc).min() {
            self.offset = min_doc;
            self.cursor = 0;
            self.doc = min_doc;
            refill(
            ...
            if TScoreCombiner::FILTER_MATCHES {
                let mut has_matches = false;
                for (bucket, bitset) in self.bitsets.iter_mut().enumerate() {
                    let mut candidates = *bitset;
                    bitset.clear();
                    while let Some(bit) = candidates.pop_lowest() {
                        let combiner = &mut self.scores[bucket * 64 + bit as usize];
                        if combiner.matches() {
                            bitset.insert_mut(bit);
                            has_matches = true;
                        } else {
                            combiner.clear();
                        }
                ...
                // An entire horizon can fail the threshold without exhausting the postings.
                if !has_matches {
                    continue;
                }
            }
    ...
    TScoreCombiner: ScoreCombiner,
{
    ...
    fn count_including_deleted(&mut self) -> u32 {
        ...
        if TScoreCombiner::FILTER_MATCHES {
            for combiner in self.scores.iter_mut() {
                combiner.clear();
            }
        }
        while self.refill() {
