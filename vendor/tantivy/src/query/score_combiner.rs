...
pub trait ScoreCombiner: Default + Clone + Send + Copy + 'static {
    /// Whether the union must filter buffered documents using `matches`.
    const FILTER_MATCHES: bool = false;

    /// Tests membership after all matching children have been accumulated.
