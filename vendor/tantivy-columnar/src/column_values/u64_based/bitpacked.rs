...
impl ColumnCodecEstimator for BitpackedCodecEstimator {
    ...
    fn serialize(
        ...
        if u64::from(stats.num_rows) * u64::from(num_bits) < 8192 * 8 {
            for val in vals {
                bit_packer.write(divider.divide(val - stats.min_value), num_bits, wrt)?;
            }
            return bit_packer.close(wrt);
        }
        // Batch the native packer's small writes without flushing the outer writer.
        let mut buffered = io::BufWriter::with_capacity(8192, wrt);
        let packed = (|| {
            for val in vals {
                bit_packer.write(divider.divide(val - stats.min_value), num_bits, &mut buffered)?;
            }
            bit_packer.close(&mut buffered)
        })();
        if let Err(error) = packed {
            // Do not let BufWriter's drop retry an already failed write.
            let _ = buffered.into_parts();
            return Err(error);
        }
        buffered.into_inner().map(|_| ()).map_err(|error| {
...
#[cfg(test)]
#[path = "buffered_bitpacked_tests.rs"]
mod buffered_bitpacked_tests;
