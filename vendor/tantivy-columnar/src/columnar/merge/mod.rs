...
pub fn merge_columnar(
    ...
    merge_columnar_impl(columnar_readers, required_columns, merge_row_order, output, true)
}

fn merge_columnar_impl(
    columnar_readers: &[&ColumnarReader],
    required_columns: &[(String, ColumnType)],
    merge_row_order: MergeRowOrder,
    output: &mut impl io::Write,
    ...
    for res in columns_to_merge {
        ...
        if use_raw_u64 {
            if let Some((column_type, columns)) =
                grouped_columns.open_homogeneous_u64(&merge_row_order)?
            {
                if grouped_columns.required_column_type.is_none()
                    && columns.iter().all(Option::is_none)
                {
                    continue;
                }
                ...
                merge_u64_columns(
                    &num_rows_per_columnar, columns.into_iter(), &merge_row_order, &mut column_serializer,
                )?;
...
fn merge_column(
    ...
    match column_type {
        ...
        | ColumnType::Bool => {
            merge_u64_columns(
                num_docs_per_column,
                columns.into_iter().map(|column| column.and_then(dynamic_column_to_u64_monotonic)),
                merge_row_order,
                wrt,
            )?;
        }
...
fn merge_u64_columns(
    num_docs_per_column: &[u32],
    columns: impl ExactSizeIterator<Item = Option<Column<u64>>>,
    merge_row_order: &MergeRowOrder,
    wrt: &mut impl io::Write,
) -> io::Result<()> {
    ...
    for (i, column) in columns.enumerate() {
        if let Some(Column { index, values }) = column {
            column_indexes.push(index);
            column_values.push(Some(values));
        } else {
            column_indexes.push(ColumnIndex::Empty { num_docs: num_docs_per_column[i] });
            column_values.push(None);
        }
    }
    let merged_index = crate::column_index::merge_column_index(&column_indexes, merge_row_order);
    let values = MergedColumnValues {
        column_indexes: &column_indexes,
        column_values: &column_values,
        merge_row_order,
    };
    serialize_column_mappable_to_u64(merged_index, &values, wrt)
}
...
impl GroupedColumnsHandle {
    fn open_homogeneous_u64(
        &self,
        order: &MergeRowOrder,
    ) -> io::Result<Option<(ColumnType, Vec<Option<Column<u64>>>)>> {
        // Raw encoded values are interchangeable only without coercion or row deletion.
        if !matches!(order, MergeRowOrder::Stack(_)) {
            return Ok(None);
        }
        let Some(first) = self.columns.iter().flatten().next() else {
            return Ok(None);
        };
        let ty = first.column_type();
        if !matches!(ty, ColumnType::I64 | ColumnType::U64 | ColumnType::F64
            | ColumnType::Bool | ColumnType::DateTime)
            || self.required_column_type.map_or(false, |required| required != ty)
            || self.columns.iter().flatten().any(|column| column.column_type() != ty)
        {
            return Ok(None);
        }
        ...
        for handle in &self.columns {
            let column = match handle {
                Some(handle) => handle.open_u64_lenient()?.ok_or_else(|| {
                    io::Error::new(io::ErrorKind::InvalidData, "Missing numeric column reader")
                })?,
                None => {
                    columns.push(None);
                    continue;
            ...
            columns.push(if column.values.num_vals() == 0 { None } else { Some(column) });
        }
        Ok(Some((ty, columns)))
    }
