...
fn native_authority_metadata_refresh_and_replacement_do_not_build_source_statistics() {
    for shards in [1, 3] {
        ...
        engine.create_index(CreateIndexRequest { index: "metadata".into(),
            settings: serde_json::json!({"number_of_shards":shards}),
            mappings: serde_json::json!({"properties":{"title":{"type":"text"}}}),
        }).unwrap();
        let query = parse_query(&serde_json::json!({"bool":{"must":[{"match":{"title":"alpha"}}]}})).unwrap();
        for (value, expected) in [(serde_json::json!("alpha beta"), true),
            (serde_json::json!(42), false), (serde_json::json!("alpha restored"), true)] {
            engine.index_document(IndexDocumentRequest { index:"metadata".into(), id:"one".into(),
                source:serde_json::json!({"title":value}),
            }).unwrap();
            engine.refresh(RefreshRequest { indices:vec!["metadata".into()] }).unwrap();
            let store = engine.store.read().unwrap();
            let index = &store.indices["metadata"];
            index.bm25_stats_cache.lock().unwrap().clear();
            assert_eq!(index.native_compound_score_is_authoritative(&query, None), expected);
            assert!(index.bm25_stats_cache.lock().unwrap().is_empty());
        }
        engine.index_document(IndexDocumentRequest { index:"metadata".into(), id:"bad".into(),
            source:serde_json::json!({"title":false}),
        }).unwrap();
        engine.refresh(RefreshRequest { indices:vec!["metadata".into()] }).unwrap();
        engine.delete_document(os_engine::DeleteDocumentRequest {
            index:"metadata".into(), id:"bad".into(),
        }).unwrap();
        {
            let store = engine.store.read().unwrap();
...
fn native_compound_authority_rejects_unproven_leaves_and_respects_shard_stats() {
    ...
    engine.index_document(IndexDocumentRequest {index:"authority-guard".into(),id:"good".into(),
        source:serde_json::json!({"title":"alpha beta","service":"yes"}),
    }).unwrap();
    engine.refresh(RefreshRequest {indices:vec!["authority-guard".into()]}).unwrap();
    let wrap = |leaf: Value| parse_query(&serde_json::json!({"bool":{"must":[leaf]}})).unwrap();
    let query = wrap(serde_json::json!({"multi_match":{"fields":["title^2"],"query":"alpha beta"}}));
    let (good_shard, bad_shard, bad_id) = {
        ...
        for leaf in [
            ...
            assert!(!index.native_compound_score_is_authoritative(&wrap(leaf.clone()),None),"{leaf}");
        }
        let good = index.documents.shard_id_for_write("good",None);
        let (bad_id,bad) = (0..100).map(|n|format!("bad-{n}"))
            .map(|id| {let shard=index.documents.shard_id_for_write(&id,None);(id,shard)})
            .find(|(_,shard)| *shard != good).unwrap();
        (good,bad,bad_id)
    };
    engine.index_document(IndexDocumentRequest {index:"authority-guard".into(),id:bad_id,
    ...
    let index = &store.indices["authority-guard"];
    assert!(!index.native_compound_score_is_authoritative(&query,None));
    assert!(index.native_compound_score_is_authoritative(&query,Some(&BTreeSet::from([good_shard]))));
    assert!(!index.native_compound_score_is_authoritative(&query,Some(&BTreeSet::from([bad_shard]))));
}
...
fn native_compound_authority_preserves_pages_scores_and_min_score() {
    ...
    for definition in fixture["indices"].as_array().unwrap() {
        ...
        let engine = TantivyEngine::default();
        engine.create_index(CreateIndexRequest {index:name.into(),
            settings:definition["body"]["settings"].clone(), mappings:definition["body"]["mappings"].clone(),
        }).unwrap();
        let bulk = fixture["bulk"].as_array().unwrap().iter().find(|bulk| bulk["index"] == name).unwrap();
        for doc in bulk["documents"].as_array().unwrap() {
            engine.index_document(IndexDocumentRequest {index:name.into(),
                id:doc["_id"].as_str().unwrap().into(),source:doc["_source"].clone(),
            }).unwrap();
        }
        ...
        for case in fixture["cases"].as_array().unwrap().iter()
            ...
            if !matches!(query, Query::Bool {..}) { continue; }
            let scores = {
                let store = engine.store.read().unwrap();
                let index = &store.indices[name];
                assert!(index.native_compound_score_is_authoritative(&query,None), "{}",case["name"]);
                assert!(!index.native_query_needs_post_filter(&query,None));
                native_scores(index,&query,false)
            };
            let sorts = [vec![],vec![serde_json::from_value::<SortSpec>(serde_json::json!({"field":"latency"})).unwrap()]];
            for min_score in [None,Some(1.42)] {
                for sort in &sorts {
                    for from in 0..3 {
                        let mut envelope = serde_json::json!({"query":body["query"]});
                        if let Some(minimum) = min_score { envelope["min_score"] = serde_json::json!(minimum); }
                        ...
                        let expected_count = scores.values().filter(|&&score| min_score.map_or(true, |min| score >= min as f32)).count();
                        assert_eq!(response.total_hits as usize,expected_count,"{} min={min_score:?}",case["name"]);
                        assert_eq!(response.hits.len(),usize::from(from < expected_count));
                        for hit in response.hits {
                            assert_eq!(hit.score.to_bits(),scores[&hit.metadata.id].to_bits(),
                                "{} min={min_score:?} sort={sort:?} from={from}",case["name"]);
...
fn native_msm_collectors_seek_horizons_and_deletions() {
    ...
    for doc in 0..8200 {
        ...
        if [4096, 4098, 8193].contains(&doc) { document.add_text(field, "beta"); }
        if doc == 8193 { document.add_text(field, "deleted"); }
        writer.add_document(document).unwrap();
    }
    writer.commit().unwrap();
    let reader = index.reader().unwrap();
    let make_term = |text: &str, score| -> Box<dyn TantivyQueryTrait> {
        Box::new(ConstScoreQuery::new(Box::new(TermQuery::new(
            Term::from_field_text(field, text), tantivy::schema::IndexRecordOption::Basic,
        )), score))
    };
    let query = MinimumShouldMatchQuery::new(vec![make_term("alpha", 0.0), make_term("beta", 2.0)], 2);
    for deleted in [false, true] {
        ...
        assert_eq!(searcher.search(&query, &tantivy::collector::Count).unwrap(), expected);
        ...
        assert!(searcher.search(&boosted, &TopDocs::with_limit(10)).unwrap()
            .iter().all(|(score, _)| *score == 6.0));
        ...
        let duplicate = MinimumShouldMatchQuery::new((0..128).map(|_| make_term("alpha", 0.0)).collect(), 100);
        assert_eq!(searcher.search(&duplicate, &tantivy::collector::Count).unwrap(), if deleted {8199} else {8200});
        let sparse = MinimumShouldMatchQuery::new(vec![make_term("beta", 0.0)], 1);
        assert_eq!(searcher.search(&sparse, &tantivy::collector::Count).unwrap(), expected);
        for impossible in [MinimumShouldMatchQuery::new(vec![], 1),
            MinimumShouldMatchQuery::new(vec![make_term("alpha", 1.0)], 2),
            MinimumShouldMatchQuery::new(vec![make_term("alpha", 1.0), make_term("absent", 1.0)], 2)] {
            assert_eq!(searcher.search(&impossible, &tantivy::collector::Count).unwrap(), 0);
        }
        assert!(MinimumShouldMatchQuery::new(vec![], 0)
            .weight(EnableScoring::enabled_from_searcher(&searcher)).is_err());
    }
}
...
fn native_minimum_should_match_overlap_audit() {
    ...
    for shards in [1, 3] {
        ...
        for mask in 0u32..32 {
            let tags = (0..5).filter(|bit| mask & (1 << bit) != 0)
                .map(|bit| format!("t{bit}")).collect::<Vec<_>>();
        ...
        engine.refresh(RefreshRequest {indices: vec!["msm-overlap".into()]}).unwrap();
        let store = engine.store.read().unwrap();
        let index = &store.indices["msm-overlap"];
        for minimum in 0usize..=5 {
            let should = (0..5).map(|bit| {
                let term = serde_json::json!({"term": {"tags": format!("t{bit}")}});
                if bit == 0 { serde_json::json!({"bool": {"filter": [term]}}) } else { term }
            }).collect::<Vec<_>>();
            ...
            let expected = (0u32..31).filter(|mask| mask.count_ones() as usize >= minimum)
                .map(|mask| {
            ...
            assert_eq!(probe, expected, "diagnostic max composition: shards={shards} minimum={minimum}");
            assert_eq!(actual, expected, "native MSM: shards={shards} minimum={minimum}");
...
fn native_compound_ranking_decomposition_audit() {
    ...
    for definition in fixture["indices"].as_array().unwrap() {
        ...
        let bulk = fixture["bulk"].as_array().unwrap().iter()
            .find(|bulk| bulk["index"] == name).unwrap();
        ...
        for case in fixture["cases"].as_array().unwrap().iter()
            ...
            if let Query::Bool { clauses } = &query {
                let groups = [(&clauses.must, "must"), (&clauses.should, "should"),
                    (&clauses.filter, "filter"), (&clauses.must_not, "must_not")];
                let mut expected = BTreeMap::new();
                let scored_groups = groups.iter().map(|(queries, occurrence)| {
                    let scored = queries.iter().map(|child| native_scores(index, child, false))
                        .collect::<Vec<_>>();
                    parts.push(serde_json::json!({"occurrence":occurrence,"native_scores":scored}));
                    scored
                }).collect::<Vec<_>>();
                for document in index.refreshed_documents_for_shards(None) {
                    let id = &document.metadata.id;
                    let matched_should = scored_groups[1].iter().filter(|hits| hits.contains_key(id)).count();
                    if scored_groups[0].iter().all(|hits| hits.contains_key(id))
                        ...
                        let score = scored_groups[0].iter().chain(&scored_groups[1])
                            .filter_map(|hits| hits.get(id)).copied().sum::<f32>();
                        expected.insert(id.clone(), score);
                    }
                }
                assert_eq!(native.keys().collect::<Vec<_>>(), expected.keys().collect::<Vec<_>>(),
                    "native boolean membership: {}", case["name"]);
                let delta = comparison(&native, &expected);
                assert!(delta["max_absolute_difference"].as_f64().unwrap() < 1e-5,
                    "native additive scoring: {} {delta}", case["name"]);
                let probe_delta = comparison(&probe, &expected);
                assert_eq!(probe.keys().collect::<Vec<_>>(), expected.keys().collect::<Vec<_>>());
                assert!(probe_delta["max_absolute_difference"].as_f64().unwrap() < 1e-5,
                    "native dismax additive scoring: {} {probe_delta}", case["name"]);
                parts.push(serde_json::json!({"additive_expected":expected,"native_vs_additive":delta,
...
fn native_exact_phrase_pages_preserve_native_scores_and_boosts() {
    for shards in [1,3] {
        ...
        for (id,body) in ["alpha beta","alpha beta alpha beta","alpha gap beta","other"].iter().enumerate() {
            engine.index_document(IndexDocumentRequest {index:"exact-phrase".into(),id:id.to_string(),
                source:serde_json::json!({"body":body,"n":id}),
            }).unwrap();
        }
        engine.refresh(RefreshRequest {indices:vec!["exact-phrase".into()]}).unwrap();
        let mut store = engine.store.write().unwrap();
        let index = store.indices.get_mut("exact-phrase").unwrap();
        let sorts = [vec![],vec![serde_json::from_value::<SortSpec>(serde_json::json!({"field":"n"})).unwrap()]];
        for (text, slop) in [("alpha beta",0),("alpha",0),("alpha beta",1),("alpha beta",3),("alpha alpha",1)] {
                ...
                for sort in &sorts {
                    for from in 0..3 {
                        let (total,hits) = index.search_hits_page_for_query_native_scoped(
                            "exact-phrase",None,&query,sort,from,1,
                        ).unwrap().unwrap();
...
fn native_exact_phrase_eligibility_rejects_unproven_options_and_values() {
    for options in [serde_json::json!({"analyzer":"keyword"}),serde_json::json!({"norms":false}),
        serde_json::json!({"position_increment_gap":"not-a-number"}),serde_json::json!({"index_options":"docs"}),
        serde_json::json!({"search_quote_analyzer":"simple"}),serde_json::json!({"similarity":"other"})] {
        let parsed: TantivyTextOptions = serde_json::from_value(options).unwrap();
        assert!(!parsed.supports_native_exact_phrase());
    }
    for value in [serde_json::json!("caf\u{e9}"),serde_json::json!({"body":"alpha"})] {
        assert!(!native_phrase_source_value_supported(&value));
    }
    for value in [serde_json::json!(5), serde_json::json!(true),
        serde_json::json!(["alpha",null,["beta",5,true]])] {
        assert!(native_phrase_source_value_supported(&value));
    }
    for gap in [serde_json::json!(0), serde_json::json!(1), serde_json::json!(5),
        serde_json::json!("100")] {
        let parsed: TantivyTextOptions = serde_json::from_value(
            serde_json::json!({"position_increment_gap":gap}),
        ).unwrap();
        assert!(parsed.supports_native_exact_phrase());
    }
    let engine = TantivyEngine::default();
    engine.create_index(CreateIndexRequest {index:"guard".into(),settings:serde_json::json!({}),
        mappings:serde_json::json!({"properties":{"body":{"type":"text"}}}),
    }).unwrap();
    engine.index_document(IndexDocumentRequest {index:"guard".into(),id:"1".into(),
        source:serde_json::json!({"body":"alpha beta"}),
    }).unwrap();
    engine.refresh(RefreshRequest {indices:vec!["guard".into()]}).unwrap();
    let mut store = engine.store.write().unwrap();
    ...
    for slop in [0,1,2,100] {
        let query = parse_query(&serde_json::json!({"match_phrase":{"body":{"query":"alpha beta","slop":slop}}})).unwrap();
        assert!(index.native_phrase_score_is_authoritative(&query,None));
    }
    for body in [serde_json::json!({"query":"alpha beta","slop":4294967296u64}),
    ...
    index.schema.fields.iter_mut().find(|field| field.name == "body").unwrap().text_options = None;
    assert!(!index.native_phrase_score_is_authoritative(&query,None));
}

#[test]
fn default_text_metadata_migration_keeps_legacy_schema_checks() {
    let create = CreateIndexRequest {index:"legacy-phrase".into(),settings:serde_json::json!({}),
        mappings:serde_json::json!({"properties":{"body":{"type":"text"}}})};
    let current = map_opensearch_index_to_tantivy_schema(&create).unwrap();
    let mut legacy = current.clone();
    for field in &mut legacy.fields { field.text_options = None; }
    ...
    assert!(!matches_legacy_default_text_schema_hash(&create.index,&changed,old_hash).unwrap());
    let restored: TantivyIndexSchema = serde_json::from_value(serde_json::to_value(&current).unwrap()).unwrap();
    assert_eq!(restored,current);
    let engine = TantivyEngine::default();
    engine.create_index_from_schema(create.index.clone(),legacy).unwrap();
    engine.index_document(IndexDocumentRequest {index:create.index.clone(),id:"1".into(),
        source:serde_json::json!({"body":"alpha beta alpha beta"}),
    }).unwrap();
    let path = std::env::temp_dir().join(format!("steel-native-phrase-migration-{}-{}",
        std::process::id(),std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
    engine.persist_shard_state(&create.index,&path).unwrap();
    let recovered = TantivyEngine::default();
    recovered.recover_index_from_manifest(&create.index,current.clone(),&path).unwrap();
    let migrated = recovered.shard_manifest(&create.index).unwrap();
    assert_eq!(migrated.schema_hash,schema_hash(&create.index,&current).unwrap());
    assert!(TantivyEngine::default().recover_index_from_manifest(&create.index,changed,&path).is_err());
    recovered.persist_shard_state(&create.index,&path).unwrap();
    TantivyEngine::default().recover_index_from_manifest(&create.index,current,&path).unwrap();
...
fn native_exact_phrase_value_guard_follows_refresh() {
    let engine = TantivyEngine::default();
    engine.create_index(CreateIndexRequest {index:"phrase-refresh".into(),settings:serde_json::json!({}),
        mappings:serde_json::json!({"properties":{"body":{"type":"text"}}}),
    }).unwrap();
    let query = parse_query(&serde_json::json!({"match_phrase":{"body":"alpha beta"}})).unwrap();
    for (body,expected) in [(serde_json::json!("alpha beta"),true),
        (serde_json::json!("alpha caf\u{e9}"),false),(serde_json::json!(5),false),
        (serde_json::json!("alpha beta alpha beta"),true)] {
        engine.index_document(IndexDocumentRequest {index:"phrase-refresh".into(),id:"1".into(),
            source:serde_json::json!({"body":body}),
        }).unwrap();
        engine.refresh(RefreshRequest {indices:vec!["phrase-refresh".into()]}).unwrap();
        let store = engine.store.read().unwrap();
        assert_eq!(store.indices["phrase-refresh"].native_phrase_score_is_authoritative(&query,None),expected);
    }
}
...
fn native_text_metadata_preserves_accepted_option_input_shapes() {
    ...
    for definition in fixture["indices"].as_array().unwrap() {
        let request = CreateIndexRequest {index:definition["name"].as_str().unwrap().into(),
            settings:definition["body"]["settings"].clone(),
            mappings:definition["body"]["mappings"].clone()};
        let schema = map_opensearch_index_to_tantivy_schema(&request).unwrap();
        let options = schema.fields.iter().find(|field| field.name == "body").unwrap()
            .text_options.as_ref().unwrap();
    ...
    assert_eq!(std::mem::size_of::<Option<Box<TantivyTextOptions>>>(),std::mem::size_of::<usize>());
}
...
fn native_pre_tokenized_gap_preserves_term_scores_and_norms() {
    ...
    while stream.advance() { tokens.push(stream.token().clone()); }
    assert_eq!(tokens.len(), 2);
    tokens[1].position = 101;
    let mut spaced = tantivy::Document::default();
    spaced.add_pre_tokenized_text(field, PreTokenizedString {text:"alpha beta".into(),tokens});
    ...
    for doc in 0..3 { assert_eq!(norms.fieldnorm(doc), 2); }
    let beta = Term::from_field_text(field,"beta");
    let mut postings = inverted.read_postings(&beta,IndexRecordOption::WithFreqsAndPositions)
        .unwrap().unwrap();
    ...
    assert!(hits.iter().all(|hit| hit.0.to_bits() == hits[0].0.to_bits()));
    for (terms, cases) in [
        (["alpha","beta"],vec![(0,1),(1,2),(99,2),(100,3),(101,3)]),
        (["beta","alpha"],vec![(0,0),(1,0),(2,1),(3,2),(101,2),(102,3)]),
    ] {
        for (slop,expected) in cases {
            let query = PhraseQuery::new_with_offset_and_slop(terms.iter().enumerate()
                .map(|(offset,token)| (offset,Term::from_field_text(field,token))).collect(),slop);
...
fn native_phrase_frequency_and_position_audit() {
    ...
    for definition in fixture["indices"].as_array().unwrap() {
        ...
        let states = if index.documents.shard_count == 1 {
            ...
            index.shard_search_states_for(None).map(|(_, state)| state).collect()
        };
        for case in fixture["cases"].as_array().unwrap().iter()
            .filter(|case| case["steps"][0]["path"] == format!("/{name}/_search")) {
            ...
            for state in &states {
                ...
                for (score, address) in state.searcher.search(
                    native.as_ref(), &TopDocs::with_limit(100),
                ).unwrap() {
                    ...
                    for token in ["alpha", "beta"] {
                        let mut postings = inverted.read_postings(
...
fn native_repeated_phrase_frequency_audit() {
    ...
    for definition in fixture["indices"].as_array().unwrap() {
        ...
        for case in fixture["cases"].as_array().unwrap().iter().filter(|case|
            ...
            let tokens = body["match_phrase"]["body"]["query"].as_str().unwrap()
            ...
            for state in &states {
                ...
                for (score, address) in state.searcher.search(
                    ...
                    let id = state.document_id_for_address(&state.searcher, address).unwrap();
                    let inverted = state.searcher.segment_reader(address.segment_ord)
                        .inverted_index(field).unwrap();
                    let mut positions = BTreeMap::new();
                    for token in &tokens {
                        let mut postings = inverted.read_postings(
                            &Term::from_field_text(field, token), IndexRecordOption::WithFreqsAndPositions,
                        ).unwrap().unwrap();
                        if postings.doc() < address.doc_id { postings.seek(address.doc_id); }
                    ...
                    assert!(score.is_finite() && score > 0.0);
                    assert!(hits.insert(id.to_owned(), serde_json::json!({
                        "score": score, "positions": positions,
            ...
            let ordered_tokens = body["match_phrase"]["body"]["query"].as_str().unwrap()
                .split_whitespace().collect::<Vec<_>>();
            let term_ids = ordered_tokens.iter().map(|token|
                tokens.iter().position(|candidate| candidate == token).unwrap()).collect::<Vec<_>>();
            ...
            for state in &states {
                let field = state.fields["body"].field;
                let phrase = super::native_phrase::NativePhraseQuery::new(ordered_tokens.iter()
                    .enumerate().map(|(offset, token)| (offset, Term::from_field_text(field, token)))
                    .collect(), slop).unwrap();
                let normalized = state.bm25_field_statistics.wrap(Box::new(phrase), field, &state.searcher);
                let boosted = BoostQuery::new(normalized, body["match_phrase"]["body"]["boost"].as_f64().unwrap() as f32);
                let scores = state.searcher.search(&boosted, &TopDocs::with_limit(100)).unwrap();
                assert_eq!(state.searcher.search(&boosted, &Count).unwrap(), scores.len());
                let seek_scores = super::native_bm25::score_addresses(&state.searcher, &boosted,
                    scores.iter().map(|(_, address)| *address).collect()).unwrap();
                assert_eq!(scores, seek_scores);
                for (score, address) in scores {
                    let id = state.document_id_for_address(&state.searcher, address).unwrap();
                    let explanation = serde_json::to_value(boosted.explain(&state.searcher, address).unwrap()).unwrap();
                    fn find_frequency(value: &Value) -> Option<f64> {
                        if value["description"] == "phraseFreq" { return value["value"].as_f64(); }
                        value["details"].as_array()?.iter().find_map(find_frequency)
                    }
                    let frequency = find_frequency(&explanation).unwrap() as f32;
                    assert!(scored.insert(id.to_owned(), serde_json::json!({
                ...
                let conjunction = BooleanQuery::new(tokens.iter().map(|token| (
                    Occur::Must, Box::new(TermQuery::new(Term::from_field_text(field, token),
                        IndexRecordOption::Basic)) as Box<dyn TantivyQueryTrait>,
                )).collect());
                for address in state.searcher.search(&conjunction, &DocSetCollector).unwrap() {
                    let inverted = state.searcher.segment_reader(address.segment_ord)
                        .inverted_index(field).unwrap();
                    let positions = ordered_tokens.iter().map(|token| {
                        let mut postings = inverted.read_postings(&Term::from_field_text(field, token),
                            IndexRecordOption::WithFreqsAndPositions).unwrap().unwrap();
                        if postings.doc() < address.doc_id { postings.seek(address.doc_id); }
                        assert_eq!(postings.doc(), address.doc_id);
                        let mut positions = Vec::new();
                        postings.positions(&mut positions);
            ...
            let expected = reference["rows"].as_array().unwrap().iter()
                .find(|row| row["name"] == case["name"]).unwrap()["hits"].as_object().unwrap();
            assert_eq!(matched.keys().collect::<BTreeSet<_>>(), expected.keys().collect::<BTreeSet<_>>(),
                "{}", case["name"]);
            assert_eq!(scored.keys().collect::<BTreeSet<_>>(), expected.keys().collect::<BTreeSet<_>>(),
                "{}", case["name"]);
            for (id, frequency) in &matched {
                assert_eq!(*frequency, expected[id]["frequencies"][0].as_f64().unwrap() as f32,
                    "{} {id}", case["name"]);
            ...
            if body["match_phrase"]["body"]["slop"] == 0 {
                let expected = reference["rows"].as_array().unwrap().iter()
                    .find(|row| row["name"] == case["name"]).unwrap()["hits"].as_object().unwrap();
                assert_eq!(hits.keys().collect::<BTreeSet<_>>(), expected.keys().collect::<BTreeSet<_>>());
                fn frequencies(explanation: &Value, values: &mut Vec<f64>) {
                    if explanation["description"].as_str().unwrap().starts_with("freq,") {
                        values.push(explanation["value"].as_f64().unwrap());
                    }
                    if let Some(details) = explanation["details"].as_array() {
                        for detail in details { frequencies(detail, values); }
                    }
                }
                for (id, hit) in &hits {
                    let mut actual = Vec::new();
                    frequencies(&hit["explanation"], &mut actual);
                    let expected = expected[id]["frequencies"].as_array().unwrap().iter()
...
fn compose_native_probe(state: &TantivySearchState, query: &Query) -> Box<dyn TantivyQueryTrait> {
    match query {
        Query::Bool { clauses } if effective_bool_minimum_should_match(clauses) >= 1 => {
            let mut built = clauses.must.iter().map(|query| {
                (Occur::Must, compose_native_probe(state, query))
            }).collect::<Vec<_>>();
            built.extend(clauses.filter.iter().map(|query| {
                (Occur::Must, Box::new(ConstScoreQuery::new(
                    compose_native_probe(state, query), 0.0,
                )) as Box<dyn TantivyQueryTrait>)
            }));
            built.extend(clauses.must_not.iter().map(|query| {
                (Occur::MustNot, compose_native_probe(state, query))
            }));
            let minimum = effective_bool_minimum_should_match(clauses) as usize;
            let optional: Box<dyn TantivyQueryTrait> = if minimum == 1 {
                Box::new(BooleanQuery::new(clauses.should.iter().map(|query| {
                    (Occur::Should, compose_native_probe(state, query))
                }).collect()))
            } else {
                ...
                let alternatives = query_index_combinations(clauses.should.len(), minimum)
                    .into_iter().map(|combination| {
                        Box::new(BooleanQuery::new(clauses.should.iter().enumerate().map(|(i, query)| {
                            let occur = if combination.contains(&i) { Occur::Must } else { Occur::Should };
                            (occur, compose_native_probe(state, query))
                        }).collect())) as Box<dyn TantivyQueryTrait>
                    }).collect();
...
pub(super) fn native_scores(
    index: &StoredIndex, query: &Query, probe: bool,
) -> BTreeMap<String, f32> {
    let states = if index.documents.shard_count <= 1 {
        index.search_state.iter().collect::<Vec<_>>()
    } else {
    ...
    for state in states {
        let built = if probe { compose_native_probe(state, query) } else {
            build_tantivy_query(state, query).unwrap().expect("audit query has a native builder")
        };
        for (score, address) in state.searcher.search(
            built.as_ref(), &TopDocs::with_limit(100),
        ).unwrap() {
            let id = state.document_id_for_address(&state.searcher, address).unwrap();
...
fn comparison(left: &BTreeMap<String, f32>, right: &BTreeMap<String, f32>) -> Value {
    let missing = right.keys().filter(|id| !left.contains_key(*id)).collect::<Vec<_>>();
    let extra = left.keys().filter(|id| !right.contains_key(*id)).collect::<Vec<_>>();
    ...
    for (id, &actual) in left {
        if let Some(&expected) = right.get(id) {
            ...
            if expected != 0.0 { max_relative = max_relative.max(difference / f64::from(expected).abs()); }
...
fn native_ranking_leaf_and_bool_audit() {
    ...
    let queries = vec![
        ("match-title", serde_json::json!({"match":{"title":"alpha beta"}})),
        ("match-body", serde_json::json!({"match":{"body":"alpha beta"}})),
        ("multi-match", multi),
        ("phrase-exact", serde_json::json!({"match_phrase":{"body":"alpha beta"}})),
        ("phrase-slop", phrase),
        ("phrase-boost", serde_json::json!({"match_phrase":{"body":{"query":"alpha beta", "slop":1, "boost":2}}})),
        ("bool-overlap", serde_json::json!({"bool":{
            "must":{"match_all":{}},
    ...
    for shards in [1, 3] {
        ...
        engine.create_index(CreateIndexRequest {
            index:name.clone(), settings:settings.clone(), mappings:mappings.clone(),
        }).unwrap();
        for document in &documents {
            engine.index_document(IndexDocumentRequest {
                index:name.clone(), id:document["_id"].as_str().unwrap().into(),
                source:document["_source"].clone(),
            }).unwrap();
        }
        engine.refresh(RefreshRequest {indices:vec![name.clone()]}).unwrap();
        let store = engine.store.read().unwrap();
        let index = &store.indices[&name];
        fixture_indices.push(serde_json::json!({"name":name,"body":{"settings":settings,"mappings":mappings}}));
        fixture_bulk.push(serde_json::json!({"index":name,"documents":documents}));
        for (case, body) in &queries {
            let query = parse_query(body).unwrap();
            let source = index.refreshed_documents_for_shards(None).into_iter().filter_map(|document| {
                index.score_document_query(&query, document).unwrap().map(|score| (document.metadata.id.clone(), score))
            }).collect::<BTreeMap<_, _>>();
            let native = native_scores(index, &query, false);
            let probe = native_scores(index, &query, true);
            if *case == "bool-overlap" {
                ...
                assert_eq!(native["exact"], 3.0, "native optional group must not double count");
...
fn native_minimum_one_preserves_matches_and_scores_across_overlapping_shoulds() {
    for shards in [1, 3] {
        ...
        for mask in 0..16 {
            engine.index_document(IndexDocumentRequest {
                index:"native-minimum-one".into(), id:mask.to_string(),
        ...
        engine.refresh(RefreshRequest {indices:vec!["native-minimum-one".into()]}).unwrap();
        let store = engine.store.read().unwrap();
        let index = &store.indices["native-minimum-one"];
        let term = |field:&str| Query::Term {
            field:field.into(), value:serde_json::json!("yes"), case_insensitive:false,
        };
        for count in 1..=4 {
            for required in [false, true] {
                for filtered in [false, true] {
                    for excluded in [false, true] {
                        let query = Query::Bool {clauses:BoolQuery {
                            must:if required {vec![Query::MatchAll]} else {vec![]},
                            should:["a","b","c","a"].iter().take(count).map(|field| term(field)).collect(),
                            filter:if filtered {vec![term("allowed")]} else {vec![]},
                            must_not:if excluded {vec![term("c")]} else {vec![]},
                            minimum_should_match:Some(1),
                        }};
                        let source = index.refreshed_documents_for_shards(None).into_iter()
                            .filter_map(|document| index.score_document_query(&query, document).unwrap()
