...
mod tests {
    ...
    fn lookup_hints_validate_keys_across_documents_and_container_transitions() {
        let keys = [
            "", "a", "a\0", "\0", "\u{00e9}", "a.b", "ab", "abc", "abcd", "abcde",
        ];
    ...
    fn cache_container_does_not_grow_document_fields() {
        assert!(
            std::mem::size_of::<FieldCache<String>>()
                <= std::mem::size_of::<BTreeMap<String, String>>()
        );
        assert!(
            std::mem::size_of::<FieldCache<f64>>() <= std::mem::size_of::<BTreeMap<String, f64>>()
        );
        assert!(
            std::mem::size_of::<FieldCache<i64>>() <= std::mem::size_of::<BTreeMap<String, i64>>()
        );
    }
    ...
    fn duplicate_heavy_inputs_preserve_canonical_representation_and_owned_values() {
        for unique in [1, 2, 8, 9, 16] {
            for count in [1, 7, 8, 9, 17, 64] {
                let input = (0..count)
                    .map(|i| (format!("field-{}", i % unique), format!("value-{i}")))
                    .collect::<Vec<_>>();
                let tree = input.iter().cloned().collect::<BTreeMap<_, _>>();
                let cache = input.into_iter().collect::<FieldCache<_>>();
