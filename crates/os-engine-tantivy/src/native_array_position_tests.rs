    ...
    for definition in fixture["indices"].as_array().unwrap() {
        ...
        for chunk in documents.chunks(8) {
            ...
            engine
                ...
                .unwrap();
            if old.is_none() {
                old = engine.store.read().unwrap().indices[name]
                    .search_state
                    .clone();
            }
        }
        ...
        let old_expected = expected
            .iter()
            .copied()
            .filter(|row| documents[..8].iter().any(|doc| doc["_id"] == row["id"]))
            .collect::<Vec<_>>();
        ...
        let total_tokens: u64 = state
            ...
            .segment_readers()
            .iter()
            .map(|segment| segment.inverted_index(field).unwrap().total_num_tokens())
            .sum();
            ...
            .filter(|case| case["steps"][0]["path"] == format!("/{name}/_search"))
        {
            ...
            let actual = state
                .searcher
                .search(built.as_ref(), &DocSetCollector)
                .unwrap()
                .into_iter()
                .map(|address| {
                    state
                ...
                .collect::<BTreeSet<_>>();
            let expected = reference["phrase_matches"]
                .as_array()
                .unwrap()
                .iter()
...
fn native_array_position_overflow_does_not_queue_partial_batch() {
    ...
    engine
        .index_document(IndexDocumentRequest {
            index: "position-overflow".into(),
            id: "one".into(),
            source: serde_json::json!({"body":"alpha"}),
        })
        .unwrap();
    let store = engine.store.read().unwrap();
    ...
    let mut state = TantivySearchState::build_from_documents(
        &index.schema,
        std::iter::empty::<&StoredDocument>(),
        -1,
    )
    .unwrap();
    let old = state.clone();
    assert!(state
        .append_documents(&[Arc::clone(&first), Arc::new(invalid)])
        .is_err());
