...
fn native_integer_range_candidates_match_source_for_exact_values() {
    for (shards, fast) in [(1, true), (3, true), (1, false), (3, false)] {
        ...
        let values = [
            Value::Null, serde_json::json!([]), serde_json::json!([null]),
            serde_json::json!([[-2, null], [0, 2]]),
            serde_json::json!(i64::MIN), serde_json::json!(i64::MIN + 1),
            serde_json::json!(-2), serde_json::json!(0), serde_json::json!(2),
            serde_json::json!(9_007_199_254_740_992_i64),
            serde_json::json!(9_007_199_254_740_993_i64),
            serde_json::json!(i64::MAX - 1), serde_json::json!(i64::MAX),
        ];
        for (id, value) in values.into_iter().enumerate() {
            engine.index_document(IndexDocumentRequest {
                index: "exact-range".into(), id: id.to_string(),
                source: serde_json::json!({"n": value}),
            }).unwrap();
        }
        engine.index_document(IndexDocumentRequest {
            index: "exact-range".into(), id: "missing".into(), source: serde_json::json!({}),
        }).unwrap();
        engine.refresh(RefreshRequest {indices: vec!["exact-range".into()]}).unwrap();
        let store = engine.store.read().unwrap();
        let index = &store.indices["exact-range"];
        let endpoints = [i64::MIN, i64::MIN + 1, -2, 0, 2,
            9_007_199_254_740_992, 9_007_199_254_740_993, i64::MAX - 1, i64::MAX];
            ...
            for upper in &endpoints[offset..] {
                for lower_operator in ["gt", "gte"] {
                    for upper_operator in ["lt", "lte"] {
                        cases.push(serde_json::json!({lower_operator: lower, upper_operator: upper}));
                    }
                }
            }
        }
        ...
        for bounds in cases {
            ...
            let native = index.search_documents_for_tantivy_query_unordered(&query).unwrap().unwrap()
                .into_iter().map(|document| document.metadata.id.clone()).collect::<BTreeSet<_>>();
            let source = index.refreshed_documents_for_shards(None).into_iter()
                .filter(|document| index.score_document_query(&query, document).unwrap().is_some())
                .map(|document| document.metadata.id.clone()).collect::<BTreeSet<_>>();
            assert_eq!(native, source, "shards={shards} fast={fast} bounds={bounds}");
        }
    }
}
...
fn native_integer_range_candidates_still_need_source_guard() {
    ...
    let cases = [
        ("unsigned-source", serde_json::json!({"n": u64::MAX}), "n",
            serde_json::json!({"lt": 0}), true, false),
        ("unsigned-bound", serde_json::json!({"n": -1}), "n",
            serde_json::json!({"gte": u64::MAX}), true, false),
        ("dual-lower", serde_json::json!({"n": 0}), "n",
            serde_json::json!({"gte": 0, "gt": 1}), true, false),
        ("dual-upper", serde_json::json!({"n": 0}), "n",
            serde_json::json!({"lte": 0, "lt": -1}), true, false),
        ("object-array", serde_json::json!({"obj": [{"n": 0}]}), "obj.n",
            serde_json::json!({"gte": 0}), true, false),
        ("float-source", serde_json::json!({"n": 0.5}), "n",
            serde_json::json!({"gte": 0, "lte": 1}), false, true),
        ("unbounded-null", serde_json::json!({"n": null}), "n",
    ...
    for shards in [1, 3] {
        for (name, source, field, bounds, native_expected, source_expected) in &cases {
            ...
            engine.index_document(IndexDocumentRequest {
                index: "range-guard".into(), id: "doc".into(), source: source.clone(),
            }).unwrap();
            engine.refresh(RefreshRequest {indices: vec!["range-guard".into()]}).unwrap();
            let store = engine.store.read().unwrap();
            ...
            let matched = index.score_document_query(&query, document).unwrap().is_some();
            assert_eq!(!native.is_empty(), *native_expected, "shards={shards} case={name}");
            ...
            if *native_expected && !source_expected {
                let query = Query::Bool {clauses: BoolQuery {
                    must: vec![Query::MatchAll], filter: vec![query], ..Default::default()
                }};
                let (total, hits) = index.search_hits_page_for_source_candidate_post_filter(
                    "range-guard", &query, &[], 0, 10, None,
                ).unwrap().unwrap();
...
fn deferred_bool_must_preserves_scores_and_error_visibility() {
    for shards in [1, 3] {
        let engine = TantivyEngine::default();
        engine.create_index(CreateIndexRequest {
            index:"deferred-bool".into(), settings:serde_json::json!({"number_of_shards":shards}),
            mappings:serde_json::json!({"properties":{
        ...
        engine.refresh(RefreshRequest {indices:vec!["deferred-bool".into()]}).unwrap();
        let store = engine.store.read().unwrap();
                ...
                for limit in [200, 2000] {
                    for excluded in ["selected", "absent"] {
                        for nested_bool in [false, true] {
                            ...
                            if nested_bool { must = serde_json::json!({"bool":{"must":must,"filter":{"match_all":{}}}}); }
                            ...
                            index.prepare_source_bm25_matches(&query, None, &mut context).unwrap();
                            assert_eq!(context.deferred_bool_must.len(), if nested_bool {2} else {1});
                            for document in index.refreshed_documents_for_shards(None) {
                                let expected = index.score_document_query(&query, document).unwrap();
                                let actual = index.score_document_query_with_bm25_context(&query, document, &mut context).unwrap();
                                assert_eq!(actual.map(f32::to_bits), expected.map(f32::to_bits),
                                    "shards={shards} {query:?} id={}", document.metadata.id);
                            }
                        }
                    }
                }
        ...
        let invalid = Query::Term {field:"title.raw".into(), value:serde_json::json!({"invalid":true}), case_insensitive:false};
        for clauses in [
            BoolQuery {must:vec![invalid.clone()], filter:vec![Query::MatchNone], ..Default::default()},
            BoolQuery {must:vec![Query::MatchNone], filter:vec![invalid.clone()], ..Default::default()},
            BoolQuery {must:vec![Query::MatchNone], should:vec![invalid], minimum_should_match:Some(1), ..Default::default()},
        ] {
            ...
            let actual = index.score_document_query_with_bm25_context(&query, document, &mut context);
            assert_eq!(format!("{actual:?}"), format!("{expected:?}"));
            if let Query::Bool {clauses} = &query {
                assert_eq!(actual.is_err(), !matches!(clauses.must[0], Query::MatchNone));
            }
        }
        ...
        assert!(index.score_document_query_with_bm25_context(
            &query, index.refreshed_document_by_id("1").unwrap(), &mut context,
        ).unwrap().is_none());
        assert!(context.query_tokens.is_empty(), "rejected candidate must not score its must clause");
    }
}
...
fn compact_ranking_scores_preserve_heap_accumulation_bits() {
    let engine = TantivyEngine::default();
    engine.create_index(CreateIndexRequest {
        index: "compact-ranking".into(), settings: serde_json::json!({"number_of_shards":3}),
        mappings: serde_json::json!({"properties": {
    ...
    for id in 0..9 {
        engine.index_document(IndexDocumentRequest {
            index: "compact-ranking".into(), id: id.to_string(),
            source: serde_json::json!({
    ...
    engine.refresh(RefreshRequest {indices:vec!["compact-ranking".into()]}).unwrap();
    let store = engine.store.read().unwrap();
    let index = &store.indices["compact-ranking"];
    let field_sets = [vec![], vec!["title"], vec!["title^2", "body^0.5"],
        vec!["title^0", "missing"], vec!["body", "title", "title^2"],
        vec!["title^0", "body^0.5", "title^8", "missing"]];
    for fields in field_sets {
        let fields = fields.into_iter().map(str::to_string).collect::<Vec<_>>();
        for kind in [MultiMatchType::BestFields, MultiMatchType::MostFields,
            ...
            for tie in [0.0, 0.3, 1.0, 2.0] {
                for text in ["alpha", "alpha beta", "alpha be", "missing", ""] {
                    ...
                    for document in index.refreshed_documents_for_shards(None) {
                        let mut scores = fields.iter().filter_map(|field| {
                            ...
                            match kind {
                                ...
                                _ => index.opensearch_phrase_bm25_score(
                                    field, &query, 1, matches!(kind, MultiMatchType::PhrasePrefix), document,
                                ),
                            }.map(|score| score * boost)
                        }).collect::<Vec<_>>();
                        let expected = if scores.is_empty() { None }
                        ...
                        let actual = index.opensearch_multi_match_bm25_score_with_bm25_context(
                            &fields, &query, kind, 1, tie, document, &mut Bm25Context::default(),
                        );
                        assert_eq!(actual.map(f32::to_bits), expected.map(f32::to_bits),
                            "{fields:?} {kind:?} {tie} {text} {}", document.metadata.id);
                    }
                }
            }
        }
    ...
    for must_boost in [0.0, 0.5, 100_000_000.0] {
        for should_count in 0..=8 {
            for minimum in 0..=should_count + 1 {
                let should = (0..should_count).map(|i| serde_json::json!({"match":{
                ...
                let Query::Bool {clauses} = &query else { unreachable!() };
                for document in index.refreshed_documents_for_shards(None) {
                    let must = index.score_document_query(&clauses.must[0], document).unwrap();
                    let should = clauses.should.iter().map(|q| index.score_document_query(q, document))
                        .collect::<EngineResult<Vec<_>>>().unwrap();
                    let expected = must.and_then(|must| {
                        if should.iter().flatten().count() < effective_bool_minimum_should_match(clauses) as usize {
                            return None;
                        }
                    ...
                    assert_eq!(actual.map(f32::to_bits), expected.map(f32::to_bits), "{query:?}");
                }
            }
        }
    }
...
fn source_bm25_query_tokens_are_shared_only_within_request() {
    let mut context = Bm25Context::default();
    for query in [serde_json::json!("Alpha beta beta"), serde_json::json!(42),
...
fn source_bm25_cached_queries_preserve_match_and_phrase_scores() {
    ...
    for (id, value) in [serde_json::json!("alpha beta beta"),
        serde_json::json!("beta alpha gamma"), serde_json::json!(["alpha", "beta"]),
        Value::Null].into_iter().enumerate() {
        engine.index_document(IndexDocumentRequest {
            index: "query-token-cache".into(), id: id.to_string(),
            source: serde_json::json!({"title":value}),
        }).unwrap();
    }
    engine.refresh(RefreshRequest {indices:vec!["query-token-cache".into()]}).unwrap();
    let store = engine.store.read().unwrap();
    ...
    for query in ["alpha beta", "beta beta", "alpha be", "", "missing"] {
        ...
        for id in 0..4 {
            ...
            assert_eq!(index.opensearch_match_bm25_score("title", &query, document),
                index.opensearch_match_bm25_score_with_bm25_context(
                    "title", &query, document, &mut context));
            for slop in [0, 1, 3] {
                for prefix in [false, true] {
                    assert_eq!(index.opensearch_phrase_bm25_score(
                        "title", &query, slop, prefix, document),
                        index.opensearch_phrase_bm25_score_with_bm25_context(
                            "title", &query, slop, prefix, document, &mut context));
                }
            }
        }
    }
...
fn postings_bm25_preserves_source_bits_and_unsafe_fields_fall_back() {
    for shards in [1, 3] {
        for field_override in [None, Some("x".repeat(39)), Some("x".repeat(40)), Some("\u{00e9}".into())] {
            let engine = TantivyEngine::default();
            engine.create_index(CreateIndexRequest {
            ...
            for id in 0..40 {
                ...
                if let Some(text) = &field_override { source["title"] = serde_json::json!(text); }
                engine.index_document(IndexDocumentRequest {
                    index:"postings-bm25".into(), id:id.to_string(), source,
                }).unwrap();
            }
            engine.refresh(RefreshRequest {indices:vec!["postings-bm25".into()]}).unwrap();
            let store = engine.store.read().unwrap();
            let index = &store.indices["postings-bm25"];
            for text in ["alpha beta alpha".to_string(), "missing".into(), "ALPHA gamma".into(),
                "alpha OR beta".into(), "x".repeat(39)] {
                for mode in ["best_fields", "most_fields"] {
                    ...
                    let mut context = Bm25Context::default();
                    index.prepare_source_bm25_matches(&query, None, &mut context).unwrap();
                    let safe_title = field_override.as_ref().map_or(true, |text| text.is_ascii() && text.len() < 40);
                    assert_eq!(context.native_matches.len(), if safe_title {2} else {1});
                    for document in index.refreshed_documents_for_shards(None) {
                        let expected = index.score_document_query(&query, document).unwrap().map(f32::to_bits);
                        let actual = index.score_document_query_with_bm25_context(
                            &query, document, &mut context,
                        ).unwrap().map(f32::to_bits);
                        assert_eq!(actual, expected, "shards={shards} mode={mode} text={text} id={}", document.metadata.id);
                    }
                }
            }
        }
...
fn postings_bm25_cache_budget_falls_back_without_losing_matches() {
    let engine = TantivyEngine::default();
    engine.create_index(CreateIndexRequest {
        index:"postings-budget".into(), settings:serde_json::json!({"number_of_shards":1}),
        mappings:serde_json::json!({"properties":{"title":{"type":"text"}}}),
    }).unwrap();
    engine.index_document(IndexDocumentRequest {
        index:"postings-budget".into(), id:"1".into(), source:serde_json::json!({"title":"alpha beta"}),
    }).unwrap();
    engine.refresh(RefreshRequest {indices:vec!["postings-budget".into()]}).unwrap();
    let store = engine.store.read().unwrap();
    ...
    context.native_matches.push(PreparedSourceBm25Match {
        field:"other".into(), query:serde_json::json!("other"),
        scores:BTreeMap::from([(0, (0..MAX_PREPARED_SOURCE_BM25_SCORES)
            .map(|id| (id.to_string(), 1.0)).collect())]),
    });
    index.prepare_source_bm25_matches(&query, None, &mut context).unwrap();
    assert_eq!(context.native_matches.len(), 1);
    let document = index.refreshed_document_by_id("1").unwrap();
    assert_eq!(index.score_document_query_with_bm25_context(&query, document, &mut context).unwrap(),
        index.score_document_query(&query, document).unwrap());
    assert!(index.score_document_query(&query, document).unwrap().is_some());
}
...
fn prepared_source_phrases_preserve_scores_and_fallback_contracts() {
    let engine = TantivyEngine::default();
    engine.create_index(CreateIndexRequest {
        index:"phrase-dispatch".into(), settings:serde_json::json!({"number_of_shards":3}),
        mappings:serde_json::json!({"properties":{
            "title":{"type":"text"}, "code":{"type":"keyword"}
        }}),
    }).unwrap();
    let titles = serde_json::json!(["alpha beta", "alpha gap beta", "alpha gamma", "beta alpha",
        "!!!", null, [], ["alpha", "beta"], "\u{00e9}clair beta"]);
    for (id, title) in titles.as_array().unwrap().iter().enumerate() {
        engine.index_document(IndexDocumentRequest {
            index:"phrase-dispatch".into(), id:id.to_string(),
            source:serde_json::json!({"title":title,"code":"alpha beta"}),
        }).unwrap();
    }
    engine.refresh(RefreshRequest {indices:vec!["phrase-dispatch".into()]}).unwrap();
    let store = engine.store.read().unwrap();
    let index = &store.indices["phrase-dispatch"];
    for field in ["title", "code"] {
        for kind in ["match_phrase", "match_phrase_prefix"] {
            for text in serde_json::json!(["alpha beta", "alpha be", "missing", "", "!!!", "\u{00e9}clair", 123, true])
                .as_array().unwrap() {
                for slop in [0, 1, 3] {
                    for analyzer in [None, Some("keyword")] {
                        for zero_all in [false, true] {
                            ...
                            if let Some(analyzer) = analyzer { spec["analyzer"] = serde_json::json!(analyzer); }
                            let query = parse_query(&serde_json::json!({(kind):{(field):spec}})).unwrap();
                            let mut context = Bm25Context::default();
                            index.prepare_source_bm25_matches(&query, None, &mut context).unwrap();
                            let eligible = field == "title" && analyzer.is_none() && !zero_all
                                && text.as_str().is_some_and(|text| text.chars().any(char::is_alphanumeric));
                            assert_eq!(context.complete_source_phrases.len(), usize::from(eligible));
                            for document in index.refreshed_documents_for_shards(None) {
                                let expected = index.score_document_query(&query, document).unwrap().map(f32::to_bits);
                                let actual = index.score_document_query_with_bm25_context(
                                    &query, document, &mut context,
                                ).unwrap().map(f32::to_bits);
                                assert_eq!(actual, expected, "{query:?} id={}", document.metadata.id);
                            }
                        }
                    }
                }
...
fn source_shard_statistics_and_native_pages_match_live_reference() {
    ...
    engine.create_index(CreateIndexRequest {
        index:name.into(), settings:body["settings"].clone(), mappings:body["mappings"].clone(),
    }).unwrap();
    ...
    let expected = [("len-1", 0.140054_f32), ("len-40", 0.130765),
        ("len-42", 0.083669), ("len-44", 0.082092), ("len-1000", 0.059399)];
    let mut context = Bm25Context::default();
    for case in fixture["cases"].as_array().unwrap() {
        let body = &case["steps"][0]["body"];
        let query = parse_query(&body["query"]).unwrap();
        let boost = if case["name"].as_str().unwrap().contains("multi-match") {2.0} else {1.0};
    ...
    assert_eq!(context.fields.values().map(|fields| fields["title"].doc_count).sum::<usize>(), 5);
}
...
fn native_bool_multi_match_scores_equal_source_before_page_selection() {
    ...
    for i in 0..40 {
        engine.index_document(IndexDocumentRequest {
            index: "native-bool-ranking".into(), id:format!("doc-{i:02}"),
            source:serde_json::json!({
    ...
    engine.refresh(RefreshRequest {indices:vec!["native-bool-ranking".into()]}).unwrap();
    let store = engine.store.read().unwrap();
    ...
    for kind in ["best_fields", "most_fields"] {
        for boost in [0.0, 0.5, 2.0] {
            ...
            let actual = state.searcher.search(built.as_ref(), &TopDocs::with_limit(40)).unwrap();
            assert_eq!(actual.len(), 40);
            for (score, address) in actual {
                let id = state.document_id_for_address(&state.searcher, address).unwrap();
                let document = index.refreshed_document_by_id(id).unwrap();
                let expected = index.score_document_query(&query, document).unwrap().unwrap();
...
fn shared_length_norm_matches_native_encoding_boundaries() {
    ...
    for length in (0..=65536).chain(boundaries).chain([u32::MAX]) {
        ...
        assert_eq!(os_core::bm25::normalized_document_length(length as usize), expected as usize,
            "length={length}");
    }
}
...
fn native_bm25_scaling_requires_field_statistics_and_non_scoring_filters() {
    ...
    impl tantivy::query::Bm25StatisticsProvider for FieldStatistics<'_> {
        ...
        fn total_num_docs(&self) -> tantivy::Result<u64> { Ok(self.documents) }
    ...
    for length in [5, 41, 1000] {
      for sparse in [false, true] {
        let engine = TantivyEngine::default();
        engine.create_index(CreateIndexRequest {
            index: "native-score-boundary".into(), settings: serde_json::json!({"number_of_shards":1}),
            mappings: serde_json::json!({"properties": {
                "title":{"type":"text"}, "service":{"type":"keyword"}
            }}),
        }).unwrap();
        for (id, title) in [("a", serde_json::json!("alpha")),
            ("b", serde_json::json!(format!("alpha {}", "other ".repeat(length - 1)))),
            ("c", if sparse { Value::Null } else { serde_json::json!("other") })] {
            engine.index_document(IndexDocumentRequest {
                index: "native-score-boundary".into(), id:id.into(),
                source:serde_json::json!({"title":title,"service":"yes"}),
            }).unwrap();
        }
        engine.refresh(RefreshRequest {indices:vec!["native-score-boundary".into()]}).unwrap();
        let store = engine.store.read().unwrap();
        ...
        let native_scores = |query: &Query| {
            let built = build_tantivy_query(state, query).unwrap().unwrap();
            state.searcher.search(built.as_ref(), &TopDocs::with_limit(10)).unwrap()
                .into_iter().map(|(score, address)| {
                    (state.document_id_for_address(&state.searcher, address).unwrap().to_owned(), score)
                }).collect::<BTreeMap<_, _>>()
        };
        let raw_query = QueryParser::for_index(&state.index,
            vec![state.searcher.schema().get_field("title").unwrap()])
            .parse_query("alpha").unwrap();
        let native = state.searcher.search(raw_query.as_ref(), &TopDocs::with_limit(10)).unwrap()
            .into_iter().map(|(score, address)| {
                (state.document_id_for_address(&state.searcher, address).unwrap().to_owned(), score)
            }).collect::<BTreeMap<_, _>>();
        ...
        for id in ["a", "b"] {
            let document = index.refreshed_document_by_id(id).unwrap();
            let source = index.score_document_query(&query, document).unwrap().unwrap();
            assert!((normalized[id] - source).abs() < 1e-6,
                "production native score length={length} sparse={sparse} id={id}");
            println!("sparse={sparse} id={id} native={} source={source}", native[id]);
            ratios.push(native[id] / source);
            if !sparse { assert!((native[id] / 2.2 - source).abs() < 1e-6); }
        }
        if sparse {
            assert!((ratios[0] - ratios[1]).abs() > 0.01,
        ...
        let documents = state.searcher.segment_readers().iter().map(|segment| {
            let norms = segment.get_fieldnorms_reader(field).unwrap();
            (0..segment.max_doc()).filter(|&doc| norms.fieldnorm(doc) != 0).count() as u64
        }).sum::<u64>();
        assert_eq!(documents, if sparse { 2 } else { 3 });
        let statistics = FieldStatistics { searcher: &state.searcher, documents };
        let built = raw_query;
        let weight = built.weight(tantivy::query::EnableScoring::enabled_from_statistics_provider(
            &statistics, &state.searcher,
        )).unwrap();
        for (_, address) in state.searcher.search(built.as_ref(), &TopDocs::with_limit(10)).unwrap() {
            let id = state.document_id_for_address(&state.searcher, address).unwrap();
            let document = index.refreshed_document_by_id(id).unwrap();
            let expected = index.score_document_query(&query, document).unwrap().unwrap();
            let mut scorer = weight.scorer(state.searcher.segment_reader(address.segment_ord), 1.0 / 2.2).unwrap();
            ...
            println!("field-statistics sparse={sparse} id={id} native={corrected} source={expected}");
        }
        ...
        for id in ["a", "b"] {
            assert_eq!(filtered_native[id], normalized[id], "filters must not add a native score");
            let document = index.refreshed_document_by_id(id).unwrap();
            assert_eq!(index.score_document_query(&filtered, document).unwrap(),
                index.score_document_query(&query, document).unwrap());
        }
        for minimum in [0, 1, 2] {
            ...
            assert_eq!(native_scores(&parse_query(&body).unwrap()), unfiltered,
...
fn bool_multi_match_pages_preserve_native_scores_before_truncation() {
    for shards in [1, 3] {
        let engine = TantivyEngine::default();
        engine.create_index(CreateIndexRequest {
            index: "bool-score-pages".into(), settings: serde_json::json!({"number_of_shards": shards}),
            mappings: serde_json::json!({"properties": {
                "title": {"type": "text"}, "service": {"type": "keyword"}
            }}),
        }).unwrap();
        ...
        engine.refresh(RefreshRequest { indices: vec!["bool-score-pages".into()] }).unwrap();
        let store = engine.store.read().unwrap();
        ...
        for mode in 0..3 {
            ...
            if mode == 1 { body = serde_json::json!({"bool": {"must": [body]}}); }
            if mode == 2 { body = serde_json::json!({"bool": {"should": [body]}}); }
            let query = parse_query(&body).unwrap();
            assert!(query_requires_native_candidate_post_filter(&query));
            assert!(query_allows_source_candidate_scan_for_native_post_filter(&query));
            assert!(index.native_compound_score_is_authoritative(&query, None));
            let native_scores = super::native_ranking_audit_tests::native_scores(index, &query, false);
            for selected in [None, Some(BTreeSet::new()), Some(BTreeSet::from([0])),
                Some((0..shards).collect::<BTreeSet<u32>>())] {
                for sort in [Vec::<SortSpec>::new(), vec![serde_json::from_value(
                    serde_json::json!({"field":"service", "order":"desc"})
                ).unwrap()]] {
                    let mut expected = index.refreshed_documents_for_shards(selected.as_ref()).into_iter()
                        .filter_map(|document| native_scores.get(&document.metadata.id)
                            .map(|&score| index.search_hit_for_document_with_score(
                                "bool-score-pages", document,
                                score, false,
                            )))
                        .collect::<Vec<_>>();
                    if sort.is_empty() {
                        expected.sort_by(compare_relevance_hits);
                    } else if let Some(mapped_sort) = MappedEngineSort::for_index(
                        "bool-score-pages", index, &sort,
                    ).unwrap() {
                    ...
                    for (from, size) in [(0, 1), (1, 3), (7, 10), (40, 5), (0, 0), (usize::MAX, 2)] {
                        ...
                        let actual = hits.into_iter().map(|hit| (hit.metadata.id, hit.score, hit.source)).collect::<Vec<_>>();
                        let page = expected.iter().skip(from).take(size)
                            .map(|hit| (hit.metadata.id.clone(), hit.score, hit.source.clone())).collect::<Vec<_>>();
...
fn field_annotations_preserve_existing_base_name_rules() {
    for (input, field, boost) in [
        ("title", "title", 1.0), ("title^2", "title", 2.0),
        ("object.title^0.5", "object.title", 0.5), ("_id^0", "_id", 0.0),
        ("a^b^3", "a^b", 3.0), ("^2", "^2", 1.0),
        ("title^NaN", "title^NaN", 1.0), ("title^inf", "title^inf", 1.0),
...
fn boosted_source_fields_match_base_fields_for_scalar_array_missing_and_id() {
    for source in [
        ...
        serde_json::json!({"title": null}), serde_json::json!({}),
    ] {
        for field in ["title", "object.title", "_id"] {
            for kind in ["best_fields", "most_fields", "phrase", "phrase_prefix", "bool_prefix"] {
                ...
                assert_eq!(document_matches_query(&boosted, "alpha", &source),
                    document_matches_query(&plain, "alpha", &source), "{kind}/{field}/{source}");
                assert_eq!(multi_match_matched_value_count("alpha", &source, &[format!("{field}^2")], &Value::String("alpha".into())),
                    multi_match_matched_value_count("alpha", &source, &[field.into()], &Value::String("alpha".into())));
            }
        }
    }
}
...
fn native_and_source_multi_match_apply_per_field_boost() {
    let engine = TantivyEngine::default();
    engine.create_index(CreateIndexRequest {
        index: "field-boost".into(), settings: serde_json::json!({"number_of_shards": 1}),
        mappings: serde_json::json!({"properties": {"title": {"type": "text"}}}),
    }).unwrap();
    engine.index_document(IndexDocumentRequest {
        index: "field-boost".into(), id: "doc".into(), source: serde_json::json!({"title": "alpha beta"}),
    }).unwrap();
    engine.refresh(RefreshRequest { indices: vec!["field-boost".into()] }).unwrap();
    let store = engine.store.read().unwrap();
    ...
    for kind in ["best_fields", "most_fields", "phrase", "phrase_prefix"] {
        let query_for = |field: &str| parse_query(&serde_json::json!({"multi_match": {
            "fields": [field], "query": "alpha", "type": kind
        }})).unwrap();
        ...
        let source_boosted = index.opensearch_text_bm25_score(&boosted, document).unwrap();
        assert!((source_boosted - 2.0 * source_plain).abs() < 1e-6, "source {kind}");
        assert_eq!(index.score_document_query(&boosted, document).unwrap(), Some(source_boosted), "post-filter {kind}");
        let native_score = |query: &Query| {
            let query = build_tantivy_query(state, query).unwrap().unwrap();
            state.searcher.search(query.as_ref(), &TopDocs::with_limit(1)).unwrap()[0].0
        };
        assert!((native_score(&boosted) - 2.0 * native_score(&plain)).abs() < 1e-6, "native {kind}");
    }
}
