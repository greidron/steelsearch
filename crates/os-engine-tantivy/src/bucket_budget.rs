...
impl BucketAllocationBudget {
    ...
    pub(crate) fn consume(&self, count: u64) -> EngineResult<()> {
        ...
        if total > u64::from(self.limit) {
            return Err(EngineError::TooManyBuckets { max_buckets: self.limit, bucket_count: total });
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn cumulative_limit_is_inclusive_and_failure_cannot_be_cleared() {
        ...
        let expected = Err(EngineError::TooManyBuckets { max_buckets: 5, bucket_count: 6 });
    ...
    fn zero_limit_and_overflow_do_not_disable_accounting() {
        ...
        assert_eq!(budget.consume(u64::MAX), Err(EngineError::TooManyBuckets {
            max_buckets: u32::MAX, bucket_count: u64::MAX,
        }));
