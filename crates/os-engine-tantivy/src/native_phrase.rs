...
impl NativePhraseQuery {
    pub(super) fn new(terms: Vec<(usize, Term)>, slop: u32) -> tantivy::Result<Self> {
        if terms.len() < 2
            || terms
                .windows(2)
                .any(|pair| pair[0].0 >= pair[1].0 || pair[0].1.field() != pair[1].1.field())
        {
        ...
        let term_ids = terms
            .iter()
            .map(|term| terms.iter().position(|other| other == term).unwrap())
            .collect();
        Ok(Self {
            terms,
            offsets,
...
impl Query for NativePhraseQuery {
    fn weight(&self, scoring: EnableScoring<'_>) -> tantivy::Result<Box<dyn Weight>> {
        ...
        let _timer =
            super::diagnostic_search::start(&super::diagnostic_search::NATIVE_PHRASE_WEIGHT);
        ...
        if record != Some(IndexRecordOption::WithFreqsAndPositions) {
            return Err(tantivy::TantivyError::SchemaError(
                "phrase field requires indexed positions".into(),
            ));
        }
...
impl PhraseWeight {
    fn phrase_scorer(
        ...
        #[cfg(feature = "diagnostic-search-timing")]
        let _timer =
            super::diagnostic_search::start(&super::diagnostic_search::NATIVE_PHRASE_SCORER);
        ...
        let mut scorer = PhraseScorer {
            ...
            similarity: self
                .similarity
                .as_ref()
                .map(|similarity| similarity.boost_by(boost)),
        };
        scorer.find_match();
        Ok(Some(scorer))
    }
...
impl Weight for PhraseWeight {
    ...
    fn explain(&self, reader: &SegmentReader, doc: DocId) -> tantivy::Result<Explanation> {
        ...
        if doc == TERMINATED || scorer.seek(doc) != doc || reader.is_deleted(doc) {
            return Err(tantivy::TantivyError::InvalidArgument(
                "document does not match phrase".into(),
            ));
        }
        ...
        if let Some(similarity) = &scorer.similarity {
            explanation.add_detail(
                similarity.explain_fractional(scorer.norms.fieldnorm_id(doc), scorer.frequency),
            );
        }
        Ok(explanation)
    }
...
impl PhraseScorer {
    fn find_match(&mut self) -> DocId {
        #[cfg(feature = "diagnostic-search-timing")]
        let _timer =
            super::diagnostic_search::start(&super::diagnostic_search::NATIVE_PHRASE_FIND_MATCH);
...
mod tests {
    ...
    fn native_fractional_bm25_preserves_integer_scores_and_explanation_values() {
        ...
        for norm in 0..=255 {
            for freq in [0, 1, 2, 10_000, u32::MAX] {
                assert_eq!(
                    weight.score(norm, freq).to_bits(),
                    weight.score_fractional(norm, freq as f32).to_bits()
                );
            }
            for freq in [0.0, 0.5, 1.0 / 3.0, 1.3333334, 100.5] {
                ...
                assert_eq!(
                    score.to_bits(),
                    weight.explain_fractional(norm, freq).value().to_bits()
                );
                assert_eq!(
                    weight.boost_by(2.0).score_fractional(norm, freq),
                    score * 2.0
                );
            }
        }
    ...
    fn native_phrase_scorer_seeks_and_preserves_refresh_snapshots() {
        ...
        for (ordinal, text) in [
            ...
            "alpha gamma beta",
            "alpha beta",
            "alpha gap beta gamma",
        ]
        .iter()
        .enumerate()
        {
            writer
                .add_document(doc!(id => ordinal as u64, body => *text))
                .unwrap();
        }
        ...
        let query = NativePhraseQuery::new(
            ["alpha", "beta", "gamma"]
                .iter()
                .enumerate()
                .map(|(offset, text)| (offset, Term::from_field_text(body, text)))
                .collect(),
            2,
        )
        .unwrap();
        let weight = query
            .weight(EnableScoring::disabled_from_searcher(&old))
            .unwrap();
        ...
        let ids = |searcher: &tantivy::Searcher| {
            searcher
        ...
        writer
            .add_document(doc!(id => 4u64, body => "alpha beta gamma"))
            .unwrap();
        writer.commit().unwrap();
        ...
        let scored = query
            .weight(EnableScoring::enabled_from_searcher(&old))
            .unwrap();
        for doc in [0, 1, 3] {
            ...
            assert_eq!(
                scored.explain(old.segment_reader(0), doc).unwrap().value(),
                base.score()
            );
        }
        assert!(scored.explain(old.segment_reader(0), 2).is_err());
        assert!(
            NativePhraseQuery::new(vec![(0, Term::from_field_text(body, "alpha"))], 1).is_err()
        );
    }
}
