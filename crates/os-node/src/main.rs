...
fn handle_transport_seed_connection<S: TransportConnection>(
        ...
        && normalized_action_hint == Some("internal:cluster/nodes/indices/shard/store/batch")
    {
        ...
        let response = build_empty_get_mappings_response(request_id, header_version_id);
        response_frame = summarize_transport_response_frame_for_action(
            &response,
...
fn build_empty_get_mappings_response(request_id: i64, header_version_id: u32) -> Vec<u8> {
    os_transport::action::build_opensearch_get_mappings_response_message(
        request_id,
        Version::from_id(header_version_id as i32),
        &os_transport::action::OpenSearchGetMappingsResponseWire::empty(),
    )
    .map(|frame| frame.to_vec())
    .unwrap_or_else(|_| build_empty_transport_response(request_id, header_version_id))
}
...
fn handle_subsequent_transport_request<S: TransportConnection>(
    ...
    let response = match normalized_action_hint {
        ...
        Some("indices:admin/mappings/get") => {
            Some(build_empty_get_mappings_response(request_id, header_version_id))
        }
...
mod tests {
  ...
  "rolling_upgrade_coverage": {"passed": true, "artifact_path": "rolling-upgrade.md"}
    ...
    fn get_aliases_transport_route_builds_opensearch_shaped_empty_response() {
        let response =
            build_empty_get_aliases_response(82, OPENSEARCH_3_7_0_TRANSPORT.id() as u32);
        let mut frame = BytesMut::from(&response[..]);
        let os_transport::frame::DecodedFrame::Message(message) =
            os_transport::frame::decode_frame(&mut frame)
                .unwrap()
    ...
    #[test]
    fn get_mappings_transport_route_builds_opensearch_shaped_empty_response() {
        let response =
            build_empty_get_mappings_response(84, OPENSEARCH_3_7_0_TRANSPORT.id() as u32);
        let mut frame = BytesMut::from(&response[..]);
        let os_transport::frame::DecodedFrame::Message(message) =
            os_transport::frame::decode_frame(&mut frame)
                .unwrap()
        ...
        assert!(response.empty_mapping_indices.is_empty());
    }
