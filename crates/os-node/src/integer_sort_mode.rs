...
pub(crate) fn reduce_integer_sort_values(values: &[i64], mode: IntegerSortMode) -> Option<i64> {
    ...
    match mode {
        IntegerSortMode::Avg => {
            ...
            let sum = values.iter().fold(0_i64, |sum, value| sum.wrapping_add(*value));
...
fn java_round(value: f64) -> i64 {
    ...
    (if value - floor >= 0.5 { floor + 1.0 } else { floor }) as i64
}
...
mod tests {
    ...
    fn half_values_round_toward_positive_infinity() {
        for (value, expected) in [(-2.5, -2), (-1.5, -1), (-0.5, 0),
                                  (0.5, 1), (1.5, 2), (2.5, 3)] {
    ...
    fn integer_extremes_and_singletons_preserve_java_semantics() {
        ...
        assert_eq!(reduce_integer_sort_values(&[i64::MAX, i64::MAX], IntegerSortMode::Avg), Some(-1));
        assert_eq!(reduce_integer_sort_values(&[i64::MIN, i64::MIN], IntegerSortMode::Avg), Some(0));
        assert_eq!(reduce_integer_sort_values(&[i64::MAX, i64::MAX], IntegerSortMode::Median), Some(i64::MAX));
        assert_eq!(reduce_integer_sort_values(&[i64::MIN, i64::MAX], IntegerSortMode::Median), Some(0));
    }

    #[test]
    fn unsorted_and_duplicate_values_keep_all_contributions() {
        assert_eq!(reduce_integer_sort_values(&[9, 2, 5], IntegerSortMode::Avg), Some(5));
        assert_eq!(reduce_integer_sort_values(&[9, 2, 5], IntegerSortMode::Median), Some(5));
        assert_eq!(reduce_integer_sort_values(&[9, 2, 2, 5], IntegerSortMode::Avg), Some(5));
        assert_eq!(reduce_integer_sort_values(&[9, 2, 2, 5], IntegerSortMode::Median), Some(4));
    }
}
