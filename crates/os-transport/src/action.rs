...
pub const OPENSEARCH_DELETE_COMPONENT_TEMPLATE_ACTION_NAME: &str =
...
pub const SOURCE_DERIVED_CLUSTER_ACTIONS: &[SourceTransportActionSpec] = &[
    ...
    SourceTransportActionSpec {
        action_name: REMOTE_INFO_ACTION_NAME,
        action_type: "RemoteInfoAction",
    ...
    SourceTransportActionSpec {
        action_name: DELETE_SNAPSHOT_ACTION_NAME,
        action_type: "DeleteSnapshotAction",
    ...
    SourceTransportActionSpec {
        action_name: CLONE_SNAPSHOT_ACTION_NAME,
        action_type: "CloneSnapshotAction",
...
pub const OPENSEARCH_PRIORITY_TRANSPORT_ACTIONS: &[OpenSearchPriorityTransportActionSpec] = &[
    ...
    OpenSearchPriorityTransportActionSpec {
        action_name: OPENSEARCH_GET_INDEX_TEMPLATES_ACTION_NAME,
        action_type: "GetIndexTemplatesAction",
        transport_action: "TransportGetIndexTemplatesAction",
        request_wire_type: "GetIndexTemplatesRequest",
        response_wire_type: "GetIndexTemplatesResponse",
        adapter_stage: "metadata-read",
    ...
    OpenSearchPriorityTransportActionSpec {
        ...
        request_wire_type: "DeleteIndexTemplateRequest",
        response_wire_type: "AcknowledgedResponse",
        adapter_stage: "metadata-write",
    ...
    OpenSearchPriorityTransportActionSpec {
        action_name: OPENSEARCH_DELETE_COMPONENT_TEMPLATE_ACTION_NAME,
        action_type: "DeleteComponentTemplateAction",
        transport_action: "TransportDeleteComponentTemplateAction",
        request_wire_type: "DeleteComponentTemplateAction.Request",
        response_wire_type: "AcknowledgedResponse",
        adapter_stage: "metadata-write",
        next_step: "map component-template deletion onto Rust template metadata mutation and ack rendering",
    },
    OpenSearchPriorityTransportActionSpec {
        action_name: OPENSEARCH_GET_COMPOSABLE_INDEX_TEMPLATE_ACTION_NAME,
        action_type: "GetComposableIndexTemplateAction",
        transport_action: "TransportGetComposableIndexTemplateAction",
        request_wire_type: "GetComposableIndexTemplateAction.Request",
    ...
    OpenSearchPriorityTransportActionSpec {
        action_name: OPENSEARCH_FIELD_CAPABILITIES_ACTION_NAME,
        action_type: "FieldCapabilitiesAction",
        transport_action: "TransportFieldCapabilitiesAction",
        request_wire_type: "FieldCapabilitiesRequest",
        response_wire_type: "FieldCapabilitiesResponse",
        adapter_stage: "metadata-read",
        next_step: "map field capabilities reads onto Rust mapping/type metadata response rendering",
    },
    OpenSearchPriorityTransportActionSpec {
    ...
    OpenSearchPriorityTransportActionSpec {
        action_name: OPENSEARCH_PIT_SEGMENTS_ACTION_NAME,
        action_type: "PitSegmentsAction",
        transport_action: "TransportPitSegmentsAction",
        request_wire_type: "PitSegmentsRequest",
        response_wire_type: "IndicesSegmentResponse",
        adapter_stage: "search-pit",
        next_step: "map PIT segment metadata reads onto Rust PIT context and shard segment metadata response rendering",
    },
    OpenSearchPriorityTransportActionSpec {
    ...
    OpenSearchPriorityTransportActionSpec {
        action_name: OPENSEARCH_DATA_STREAMS_STATS_ACTION_NAME,
        action_type: "DataStreamsStatsAction",
        transport_action: "DataStreamsStatsAction.TransportAction",
        request_wire_type: "DataStreamsStatsAction.Request",
...
pub fn build_delete_snapshot_request_message(
    request_id: i64,
    version: Version,
    request: &DeleteSnapshotRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(DELETE_SNAPSHOT_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_delete_snapshot_request_message(
    message: &TransportMessage,
) -> Result<DeleteSnapshotRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != DELETE_SNAPSHOT_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: DELETE_SNAPSHOT_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_clone_snapshot_request_message(
    request_id: i64,
    version: Version,
    request: &CloneSnapshotRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(CLONE_SNAPSHOT_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_clone_snapshot_request_message(
    message: &TransportMessage,
) -> Result<CloneSnapshotRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != CLONE_SNAPSHOT_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: CLONE_SNAPSHOT_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_cluster_put_weighted_routing_request_message(
    request_id: i64,
    version: Version,
    request: &ClusterPutWeightedRoutingRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(CLUSTER_ADD_WEIGHTED_ROUTING_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_cluster_put_weighted_routing_request_message(
    message: &TransportMessage,
) -> Result<ClusterPutWeightedRoutingRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != CLUSTER_ADD_WEIGHTED_ROUTING_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: CLUSTER_ADD_WEIGHTED_ROUTING_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_opensearch_get_index_templates_request_message(
    request_id: i64,
    version: Version,
    request: &OpenSearchGetIndexTemplatesRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(OPENSEARCH_GET_INDEX_TEMPLATES_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_opensearch_get_index_templates_request_message(
    message: &TransportMessage,
) -> Result<OpenSearchGetIndexTemplatesRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != OPENSEARCH_GET_INDEX_TEMPLATES_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: OPENSEARCH_GET_INDEX_TEMPLATES_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_opensearch_create_index_request_message(
    ...
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(OPENSEARCH_CREATE_INDEX_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_opensearch_create_index_request_message(
    message: &TransportMessage,
...
pub fn build_opensearch_delete_index_template_request_message(
    request_id: i64,
    version: Version,
    request: &OpenSearchDeleteIndexTemplateRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(OPENSEARCH_DELETE_INDEX_TEMPLATE_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_opensearch_delete_index_template_request_message(
    message: &TransportMessage,
) -> Result<OpenSearchDeleteIndexTemplateRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != OPENSEARCH_DELETE_INDEX_TEMPLATE_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: OPENSEARCH_DELETE_INDEX_TEMPLATE_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_opensearch_delete_component_template_request_message(
    request_id: i64,
    version: Version,
    request: &OpenSearchDeleteComponentTemplateRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(OPENSEARCH_DELETE_COMPONENT_TEMPLATE_ACTION_NAME)
                .to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
...
pub fn read_opensearch_delete_component_template_request_message(
    message: &TransportMessage,
) -> Result<OpenSearchDeleteComponentTemplateRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != OPENSEARCH_DELETE_COMPONENT_TEMPLATE_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: OPENSEARCH_DELETE_COMPONENT_TEMPLATE_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_opensearch_get_composable_index_template_request_message(
    request_id: i64,
    version: Version,
    request: &OpenSearchGetComposableIndexTemplateRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(OPENSEARCH_GET_COMPOSABLE_INDEX_TEMPLATE_ACTION_NAME)
                .to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
...
pub fn read_opensearch_get_composable_index_template_request_message(
    message: &TransportMessage,
) -> Result<OpenSearchGetComposableIndexTemplateRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != OPENSEARCH_GET_COMPOSABLE_INDEX_TEMPLATE_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: OPENSEARCH_GET_COMPOSABLE_INDEX_TEMPLATE_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_opensearch_field_capabilities_request_message(
    request_id: i64,
    version: Version,
    request: &OpenSearchFieldCapabilitiesRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(OPENSEARCH_FIELD_CAPABILITIES_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_opensearch_field_capabilities_request_message(
    message: &TransportMessage,
) -> Result<OpenSearchFieldCapabilitiesRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != OPENSEARCH_FIELD_CAPABILITIES_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: OPENSEARCH_FIELD_CAPABILITIES_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_opensearch_pit_segments_request_message(
    request_id: i64,
    version: Version,
    request: &OpenSearchPitSegmentsRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(OPENSEARCH_PIT_SEGMENTS_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_opensearch_pit_segments_request_message(
    message: &TransportMessage,
) -> Result<OpenSearchPitSegmentsRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != OPENSEARCH_PIT_SEGMENTS_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: OPENSEARCH_PIT_SEGMENTS_ACTION_NAME,
            actual: header.action,
        });
    }
...
pub fn build_opensearch_data_streams_stats_request_message(
    request_id: i64,
    version: Version,
    request: &OpenSearchDataStreamsStatsRequestWire,
) -> Result<BytesMut, TransportActionWireError> {
    let mut body = StreamOutput::new();
    request.write(&mut body);
    let message = TransportMessage {
        ...
        version,
        variable_header: BytesMut::from(
            &RequestVariableHeader::new(OPENSEARCH_DATA_STREAMS_STATS_ACTION_NAME).to_bytes()[..],
        ),
        body: BytesMut::from(&body.freeze()[..]),
    };
    Ok(encode_message(&message))
...
pub fn read_opensearch_data_streams_stats_request_message(
    message: &TransportMessage,
) -> Result<OpenSearchDataStreamsStatsRequestWire, TransportActionWireError> {
    if !message.status.is_request() {
        return Err(TransportActionWireError::UnexpectedMessageStatus {
            expected: "request",
            actual: message.status.bits(),
    ...
    let header = RequestVariableHeader::read(message.variable_header.clone().freeze())?;
    if header.action != OPENSEARCH_DATA_STREAMS_STATS_ACTION_NAME {
        return Err(TransportActionWireError::UnexpectedAction {
            expected: OPENSEARCH_DATA_STREAMS_STATS_ACTION_NAME,
            actual: header.action,
        });
    }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct DeleteSnapshotRequestWire {
    pub parent_task_node: String,
    pub parent_task_id: Option<i64>,
    pub cluster_manager_timeout: TimeValueWire,
    pub repository: String,
...
impl Default for DeleteSnapshotRequestWire {
    fn default() -> Self {
        Self {
            parent_task_node: String::new(),
            parent_task_id: None,
            cluster_manager_timeout: TimeValueWire::seconds(30),
            repository: "repo".to_string(),
            snapshots: vec!["snap".to_string()],
        }
    }
}

impl DeleteSnapshotRequestWire {
    pub fn write(&self, output: &mut StreamOutput) {
        write_parent_task_id(output, &self.parent_task_node, self.parent_task_id);
        self.cluster_manager_timeout.write(output);
        output.write_string(&self.repository);
    ...
    pub fn reject_unsupported_execution(&self) -> Result<(), TransportActionWireError> {
        if self.cluster_manager_timeout != TimeValueWire::seconds(30) {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason: "custom cluster-manager timeout requires snapshot deletion coordination semantics",
            });
        }
        if self.repository.trim().is_empty() {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "delete snapshot missing repository",
                reason: "OpenSearch delete-snapshot requests require a repository name",
            });
        }
        if self.snapshots.is_empty()
            || self
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CloneSnapshotRequestWire {
    pub parent_task_node: String,
    pub parent_task_id: Option<i64>,
    pub cluster_manager_timeout: TimeValueWire,
    pub repository: String,
...
impl Default for CloneSnapshotRequestWire {
    fn default() -> Self {
        Self {
            parent_task_node: String::new(),
            parent_task_id: None,
            cluster_manager_timeout: TimeValueWire::seconds(30),
            repository: "repo".to_string(),
            source: "snap-a".to_string(),
            target: "snap-b".to_string(),
...
impl CloneSnapshotRequestWire {
    pub fn write(&self, output: &mut StreamOutput) {
        write_parent_task_id(output, &self.parent_task_node, self.parent_task_id);
        self.cluster_manager_timeout.write(output);
        output.write_string(&self.repository);
        output.write_string(&self.source);
        output.write_string(&self.target);
        output.write_string_array(&self.indices);
        self.indices_options.write(output);
    }
    ...
    pub fn reject_unsupported_execution(&self) -> Result<(), TransportActionWireError> {
        if self.cluster_manager_timeout != TimeValueWire::seconds(30) {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason:
                    "custom cluster-manager timeout requires snapshot clone coordination semantics",
            });
        }
        if self.repository.trim().is_empty() {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "clone snapshot missing repository",
                reason: "OpenSearch clone-snapshot requests require a repository name",
            });
        }
        if self.source.trim().is_empty() {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "clone snapshot missing source",
                reason: "OpenSearch clone-snapshot requests require a source snapshot name",
            });
        }
    ...
    pub fn reject_unsupported_execution(&self) -> Result<(), TransportActionWireError> {
        if self.cluster_manager_timeout != TimeValueWire::seconds(30) {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason:
                    "custom cluster-manager timeout is not mapped by the get-index-templates adapter yet",
            });
        }
        if self.local {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "get index templates local",
                reason: "local index-template reads require local cluster-state response semantics",
            });
        }
    ...
    pub fn reject_unsupported_execution(&self) -> Result<(), TransportActionWireError> {
        if self.cluster_manager_timeout != TimeValueWire::seconds(30) {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason:
                    "custom cluster-manager timeout is not mapped by the create-index adapter yet",
            });
        }
        if self.ack_timeout != TimeValueWire::seconds(30) {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "create index ack timeout",
                reason: "custom ack timeout is not mapped by the create-index adapter yet",
            });
        }
        ...
        if self.has_context {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "create index context",
                reason: "create-index context requires context metadata mutation semantics",
            });
        }
        Err(TransportActionWireError::UnsupportedWireShape {
    ...
    pub fn reject_unsupported_execution(&self) -> Result<(), TransportActionWireError> {
        if self.cluster_manager_timeout != TimeValueWire::seconds(30) {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason:
                    "custom cluster-manager timeout is not mapped by the get-composable-index-template adapter yet",
            });
        }
        if self.local {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason:
                    "local composable index-template reads require local cluster-state response semantics",
            });
        }
        if self.name.as_deref().is_some_and(|name| !name.is_empty()) {
            return Err(TransportActionWireError::UnsupportedWireShape {
    ...
    pub fn reject_unsupported_execution(&self) -> Result<(), TransportActionWireError> {
        if self.fields.is_empty() {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason: "OpenSearch field-capabilities requests require at least one field",
            });
        }
        if !self.indices.is_empty() {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "field capabilities index filter",
                reason: "index-scoped field-capabilities reads require index resolution semantics",
            });
        }
        if self.indices_options != OpenSearchIndicesOptionsWire::strict_expand_open() {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason:
                    "custom field-capabilities indices options require index resolution semantics",
            });
        }
    ...
    pub fn reject_unsupported_execution(&self) -> Result<(), TransportActionWireError> {
        if !self.indices.is_empty() {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason: "index-scoped PIT segment reads require PIT index resolution semantics",
            });
        }
        if self.indices_options != OpenSearchIndicesOptionsWire::strict_expand_open_forbid_closed()
        {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "pit segments indices options",
    ...
    pub fn reject_unsupported_execution(&self) -> Result<(), TransportActionWireError> {
        if !self.indices.is_empty() {
            return Err(TransportActionWireError::UnsupportedWireShape {
                ...
                reason: "name-scoped data-stream stats require data-stream stats filtering",
            });
        }
        if self.indices_options != OpenSearchIndicesOptionsWire::strict_expand_open_forbid_closed()
        {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "data streams stats indices options",
                reason:
...
fn read_generic_string_double_map(
    input: &mut StreamInput,
) -> Result<BTreeMap<String, f64>, TransportActionWireError> {
    match input.read_byte()? {
        9 | 10 => {
            ...
            for _ in 0..len {
                ...
                let generic_type = input.read_byte()?;
                if generic_type != 4 {
                    return Err(TransportActionWireError::UnsupportedWireShape {
                        shape: "generic double map value",
                        reason: "weighted routing wire payload only supports string keys with double weight values",
                    });
                }
                values.insert(key, input.read_f64()?);
            }
            Ok(values)
        }
        255 => Err(TransportActionWireError::UnsupportedWireShape {
            shape: "generic double map null",
            reason: "OpenSearch put-weighted-routing requests require a non-null weights map",
        }),
        _ => Err(TransportActionWireError::UnsupportedWireShape {
            shape: "generic double map type",
            reason: "weighted routing wire payload must be encoded as an OpenSearch generic map",
        }),
    }
}
...
fn write_opensearch_settings_string_map(
    output: &mut StreamOutput,
...
fn read_opensearch_settings_string_map(
    input: &mut StreamInput,
    ...
    for _ in 0..len {
        ...
        if generic_type != 0 {
            return Err(TransportActionWireError::UnsupportedWireShape {
                shape: "settings generic value",
                reason:
                    "only string-valued OpenSearch settings are decoded by this transport boundary",
            });
        }
        settings.insert(key, input.read_string()?);
    }
    Ok(settings)
}
...
mod tests {
    ...
    fn source_derived_cluster_actions_have_opensearch_names_and_wire_types() {
        assert_eq!(
            ...
            &[
                ...
                SourceTransportActionSpec {
                    action_name: "cluster:monitor/remote/info",
                    action_type: "RemoteInfoAction",
                    transport_action: "TransportRemoteInfoAction",
                    request_wire_type: "RemoteInfoRequest",
                    response_wire_type: "RemoteInfoResponse",
                ...
                SourceTransportActionSpec {
                    action_name: "cluster:admin/snapshot/delete",
                    action_type: "DeleteSnapshotAction",
                    transport_action: "TransportDeleteSnapshotAction",
                    request_wire_type: "DeleteSnapshotRequest",
                    response_wire_type: "AcknowledgedResponse",
                ...
                SourceTransportActionSpec {
                    action_name: "cluster:admin/snapshot/clone",
                    action_type: "CloneSnapshotAction",
                    transport_action: "TransportCloneSnapshotAction",
                    request_wire_type: "CloneSnapshotRequest",
                    response_wire_type: "AcknowledgedResponse",
    ...
    fn opensearch_priority_transport_actions_have_source_names_and_stages() {
        assert_eq!(
            ...
            &[
                ...
                OpenSearchPriorityTransportActionSpec {
                    action_name: "indices:admin/template/get",
                    action_type: "GetIndexTemplatesAction",
                    transport_action: "TransportGetIndexTemplatesAction",
                    request_wire_type: "GetIndexTemplatesRequest",
                    response_wire_type: "GetIndexTemplatesResponse",
                ...
                OpenSearchPriorityTransportActionSpec {
                    action_name: "cluster:admin/component_template/delete",
                    action_type: "DeleteComponentTemplateAction",
                    transport_action: "TransportDeleteComponentTemplateAction",
                    request_wire_type: "DeleteComponentTemplateAction.Request",
                    response_wire_type: "AcknowledgedResponse",
                ...
                OpenSearchPriorityTransportActionSpec {
                    action_name: "indices:admin/index_template/get",
                    action_type: "GetComposableIndexTemplateAction",
                    transport_action: "TransportGetComposableIndexTemplateAction",
                    request_wire_type: "GetComposableIndexTemplateAction.Request",
                    response_wire_type: "GetComposableIndexTemplateAction.Response",
                ...
                OpenSearchPriorityTransportActionSpec {
                    action_name: "indices:data/read/field_caps",
                    action_type: "FieldCapabilitiesAction",
                    transport_action: "TransportFieldCapabilitiesAction",
                    request_wire_type: "FieldCapabilitiesRequest",
                    response_wire_type: "FieldCapabilitiesResponse",
                ...
                OpenSearchPriorityTransportActionSpec {
                    action_name: "indices:monitor/point_in_time/segments",
                    action_type: "PitSegmentsAction",
                    transport_action: "TransportPitSegmentsAction",
                    request_wire_type: "PitSegmentsRequest",
                    response_wire_type: "IndicesSegmentResponse",
                ...
                OpenSearchPriorityTransportActionSpec {
                    action_name: "indices:monitor/data_stream/stats",
                    action_type: "DataStreamsStatsAction",
                    transport_action: "DataStreamsStatsAction.TransportAction",
                    request_wire_type: "DataStreamsStatsAction.Request",
                    response_wire_type: "DataStreamsStatsAction.Response",
    ...
    #[test]
    fn opensearch_create_index_transport_messages_bind_rejected_action_frame() {
        let request = OpenSearchCreateIndexRequestWire::default();
        let mut frame =
    ...
    #[test]
    fn opensearch_pit_segments_request_rejects_unsupported_shapes() {
        let index_filter = OpenSearchPitSegmentsRequestWire {
            indices: vec!["logs".to_string()],
    ...
    fn opensearch_pit_segments_transport_messages_bind_rejected_action_frame() {
        ...
        let mut frame =
            build_opensearch_pit_segments_request_message(54, OPENSEARCH_3_7_0_TRANSPORT, &request)
                .unwrap();
        let DecodedFrame::Message(message) = decode_frame(&mut frame).unwrap().unwrap() else {
            panic!("expected PIT segments request message");
        };
        assert_eq!(
    ...
    #[test]
    fn opensearch_data_streams_stats_request_rejects_unsupported_shapes() {
        let name_filter = OpenSearchDataStreamsStatsRequestWire {
            indices: vec!["logs".to_string()],
            ..OpenSearchDataStreamsStatsRequestWire::default()
        };
        assert!(matches!(
            name_filter.reject_unsupported_execution(),
            Err(TransportActionWireError::UnsupportedWireShape {
        ...
        let custom_options = OpenSearchDataStreamsStatsRequestWire {
            indices_options: OpenSearchIndicesOptionsWire::strict_expand_open(),
            ..OpenSearchDataStreamsStatsRequestWire::default()
        };
        assert!(matches!(
    ...
    fn opensearch_data_streams_stats_transport_messages_bind_rejected_action_frame() {
        ...
        .unwrap();
        let DecodedFrame::Message(message) = decode_frame(&mut frame).unwrap().unwrap() else {
            panic!("expected data streams stats request message");
        };
        assert_eq!(
    ...
    #[test]
    fn opensearch_delete_pit_request_wire_round_trips_and_rejects_execution_boundary() {
    ...
    #[test]
    fn opensearch_delete_pit_transport_messages_bind_rejected_action_frame() {
        let request = OpenSearchDeletePitRequestWire::default();
        let mut frame =
