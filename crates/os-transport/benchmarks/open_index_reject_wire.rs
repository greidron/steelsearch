...
use os_transport::action::{
    build_opensearch_open_index_request_message, read_opensearch_open_index_request_message,
    OpenSearchOpenIndexRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure("open_index_reject_request_encode", ITERATIONS, || {
    ...
    let request_decode = measure("open_index_reject_request_decode", ITERATIONS, || {
    ...
    let reject_validate = measure("open_index_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_open_index_request_message(black_box(&message))
            .expect("open-index request decode");
        let err = decoded
            ...
            .expect_err("open-index execution should reject");
        black_box(err);
    });
    ...
    println!("open_index_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}");
}
