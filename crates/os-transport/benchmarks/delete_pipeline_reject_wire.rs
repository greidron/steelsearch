...
use os_transport::action::{
    build_opensearch_delete_pipeline_request_message,
    build_opensearch_delete_pipeline_response_message,
    read_opensearch_delete_pipeline_request_message,
    read_opensearch_delete_pipeline_response_message, AcknowledgedResponseWire,
    OpenSearchDeletePipelineRequestWire,
};
...
fn main() {
    ...
    let response_frame = build_opensearch_delete_pipeline_response_message(
        76,
        OPENSEARCH_3_7_0_TRANSPORT,
        &response,
    )
    ...
    let response_decode = measure("delete_pipeline_ack_response_decode", ITERATIONS, || {
        ...
        let decoded = read_opensearch_delete_pipeline_response_message(black_box(&message))
            .expect("delete-pipeline response decode");
        black_box(decoded);
    });
