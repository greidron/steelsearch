...
fn main() {
    ...
    let request_encode = measure("delete_pit_request_encode", ITERATIONS, || {
        ...
        .expect("delete-PIT request encode should succeed");
        black_box(frame);
    });
    ...
    let request_decode = measure("delete_pit_request_decode", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_delete_pit_request_message(black_box(&message))
            .expect("delete-PIT request decode");
        black_box(decoded);
    });

    let request_validate = measure("delete_pit_request_validate", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_delete_pit_request_message(black_box(&message))
            .expect("delete-PIT request decode");
    ...
    let response_encode = measure("delete_pit_response_encode", ITERATIONS, || {
        let frame = build_opensearch_delete_pit_response_message(
            52,
            OPENSEARCH_3_7_0_TRANSPORT,
            black_box(&response),
        )
        .expect("delete-PIT response encode should succeed");
        black_box(frame);
    });
    ...
    let response_decode = measure("delete_pit_response_decode", ITERATIONS, || {
        let mut frame = black_box(response_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_delete_pit_response_message(black_box(&message))
            .expect("delete-PIT response decode");
        black_box(decoded);
    });
    ...
    println!("delete_pit_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}");
}
