...
use os_transport::action::{
    ...
    read_opensearch_indices_segments_request_message, OpenSearchIndicesSegmentsRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure("indices_segments_reject_request_encode", ITERATIONS, || {
    ...
    let request_decode = measure("indices_segments_reject_request_decode", ITERATIONS, || {
    ...
    let reject_validate = measure("indices_segments_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_indices_segments_request_message(black_box(&message))
            .expect("indices segments request decode");
        let err = decoded
            ...
            .expect_err("indices segments execution should reject");
        black_box(err);
    });
    ...
    println!("indices_segments_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}");
}
