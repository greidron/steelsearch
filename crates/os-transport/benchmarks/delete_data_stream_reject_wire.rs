...
use os_transport::action::{
    build_opensearch_delete_data_stream_request_message,
    build_opensearch_delete_data_stream_response_message,
    read_opensearch_delete_data_stream_request_message,
    read_opensearch_delete_data_stream_response_message, AcknowledgedResponseWire,
    OpenSearchDeleteDataStreamRequestWire,
};
...
fn main() {
    ...
    let response_frame = build_opensearch_delete_data_stream_response_message(
        79,
        OPENSEARCH_3_7_0_TRANSPORT,
        &response,
    )
    .expect("delete-data-stream response encode should succeed");

    let response_decode = measure("delete_data_stream_ack_response_decode", ITERATIONS, || {
        let mut frame = black_box(response_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_delete_data_stream_response_message(black_box(&message))
            .expect("delete-data-stream response decode");
        black_box(decoded);
    });
