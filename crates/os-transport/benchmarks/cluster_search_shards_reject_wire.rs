...
use os_transport::action::{
    ...
    OpenSearchClusterSearchShardsRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure(
        "cluster_search_shards_reject_request_encode",
        ITERATIONS,
        || {
    ...
    let request_decode = measure(
        "cluster_search_shards_reject_request_decode",
        ITERATIONS,
        || {
            let mut frame = black_box(request_frame.clone());
            let message = decode_message(&mut frame);
            let decoded =
                read_opensearch_cluster_search_shards_request_message(black_box(&message))
    ...
    let reject_validate = measure(
        "cluster_search_shards_reject_validation",
        ITERATIONS,
        || {
            let mut frame = black_box(request_frame.clone());
            let message = decode_message(&mut frame);
            ...
            let err = decoded
                ...
                .expect_err("cluster search shards execution should reject");
            black_box(err);
        },
    );
    ...
    println!(
        "cluster_search_shards_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}"
    );
}
