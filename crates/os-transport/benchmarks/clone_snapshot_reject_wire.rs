...
use os_transport::action::{
    build_clone_snapshot_request_message, read_clone_snapshot_request_message,
    CloneSnapshotRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure("clone_snapshot_reject_request_encode", ITERATIONS, || {
    ...
    let request_decode = measure("clone_snapshot_reject_request_decode", ITERATIONS, || {
    ...
    let reject_validate = measure("clone_snapshot_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_clone_snapshot_request_message(black_box(&message))
            .expect("clone snapshot request decode");
        let err = decoded
            ...
            .expect_err("clone snapshot execution should reject");
        black_box(err);
    });
    ...
    println!("clone_snapshot_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}");
}
