...
use os_transport::action::{
    build_opensearch_get_all_pits_request_message, read_opensearch_get_all_pits_request_message,
    OpenSearchGetAllPitsRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure("get_all_pits_reject_request_encode", ITERATIONS, || {
    ...
    let request_decode = measure("get_all_pits_reject_request_decode", ITERATIONS, || {
    ...
    let reject_validate = measure("get_all_pits_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_get_all_pits_request_message(black_box(&message))
            .expect("get-all-PITs request decode");
        let err = decoded
            ...
            .expect_err("get-all-PITs execution should reject");
        black_box(err);
    });
    ...
    println!("get_all_pits_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}");
}
