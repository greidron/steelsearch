...
use os_transport::action::{
    build_opensearch_get_aliases_request_message, read_opensearch_get_aliases_request_message,
    OpenSearchGetAliasesRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure("get_aliases_reject_request_encode", ITERATIONS, || {
    ...
    let request_decode = measure("get_aliases_reject_request_decode", ITERATIONS, || {
    ...
    let reject_validate = measure("get_aliases_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_get_aliases_request_message(black_box(&message))
            .expect("get aliases request decode");
        let err = decoded
            ...
            .expect_err("get aliases execution should reject");
        black_box(err);
    });
    ...
    println!("get_aliases_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}");
}
