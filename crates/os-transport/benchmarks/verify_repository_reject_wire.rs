...
use os_transport::action::{
    build_verify_repository_request_message, read_verify_repository_request_message,
    VerifyRepositoryRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure(
        "verify_repository_reject_request_encode",
        ITERATIONS,
        || {
    ...
    let request_decode = measure(
        "verify_repository_reject_request_decode",
        ITERATIONS,
        || {
    ...
    let reject_validate = measure("verify_repository_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_verify_repository_request_message(black_box(&message))
            .expect("verify repository request decode");
        let err = decoded
            ...
            .expect_err("verify repository execution should reject");
        black_box(err);
    });
    ...
    println!(
        "verify_repository_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}"
    );
}
