...
use os_transport::action::{
    build_opensearch_put_pipeline_request_message, build_opensearch_put_pipeline_response_message,
    read_opensearch_put_pipeline_request_message, read_opensearch_put_pipeline_response_message,
    AcknowledgedResponseWire, OpenSearchPutPipelineRequestWire,
};
...
fn main() {
    ...
    let response_decode = measure("put_pipeline_ack_response_decode", ITERATIONS, || {
        ...
        let decoded = read_opensearch_put_pipeline_response_message(black_box(&message))
            .expect("put-pipeline response decode");
        black_box(decoded);
    });
