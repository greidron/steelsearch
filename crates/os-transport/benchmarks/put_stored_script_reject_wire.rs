...
use os_transport::action::{
    ...
    read_opensearch_put_stored_script_request_message, OpenSearchPutStoredScriptRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure(
        "put_stored_script_reject_request_encode",
        ITERATIONS,
        || {
    ...
    let request_decode = measure(
        "put_stored_script_reject_request_decode",
        ITERATIONS,
        || {
    ...
    let reject_validate = measure("put_stored_script_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_put_stored_script_request_message(black_box(&message))
            .expect("put-stored-script request decode");
        let err = decoded
            ...
            .expect_err("put-stored-script execution should reject");
        black_box(err);
    });
    ...
    println!(
        "put_stored_script_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}"
    );
}
