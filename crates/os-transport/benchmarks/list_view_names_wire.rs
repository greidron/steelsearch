...
use os_transport::action::{
    build_opensearch_list_view_names_request_message,
    build_opensearch_list_view_names_response_message,
    read_opensearch_list_view_names_request_message,
    read_opensearch_list_view_names_response_message, OpenSearchListViewNamesRequestWire,
    OpenSearchListViewNamesResponseWire,
};
...
fn main() {
    ...
    let response_decode = measure("list_view_names_response_decode", ITERATIONS, || {
        let mut frame = black_box(response_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_opensearch_list_view_names_response_message(black_box(&message))
            .expect("list-view-names response decode");
        black_box(decoded);
    });
