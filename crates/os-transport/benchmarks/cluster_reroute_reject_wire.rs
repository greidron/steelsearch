...
use os_transport::action::{
    build_cluster_reroute_request_message, read_cluster_reroute_request_message,
    ClusterRerouteRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure("cluster_reroute_reject_request_encode", ITERATIONS, || {
    ...
    let request_decode = measure("cluster_reroute_reject_request_decode", ITERATIONS, || {
    ...
    let reject_validate = measure("cluster_reroute_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded = read_cluster_reroute_request_message(black_box(&message))
            .expect("cluster reroute request decode");
        let err = decoded
            ...
            .expect_err("cluster reroute execution should reject");
        black_box(err);
    });
    ...
    println!("cluster_reroute_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}");
}
