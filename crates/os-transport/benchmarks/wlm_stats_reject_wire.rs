...
use os_transport::action::{
    build_wlm_stats_request_message, read_wlm_stats_request_message, WlmStatsRequestWire,
};
...
fn main() {
    ...
    let request_encode = measure("wlm_stats_reject_request_encode", ITERATIONS, || {
        let frame =
    ...
    let request_decode = measure("wlm_stats_reject_request_decode", ITERATIONS, || {
    ...
    let reject_validate = measure("wlm_stats_reject_validation", ITERATIONS, || {
        let mut frame = black_box(request_frame.clone());
        let message = decode_message(&mut frame);
        let decoded =
            read_wlm_stats_request_message(black_box(&message)).expect("wlm stats request decode");
        let err = decoded
            ...
            .expect_err("wlm stats execution should reject");
        black_box(err);
    });
    ...
    println!("wlm_stats_reject_wire_bottleneck_ops_per_second={combined_ops_per_second:.2}");
}
